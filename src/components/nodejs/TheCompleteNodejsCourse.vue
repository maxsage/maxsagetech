    ...
    <div class="panel-group">
      <div class="panel panel-primary">
        ...
        <div class="panel-body">
          ...
          <h2>Section 9: Mongoose - Modelling Relationships Between Connected Data</h2>
          <ul>
          ...
          <h2>Section 12: Unit Testings</h2>
          <ul>
          ...
          <h2>Section 13: Integration Testing</h2>
          <ul>
          ...
          <h2>Section 14: Test-driven Development</h2>
          <ul>
          ...
          <p>Node is a runtime environment for executing Javascript code. Prior to node Javascript was only used to
            ...
            Javascript code and converts it to machine code.
          </p>
          ...
          <p>A browser provides a run-time environment for Javascript. In browsers we have window and document objects
            which allow us to work in the browser environment.
          </p>
          ...
          <p>In 2009 Ryan Dahl took Google's V8 engine and embedded it inside a C++ program called Node - so similar to
            ...
            with the file system or os for interrogating the Operating System.
          </p>
          ...
          <p>In contrast blocking or synchronous architecture (ASP.Net or Rails) means that a single thread is allocated
            ...
            clients waiting for a thread to become available.
          </p>
          ...
          <p>Node applications are asynchronous by default making it ideal for I/O intensive apps (disk or network).
            ...
            applications involve a lot of CPU calculations but few operations that touch the file or network systems.
          </p>

          <p>Because Node applications are single threaded when performing the calculations to serve one client other
            clients have to wait - which is what makes it unsuitable for CPU intensive applications.
          </p>
...
sayHello('Mosh');
          ...
          <p>You may have questions throughout your course. Follow these steps to make the most of your course and
            become the best coder you can be.
          </p>
          ...
          <p>There are a bunch of other objects and functions that are globally available in Node. setTimeout() is used
            ...
            client, in the browser or in Node. We also have:
          </p>
          ...
          <p>In browsers we have the window object that represents the global scope. So all the variables and functions
            that are defined globally we can access via the window object:
          </p>
          ...
          <p>In Node instead of the window object we have an object called global so the functions discussed above
            through the global object. One thing to know about Node is that a variable declared like this:
          </p>
          ...
          <p>In the last section we learned that in client side Javascript, which runs in the browser, when we declare a
            variable or a function it is added to the global scope:
          </p>
...
window.sayHello();</code></pre>
          ...
          <p>In order to build reliable and maintainable applications we should avoid defining variables and functions
            ...
            define our variables and functions. They are then encapsulated in that module.
          </p>
          ...
          <p>Variables and functions defined within the module are private (in OOP terms). If we want to make a variable
            of function available outside of the module we need to explicitly export it.
          </p>
    ...
    [ 'C:\\DevelopmentTutorials\\TheCompleteNodeJSCourse\\02-node-module-system\\node_modules',
      ...
      'C:\\node_modules' ] }</code></pre>
          ...
          <p>Let's add a new module called logger.js for logging messages. The module will be re-used in this
            ...
            url where we can send an http request to log a message:
          </p>
          ...
          <p>We also create a function called log that takes a message and sends an http request. For now, for
            simplicity, we will just write the message to the console:
          </p>
  ...
  // Send an HTTP request
          ...
          <p>One of the properties of the module object is exports which by default is set to an empty object. Anything
            that we add to this object will be exported from the module:
          </p>
          ...
          <p>This is a node function it is not available in the browser. The js file extension is optional as Node
            assumes we will specify a Javascript file and therefore automatically adds the js extension.
          </p>
...
console.log(logger);</code></pre>
          ...
          <p>In more recent versions of Javascript we have the ability to use a const. So, as a best practice, when
            loading a module using the require function it is better to store the result in constant:
          </p>
...
__dirname returns a path to the directory that contains that module.</code></pre>
          ...
          <p>In the last section we mentioned that there are a few useful modules that are built into the core of Node.
            These modules allow you to work with files, the network, operating system etc. and include the following:
          </p>
          ...
          <p>In this section we will investigate the Path module. If you run <span
            class="filename">16-path-module.js</span> you will get the following information:
          </p>
...
Free Memory: 10441883648</pre>
          ...
          <p>Before node it was not possible to get information about the operating system using Javascript. Javascript
            only ran inside a browser and we could only work with the Window or Document objects.
          </p>
          ...
          <p>Almost all of the functions in the fs module come in two forms - synchronous (blocking) or asynchronous
            ...
            application you may have hundreds or thousands of clients connecting to that back-end.
          </p>
  ...
  console.log('Listener called');
          ...
          <p>The order in which you define the emitter and the listener is important. If you register the listener after
            ...
            iterates over all the registered listeners and calls them synchronously.
          </p>
          ...
          <p>Often when you raise an event you also want to send some information about that event. As an example, let's
            ...
            raising an event, we can add additional arguments, referred to as event arguments:
          </p>
  ...
  console.log('Listener called');
          ...
          <p>One of the powerful building blocks of Node is the Http module that we use for creating networking
            ...
            build with React or Angular or a mobile application running on a mobile device.
          </p>
          ...
          <ul>
            ...
            <li>To export a variable or function from a module, you need to add them to module.exports: <code
              class="language-">module.exports.sayHello = sayHello;</code></li>
            ...
            <li>To create a class with the ability to raise events, we should extend EventEmitter: <code
              class="language-">class Logger extends EventEmitter { }</code></li>
          </ul>
          <h1>Section 3 - Node Package Manager</h1>
          <h2>Introduction</h2>
          ...
          <h2>26 Installing a Node Package</h2>
          ...
          <p>In the underscore folder there will be another <span class="filename">package.json</span> file. Every node module has a package.json file just like our node application.
          </p>
...
<code class="language-javascript">var _ = require('underscore');</code></pre>
          ...
          <p>you should see that the <span class="filename">node_modules</span> folder is listed as one of the files or folders to be uploaded to
            git.
          </p>
          <p>If we add a file called <span class="filename">.gitignore</span> to the root folder with the following contents:</p>
          <figure>
              <pre>
...
<code class="language-json">"mongoose": "^4.13.6"</code></pre>
          ...
          <p>Consider the above mongoose dependency. If the developers of mongoose discover a bug in this version they
            will fix the bug and release a new version 4.13.7.
          </p>
          <p>The minor version is used for adding new features that don't break the existing API: 4.14.0. The patch
            version is 0 here because the developers have not yet found a bug.
          </p>
...
<code class="language-json">&quot;mongoose&quot;: &quot;^4.13.6&quot;</code></pre>
          ...
          <h2>31 Listing the Installed Packages</h2>
          ...
          <h2>32 Viewing Registry Info for a Package</h2>
...
npm view mongoose dependencies</pre>
          ...
          <h2>33 Installing a Specific Version of a Package</h2>
          ...
          <h2>34 Update Local Packages</h2>
          ...
          <p>You should see jshint has been added to a devDependencies section in <span
            class="filename">package.json</span>.</p>
          <h2>Uninstalling a Package</h2>
          <p>To uninstall a package:</p>
          <figure>
              <pre  class="terminal">npm un mongoose</code></pre>
          <figcaption>Fig 03-032</figcaption>
          </figure>
          <h2>37 Working with Global Packages</h2>
          ...
          <h2>39 Updating a Published Package</h2>
          <p>If we run:</p>
          <figure>
              <pre class="terminal">npm publish</pre>
          ...
          <p>Depending on the kind of change we have made the we need to update the version number. In this instance we
            will just update the minor version:
          </p>
          <figure>
          ...
          <p>Every Node application has a package.json file that includes metadata about the application. This includes
            the name of the application, its version, dependencies, etc.
          </p>
          <p>We use NPM to download and install 3rd-party packages from NPM registry. All the installed packages and
            their dependencies are stored under <span class="filename">node_modules</span> folders. This folder should be excluded from the source control.
          ...
          <p>Update packages</p>
          <figure>
              <pre class="terminal">npm update</code></pre>
          ...
          While this approach is fine, it is not suitable for building a large complex
          application because you will more than likely end up adding a lot of endpoints.
          So in this section we will look at express which is a fast and lightweight framework
          for building web applications.

          42 RESTful Services
          -------------------
          Most applications follow the Client/Server architecture
          Communication between the Client and Server occurs over HTTP
          We expose services on the Server that are available to the client via HTTP requests
          This process uses REST (REpresentational State Transfer) - REST is basically a principle
          for building these HTTP services
          We use simple HTTP protocol principles to provide support to Create, Read, Update and Delete
          data - referred to as CRUD

          We expose endpoints like this:
          ...
          GET     Get     /api/customers, /api/customers/1
          POST    Create  /api/customers/                     Include customer object in the body of the request
          PUT     Update  /api/customers/1                    Include customer object in the body of the request
          DELETE  Delete  /api/customers/1

          43 Introducing Express
          ----------------------
          As we define more routes for our application it will become necessary to use a framework to give our
          ...
          ---------------------------------
          We will add a new file called index.js (it could also be called app.js). First we load the express module:
          const express = require('express');
          const app = express();

          The app object exposes several useful methods:
          app.get();
          app.post();
          app.put();
          app.delete();

          All these methods correspond to the http verbs discussed earlier. So we
          ...
          Now in chrome browse to localhost:3000 and you should see the "Hello
          World" message.
          ...
          So in this implementation we don't have the if blocks we used earlier
          in the HTTP module code. Instead we define new routes by calling
          app.get. As our application grows we are free to move routes to other
          files (e.g. courses.js for routes related to courses).

          45 Nodemon
          ----------
          Up to this point, each time the code is changed we have been required
          to restart the node process in the terminal. We can use the nodemon
          node package to watch for changes and restart our process
          automatically:
          ...
          If you make a change to one of the routes and refresh the browser
          you should see the change takes effect immediately - there is no
          need to restart the process manually. nodemon also writes "[nodemon]
          restarting due to changes....." to the terminal

          46 Environment Variables
          ------------------------
          One thing we could improve on in the code is switching the hard
          coded port value of 3000:
          app.listen(3000, () => console.log('Listening on port 3000...'));

          For an environment variable. 3000 is an arbitrary number. While
          this should work on your development machine it is unlikely to work
          in a development environment. In a hosting environment the port will
          ...
          In this section we will demonstrate how to create a route that gets
          a single course. In the discussion earlier on RESTful services we
          said that in order to retrieve a single course we include the id of
          ...
          Now in chrome if you browse to localhost:3000/api/courses/1 you should
          see the course id of "1" displayed.

          It is possible to have multiple parameters in a route. Imagine a route
          ...
          If you browse to localhost:5000/api/posts/2018/1 you should see the
          following:
          ...
          This is the req.params object. With express we can retrieve querystring
          parameters as well as route parameters. These are parameters that appear
          in the Url after a question mark. For example we can get all the posts
          for January 2018 and sort by name using the following:

          http://localhost:5000/api/posts/2018/1?sortBy=name

          By convention, route parameters are used for mandatory values whereas
          query parameters are used for optional values. We can read a url
          parameter like so:
          ...
          -----------------------------
          Now let's implement a new endpoint to get a single course from the
          server. Define an array called courses:
          ...
          So now we have two endpoints - one to get all the courses and the
          other to get a single course. In the endpoint to get all the courses
          we will return our courses array:

          app.get('/api/courses', (req, res) => {
          res.send(courses);
          })

          In the second one we will write some logic to look for the course
          with the given id:

          const course = courses.find(c => c.id === parseInt(req.params.id));

          find is a method on every array in Javascript. As an argument we pass
          the method a function using the arrow function syntax. We write some
          logic that returns a boolean value that determines if this course is
          the one we are looking for or not. We store the result in a const
          called course.
          You might ask why I didn't use var here. var is perfectly fine but
          going forward best practice states to either use the let or const
          keywords. We use let to define a variable that we can reset later. We
          use const when defining a constant. This prevents the variable from
          being overwritten elsewhere in the code.

          If we don't find a course with the given id the  convention (with
          RESTful apis) is to return a status code of 404:

          if (!course) res.status(404).send('The course with the given id was not found');

          In the code above we also send the optional message. If we do find
          a course with that id then we will just return it to the client:
          res.send(course);
          ...
          then just that course is returned. If we browse to a course that doesn't
          exist:

          localhost:5000/api/courses/10

          We will receive the message saying the course was not found. If you open
          chrome developer tools and click on the Network tab refresh the page
          and you will see the status 404.

          49 Handling HTTP POST Requests
          ------------------------------
          So far the routes we have created all response to HTTP GET requests. In
          this lecture we will learn how to respond to HTTP POST requests. We use
          an HTTP POST request to create a new course:

          app.post('/api/courses', (req, res) => {

          });

          Similar to the get method we need to specify a path: /api/courses because
          we are going to POST to the collection of courses. That's why we use the
          plural name. Next we add our route handler using the arrow function.

          In this route handler we need to read the course object which should be in
          the body of the request and use it's properties to create a new course
          object and then add that object to our courses array:
          ...
          Again, we use a const here because we are not going to reset this course
          object later. In this demo, to keep things simple, we are not working with
          a database so we just add one to the length of our courses array.
          Next we need to read the name property from the body of the request. We
          assume that in the request body we have an object and that object has a
          name property. In order for this line to work we need to enable parsing of
          JSON objects in the body of the request because by default this feature
          is not enabled in express. So at the top of the file after we get the
          app object:

          app.use(express.json());

          This may look a bit strange at this point in time but we will cover this
          in detail later in the course. Basically, here, we are adding a piece of
          middleware. When we call express.json() this method returns a piece of
          middleware and then we call app.use to use that middleware in the request
          processing pipeline.
          ...
          Finally, by convention, when we post an object to the server and the server
          creates a new object or resource we should return that object in the body
          of the response:

          res.send(course);

          We do this because we are assigning this id on the server and we need to
          return this course object to the client because chances are the client needs
          to know the id of this new object.

          50 - Calling Endpoints Using Postman
          ------------------------------------
          ...
          51 - Input Validation
          ---------------------
          In this lecture we will look at input validation. Best practice dictates that
          we should never trust information sent by the client. It should always be
          validated.
          ...
          We are dealing with a simple object with only one property: name we can write
          some validation logic like this:
          ...
          So if the name property doesn't exist or is less than 3 characters in length we
          set the response status to 400 Bad Request. We then send a rather generic error
          message along with setting the status.  Finally we return to prevent the rest of
          the function from being executed.

          In a real world application we are likely to be working with more complex objects
          than the one we are using here. We don't want to add too many validation rules in
          if statements at the top of the route handler. We will look at a node package that
          makes it easier to validate input: joi.
          ...
          We use a capital for the Joi variable because this module returns a class. As we
          said before we use Pascal naming conventions when we name our classes - so the
          first letter of every word should be uppercase.

          Also as a best practice put all your require calls at the top of the file. This
          way you can easily see what are the dependencies of this module. So our current
          module is dependent on joi and express.

          Go back the route handler. With joi we need to define a schema. A schema defines
          the shape of our objects - what properties do we have in our object, what is the
          type of each property in the object, do we have an email, do we have a string,
          what are the minimum or maximum number of characters, do we have a number, what
          ...
          So we set schema to an object with a name property. We tell joi to expect a string
          with a minimum of three characters that is required.
          ...
          We store the object returned by the validate method in a const called result. For
          this demo I'm going to log this result on the console.

          Now if we use Postman to create another course object (POST to http://localhost:3000/api/courses
          with the course specified in the body and the object sent to JSON(application/json))
          in the terminal we should see a result object with two properties - error and value.
          Only one of these can have a value. In this case because we sent a valid course
          object we have this course object here as the value of the value property and you can
          see error is null:
          ...
          If we send an invalid object, for example by removing the name property, then value will
          be null and error will  be set:

          { ValidationError: child "name" fails because ["name" is required]

          Back in the route handler instead of using the manual validation logic we can check
          the value of the result.error property:
          ...
          If we use Postman to submit another invalid request and look at the response we get
          an object with properties such as isJoi, name, details etc. details is an array of
          error messages. The first one should be "name is required". This object is too
          complex to send to the client. If you want to simplify this go back to the code
          and instead of returning the whole result.error object you can drill down to the
          detail arrays first elements message property:

          result.error.details[0].message

          52 - Handling HTTP PUT Requests
          -------------------------------
          Now let's see how we can update a course. Add a new route handler with the PUT
          method:
          ...
          The route handler contains a lot of logic, a good proportion of which we have already
          covered and can pull in from different methods.
          The code to look up the course and return a 404 if it doesn't exist can be found in the
          route handler to get a single course
          We can get the validation from the post endpoint. The problem with this approach is that
          , although in this example we are dealing with a simple schema, real world applications
          may have much more complex schemas. This would mean our validation code was duplicated in
          two route handlers. We'll resolve this shortly.
          Next we update the course.name property. If we had more properties we would update them
          here also.
          Finally we need to return the updated course to the client.

          Now we will extract the validation logic into it's own function which can be used in
          both route handlers:
          ...
          In the code above, instead of validating the req.body we validate the course object. Also
          we return the result to the caller - there is no need to define a const.

          New we can reuse this in our PUT endpoint:

          const result = validateCourse(req.body);

          This code can be made a bit cleaner by using the object destructuring features of modern
          Javascript. Currently we get the result object and then we are accessing result.error
          in two different places. Since all we are interested in is the error property we can get
          this using object destructuring. With object destructuring when declaring a variable or
          ...
          With the above code we don't need to repeat result.error in two seperate places we can
          simply use error.

          We now need to add this new way of validating a course to the HTTP POST request.

          Back in POSTMAN test the PUT and POST endpoints with valid and invalid values in the
          data.

          Handling HTTP Delete Requests
          -----------------------------
          Out of all the CRUD operations we have now implemented Read, Update and Delete. In this
          lecture we will look at how to respond to HTTP DELETE requests:
          ...
          There are currently three bugs in this code. Currently in the PUT endpoint if we don't
          have a course with a given id we return a 404 to the client. However at this point we
          should exit the route handler otherwise the rest of the code will be executed:

          if (!course) return res.status(404).send('The course with the given id was not found');

          To make this code cleaner we can use the same technique to deal with an invalid
          request:
          ...
          we will build the backend of vidly bit by bit. Your first task is to create a service
          for managing the list of genres. Each movie has a genre like action, horror etc. We
          should have an endpoint for getting all the genres. We should also have endpoints for
