        ...
        <div class="panel-group">
            <div class="panel panel-primary">
                ...
                <div class="panel-body">
                    <h3>Introduction</h3>
                    <p>Hi, this is Brian Noyes. In this last module, we're going to look into the validation mechanisms of
                        data bindings and how you can use it to provide validation indications to a user when they put in
                        bad data.
                    </p>
                    <p>First we're going to talk about the simplest mechanism to use, which is to throw an exception when
                        you get bad data and to turn on a flag on the binding to have it give validation indications based
                        on that.
                    </p>
                    <p>After that, we'll look at a built-in mechanism in WPF called ValidationRules. ValidationRules are
                        objects that you implement that encapsulate a rule and get evaluated by a binding when you hook it
                        up to it.
                    </p>
                    <p>If that rule returns an error, then you'll get an error indication on the screen. Next, we'll look at
                    ...
                    <p>Next, we'll look at INotifyDataErrorInfo, which was added in .NET 4.5 and supports asynchronous
                    ...
                    <p>Next, we'll talk briefly about some binding events that are available that can be handy in dealing
                    ...
                    <p>And then we'll finish off this module by talking about DataAnnotations, which are a standard
                ...
                <div class="panel-body">
                    ...
                    <p>One of the simplest ways to do validation with bindings is to use exceptions. Normally when you hook
                    ...
                    <p>All you do to do this is set a property on the binding that says ValidatesOnExceptions=True. Once you
                    ...
                    <p>One of the other advantages of this mechanism, besides its simplicity, is that it allows you to
                ...
                <div class="panel-body">
                    ...
                    <div class="example">
                        <div class="input-group">
                	        ...
                	        <button class="btn" data-clipboard-target="#Ex09-001">
                    ...
                    <div class="example">
                        <div class="input-group">
                            ...
                            <span class="input-group-btn">
                	        <button class="btn" data-clipboard-target="#Ex09-002">
                        	    <img src="/./src/assets/clippy.svg" width="13" alt="Copy to clipboard">
                            </button>
        	            </span>
                        </div>
                    </div>
                    <p>In this demo, we're going to get started with validation by showing you the easiest form of
                    ...
                    <p>It's got an empty MainWindow at this point. The only thing in the MainWindow is a
                        CollectionViewSource:</p>
                    <figure>
...
&lt;/Window.Resources&gt;</code></pre>
                    ...
                    <p>that's going to be working with a Customer Type. In the code-behind of the MainWindow, we've got the
        ...
        set
        {
            if (_HomePhone != value)
            {
                ...
                PropertyChanged(this, new PropertyChangedEventArgs(&quot;HomePhone&quot;));
                    ...
                    <p>It's got three properties at this point, it's got a HomePhone, a WorkPhone, and a CellPhone, and you
                        can see it's just implementing the INotifyPropertyChanged on that Entity Type.
                    </p>
                    <p>So that's our starting point. Let's go ahead and add a couple of fields to our MainWindow to start
                    ...
                    <p>Now if you remember from the Data Sources Window demos in a previous module, when we do that
...
&lt;/Window.Resources&gt;</code></pre>
                    ...
                    <p>it didn't generate a new one, it just set the DataContext on the two Grids that it generated, you can
...
HorizontalAlignment=&quot;Left&quot;</code></pre>
                    ...
                    <p>It sets the DataContext on the grid to the customerViewSource, and then on the data input field it
                        added the binding to the appropriate field.
                    </p>
                    <figure>
                <pre class="prettyprint"><code>&lt;TextBox x:Name=&quot;homePhoneTextBox&quot;
    Grid.Column=&quot;1&quot;
    ...
    Width=&quot;120&quot; /&gt;</code></pre>
                    ...
                    <p>Now as we talked about before, when you do these drag-and-drops it adds some of these other
                        properties relative to validation. You can see, by default, it's going to turn on
                        ValidatesOnExceptions=true.
                    </p>
                    <figure>
                        <pre class="prettyprint"><code>Binding HomePhone, Mode=TwoWay, NotifyOnValidationError=true,  ValidatesOnExceptions=true</code></pre>
                        <figcaption>Fig 07-198</figcaption>
                    </figure>
                    <p>The NotifyOnValidationError we'll get to later in the module. I'll take that off for now, as well as
...
/&gt;</code></pre>
                    ...
                    <p>So let's drop back to the customer here and add a little Helper method to validate a phone
                        number.</p>
                    <figure>
                <pre class="prettyprint"><code>private void ValidatePhone(string value)
{
    ...
    if (regex.Match(value) == Match.Empty)
        throw new ArgumentException(&quot;Invalid phone format&quot;);
                    ...
                    <p>This method's going to take in a value and it's going to use a RegularExpression, which you don't
                    ...
                    <p>And if it doesn't match, it's going to throw an exception, an argument exception in this case, with
                    ...
                    <figure>
                <pre class="prettyprint"><code>private string _HomePhone;
public string HomePhone
{
    get { return _HomePhone; }
    set
    {
        if (_HomePhone != value)
        {
            ...
            PropertyChanged(this, new PropertyChangedEventArgs(&quot;HomePhone&quot;));
                    ...
                    <p>So one of the advantages of using exceptions for validation is that you can prevent the value from
                        ever getting stored in your Entity object if it's not a valid value.
                    </p>
                    <p>So if we run this right now, and I've got the output window up at the bottom, note that as I put in
                    ...
                    <p>That's because, by default, a binding will swallow any exceptions raised when it calls the set and
                        get block, and it prevents your application from blowing up if it can't evaluate the property that
                        it's pointed to.
                    </p>
                    <p>But in this case, we want that exception to indicate that there's a validation error. So I'm going to
                        go back to my binding and put back on ValidatesOnExceptions=True:
                    </p>
                    <figure>
                <pre class="prettyprint"><code>Grid.Column=&quot;1&quot;
    HorizontalAlignment=&quot;Left&quot;
    Height=&quot;23&quot;
    Margin=&quot;3&quot;
    Grid.Row=&quot;0&quot;
...
/&gt;</code></pre>
                    ...
                    <p>Now if we run the same scenario, we go ahead and put in an invalid value and tab out of the field,
                    ...
                    <p>So WPF controls, by default, have a built-in error template that simply puts a red box around the
                    ...
                    <p>We'll get to how you can display those things later in this module, but for now you can see that it
                    ...
                    <p>So validating based on exceptions requires nothing more than in the set block of the property that's
                ...
                <div class="panel-body">
                    ...
                    <p>Another option you have in WPF for defining ValidationRules, getting them evaluated and hooked up to
                    ...
                    <p>Once you have the ValidationRule defined, you go and you hook it up to a ValidationRules collection
                    ...
                    <p>The good thing about ValidationRules is they can be used to encapsulate reusable rules, which you can
                    ...
                    <p>For general purpose rules they're okay, but the rule doesn't get passed enough context about where
                        it's hooked up to really be able to modify its logic based on the context of where it's being used.
                    </p>
                    <p>So let's take a look at a demo of how you define ValidationRules and get them hooked up to your
                        bindings.</p>
                </div>
                <div class="panel-body">
                    ...
                    <div class="example">
                        <div class="input-group">
                            <input id="Ex09-003" type="text" class="form-control"
                                   value="C:\Development Tutorials\Pluralsight-Courses\WPF Data Binding in Depth\Module 9\before\ValidationDemo\ValidationDemo.sln">
                            <span class="input-group-btn">
                	        <button class="btn" data-clipboard-target="#Ex09-003">
                        	    <img src="/./src/assets/clippy.svg" width="13" alt="Copy to clipboard">
                            </button>
        	            </span>
                        </div>
                    ...
                    <div class="example">
                        <div class="input-group">
                            <input id="Ex09-004" type="text" class="form-control"
                                   value="C:\Development Tutorials\Pluralsight-Courses\WPF Data Binding in Depth\Module 9\after\ValidationDemo\ValidationDemo.sln">
                            <span class="input-group-btn">
                	        <button class="btn" data-clipboard-target="#Ex09-004">
                        	    <img src="/./src/assets/clippy.svg" width="13" alt="Copy to clipboard">
                            </button>
        	            </span>
                        </div>
                    ...
                    <p>In this demo, I'm going to get you familiar with ValidationRules in WPF, which are objects you can
                    ...
                    <p>We have our work and home phone inputs. The Home Phone is still hooked up with an exception
                    ...
                    <figure>
                <pre class="prettyprint"><code>public class RegexValidationRule : ValidationRule
{
    public override ValidationResult Validate(object value, CultureInfo cultureInfo)
    {
        ...
        if (match == null || match == Match.Empty)
            ...
            return ValidationResult.ValidResult;
                    ...
                    <p>The Validate method is going to be passed in a value, the value that's being set on some binding that
                    ...
                    <p>In this case, we're writing a reusable ValidationRule based on RegularExpressions. Now as a starting
                    ...
                    <p>But you can see I'm just setting up a Regex, evaluating it with a Match call, and if it doesn't match
                    ...
                    <figure>
                <pre class="prettyprint"><code>&lt;TextBox x:Name=&quot;workPhoneTextBox&quot;
        Grid.Column=&quot;1&quot;
        HorizontalAlignment=&quot;Left&quot;
        Height=&quot;23&quot;
        Margin=&quot;3&quot;
    ...
    &lt;/TextBox.Text&gt;
                    ...
                    <p>So I can take out the Text property here and add it in like this, where I'm setting the text property
                    ...
                    <p>So that's just the equivalent syntax there for setting up the binding. Now the reason for this is
                    ...
                    <figure>
                <pre class="prettyprint"><code>&lt;TextBox.Text&gt;
    &lt;Binding Path=&quot;WorkPhone&quot;&gt;
...
&lt;/TextBox.Text&gt;</code></pre>
                    ...
                    <p>So with that in place, we could go ahead and run, and we can put the WorkPhone into an invalid state,
...
&lt;/TextBox.Text&gt;</code></pre>
                    ...
                    <p>If we set it to PropertyChanged instead of the default of LostFocus, then we get more immediate
                    ...
                    <p>In general, one of the problems with ValidationRules is the fact that there's very little context
                        available inside of the Rule itself about what you're evaluating and where it's coming from. All you
                        get is a value here.
                    </p>
                    <figure>
                        <pre class="prettyprint"><code>public override ValidationResult Validate(object value, CultureInfo cultureInfo)</code></pre>
                        <figcaption>Fig 07-214</figcaption>
                    </figure>
                    <p>You can think of this as, if a stranger walks up to you on the street, says 42, are you okay with
                    ...
                    <p>I'm going to expand my implementation here and add down at the bottom here an Expression
                        property.</p>
                    <figure>
                <pre class="prettyprint"><code>public class RegexValidationRule : ValidationRule
{
    public override ValidationResult Validate(object value, CultureInfo cultureInfo)
    {
        ...
        if (match == null || match == Match.Empty)
            ...
            return ValidationResult.ValidResult;
                    ...
                    <p>So we got rid of the hard-coded RegularExpression, and now we use the Expression property to
...
&lt;/TextBox.Text&gt;</code></pre>
                    ...
                    <p>So now we set the Expression based on the fact that here we have some context that we're trying to
                        evaluate a phone, and so we come up with a RegularExpression that's appropriate for that phone.
                    </p>
                    <p>Obviously, if you're going to put any complex, domain-specific logic into one of these
                    ...
                    <p>Whatever properties you choose to expose on your ValidationRules, they will have to be set with a
                        value that's static in the XAML.
                    </p>
                    <figure>
                <pre class="prettyprint"><code>&lt;Binding.ValidationRules&gt;
    &lt;local:RegexValidationRule Expression=&quot;^\D?(\d{3})\D?\D?(\d{3})\D?(\d{4})$&quot; /&gt;
&lt;/Binding.ValidationRules&gt;
                </code></pre>
                        <figcaption>Fig 07-217</figcaption>
                    </figure>
                    <p>Because the base class of ValidationRules is not a DependencyObject, you can't have
                ...
                <div class="panel-body">
                    ...
                    <p>Basically, IDataErrorInfo is an interface that defines an API, so that the binding can query for
                    ...
                    <p>The main one that's focused on is the Indexer. So this is a C# Indexer, or an ItemProperty in Visual
                    ...
                    <p>So, as I said, it's been around for a long time, it was actually implemented in DataSet row objects
                        in .NET 1.0 and carrying forward, and it's very flexible and easy to use.
                    </p>
                    <p>One potential downside to it, depending on your perspective, is that you do have to allow the
                        property to first be set.
                    </p>
                    <p>So the order of operations here is that the binding is going to go set a property because some input
                    ...
                    <p>So there's a property on the binding that ValidatesOnDataErrors that you set to true, and that causes
                    ...
                    <p>So let's take a look at a demo of how we implement this on an object and get it hooked up and using
                        it in a binding.
                    </p>
                </div>
                <div class="panel-body">
                    ...
                    <div class="example">
                        <div class="input-group">
                            <input id="Ex09-005" type="text" class="form-control"
                                   value="C:\Development Tutorials\Pluralsight-Courses\WPF Data Binding in Depth\Module 9\before\ValidationDemo\ValidationDemo.sln">
                            <span class="input-group-btn">
                	        <button class="btn" data-clipboard-target="#Ex09-005">
                        	    <img src="/./src/assets/clippy.svg" width="13" alt="Copy to clipboard">
                            </button>
        	            </span>
                        </div>
                    ...
                    <div class="example">
                        <div class="input-group">
                            <input id="Ex09-006" type="text" class="form-control"
                                   value="C:\Development Tutorials\Pluralsight-Courses\WPF Data Binding in Depth\Module 9\after\ValidationDemo\ValidationDemo.sln">
                            <span class="input-group-btn">
                	        <button class="btn" data-clipboard-target="#Ex09-006">
                        	    <img src="/./src/assets/clippy.svg" width="13" alt="Copy to clipboard">
                            </button>
        	            </span>
                        </div>
                    ...
                    <p>In this demo, I'm going to show you how to implement the IDataErrorInfo interface and use that to
                    ...
                    <p>So we're going to first go and drag the CellPhone property out to generate a new field that we can do
                        data binding with:
                    </p>
                    <figure>
                    ...
                    <p>I'm going to drop into the XAML for that and modify the binding that was generated to get rid of
                        these extra properties, and just put one additional property, which is ValidatesOnDataErrors=true.
                    </p>
                    <figure>
                <pre class="prettyprint"><code>&lt;TextBox x:Name=&quot;cellPhoneTextBox&quot;
    Grid.Column=&quot;1&quot;
    HorizontalAlignment=&quot;Left&quot;
    Height=&quot;23&quot;
    Margin=&quot;3&quot;
    ...
    Width=&quot;120&quot; /&gt;</code></pre>
                    ...
                    <p>We saw ValidatesOnExceptions earlier, this is an alternative to that, using IDataErrorInfo to derive
                    ...
                    <p>So validatesOnDataErrors won't have a chance to take effect, but validatesOnExceptions would show
                    ...
                    <p>So I'll drop into my Customer and add the IDataErrorInfo interface and implement it using Visual
                        Studio.</p>
                    <figure>
                <pre class="prettyprint"><code>namespace ValidationDemo
{
    public class Customer : INotifyPropertyChanged, IDataErrorInfo</code></pre>
                        <figcaption>Fig 07-221</figcaption>
                    </figure>
                    <p>Down at the bottom of my class it added two members</p>
                    <figure>
                <pre class="prettyprint"><code>public string Error
{
...
public string this[string columnName]
{
    get { throw new NotImplementedException(); }
                    ...
                    <p>A string called Error, and an Indexer that says columnName here, but you can think of this, really,
                    ...
                    <figure>
                <pre class="prettyprint"><code>public string this[string propertyName]
{
    get { throw new NotImplementedException(); }
}</code></pre>
                        <figcaption>Fig 07-223</figcaption>
                    </figure>
                    <p>So the idea here is that the Error property is meant to represent an error for the object as a whole.
                ...
                <pre class="prettyprint"><code>public string Error
{
    get { return string.Empty; }
                    ...
                    <p>The Indexer is what we're going to focus our effort on, and the idea is that this will be invoked by
                    ...
                    <figure>
                <pre class="prettyprint"><code>private string _CellPhone;
    ...
    set
    {
        if (_CellPhone != value)
        {
            ...
            PropertyChanged(this, new PropertyChangedEventArgs("CellPhone"));
                    ...
                    <p>so you're going to have to let the set block be called, not throw an exception, write it into your
                        member variable, so that down here in the Indexer:
                    </p>
                    <figure>
                <pre><code class="csharp[">public string this[string propertyName]
{
    get { throw new NotImplementedException(); }
}</code></pre>
                        <figcaption>Fig 07-226</figcaption>
                    </figure>
                    <p>you can then look at that value in the member variable and decide whether you like it or not, and if
                ...
                <pre class="prettyprint"><code>public string this[string propertyName]
{
    get { return GetErrorForProperty(propertyName); }
                    ...
                    <p>because I don't like putting a bunch of logic in a get block of a property. Then I'll go right below
                        that and implement that method:
                    </p>
                    <figure>
                <pre class="prettyprint"><code>private string GetErrorForProperty(string propertyName)
{
    switch (propertyName)
    {
        case &quot;CellPhone&quot;:
        ...
        default:
            return string.Empty;
                    ...
                    <p>And the nature of the method that you're going to have to implement is, unfortunately, generally a
                        big switch case statement with a case for each property that you have ValidationRules you want to
                        implement.
                    </p>
                    <p>That's one of the downsides of IDataErrorInfo. But because you're inside of the Entity itself, it
                    ...
                    <p>In our case, we're just going to inspect for the CellPhone property when it's set. We're going to use
                    ...
                    <figure>
                        <pre class="prettyprint"><code>public class Customer : INotifyPropertyChanged, IDataErrorInfo</code></pre>
                        <figcaption>Fig 07-229</figcaption>
                    </figure>
                    <p>and our binding says that we do want to honor that interface, this binding will basically hook up to
                        that interface and query for errors every time it goes to set the property that it's bound to.
                    </p>
                    <p>So with that, we can go ahead and run, input an invalid value, tab out, and we can see the error
                        indication there.
                    </p>
                    <figure>
                    ...
                    <p>Mostly focused on the Indexer, go decide what property is being set, invoke whatever rules are
                ...
                <div class="panel-body">
                    ...
                    <p>Now a new interface that was added in .NET 4.5 that's similar to IDataErrorInfo is
                    ...
                    <p>So to use this, you set a ValidateOnNotifyDataErrors property on the binding, the binding will call
                    ...
                    <p>To implement this, you're going to manage a dictionary of errors under the covers, so you basically
                    ...
                    <p>So one of the advantages here of INotifyDataErrorInfo over IDataErrorInfo is that you can return
                ...
                <div class="panel-body">
                    ...
                    <div class="example">
                        <div class="input-group">
                            <input id="Ex09-007" type="text" class="form-control"
                                   value="C:\Development Tutorials\Pluralsight-Courses\WPF Data Binding in Depth\Module 9\before\ValidationDemo\ValidationDemo.sln">
                            <span class="input-group-btn">
                	        <button class="btn" data-clipboard-target="#Ex09-007">
                        	    <img src="/./src/assets/clippy.svg" width="13" alt="Copy to clipboard">
                            </button>
        	            </span>
                        </div>
                    ...
                    <div class="example">
                        <div class="input-group">
                            <input id="Ex09-008" type="text" class="form-control"
                                   value="C:\Development Tutorials\Pluralsight-Courses\WPF Data Binding in Depth\Module 9\after\ValidationDemo\ValidationDemo.sln">
                            <span class="input-group-btn">
                	        <button class="btn" data-clipboard-target="#Ex09-008">
                        	    <img src="/./src/assets/clippy.svg" width="13" alt="Copy to clipboard">
                            </button>
        	            </span>
                        </div>
                    ...
                    <p>In this demo, I'm going to get you familiar with INotifyDataErrorInfo, which is an asynchronous
                ...
                <pre class="prettyprint"><code>namespace ValidationDemo
{
    ...
    {</code></pre>
                    ...
                    <p>So I changed my interface to INotifyDataErrorInfo, I will use Visual Studio to stub it out, and then
                    ...
                    <figure>
                <pre class="prettyprint"><code>public event EventHandler&lt;DataErrorsChangedEventArgs&gt; ErrorsChanged;
...
public bool HasErrors
{
    throw new NotImplementedException();
                    ...
                    <p>First off, it's got an ErrorsChanged event and I'm going to use the delegate trick here to make it so
                    ...
                    <figure>
                <pre class="prettyprint"><code>public bool HasErrors
{
    throw new NotImplementedException();
}</code></pre>
                    ...
                    <figure>
                <pre class="prettyprint"><code>public System.Collections.IEnumerable GetErrors(string propertyName)
{
    throw new NotImplementedException();
}</code></pre>
                        <figcaption>Fig 07-235</figcaption>
                    </figure>
                    <p>If at some point after it's called GetErrors, the ErrorsChanged event fires, it will come back in and
                    ...
                    <p>So, basically, to start fleshing this out, what you need is somewhere to store some errors, and the
                        shape of that is generally going to be a Dictionary&lt;string, List&lt;string&gt;&gt;:
                    </p>
                    <figure>
                        <pre class="prettyprint"><code>Dictionary&lt;string, List&lt;string&gt;&gt; _PropertyErrors = new Dictionary&lt;string, List&lt;string&gt;&gt;();</code></pre>
                        <figcaption>Fig 07-236</figcaption>
                    </figure>
                    <p>So you want to have that collection of errors that you return up here for a given property name where
                ...
                <pre class="prettyprint"><code>public System.Collections.IEnumerable GetErrors(string propertyName)
{
    ...
    return null;
                    ...
                    <p>see if there is a key for the propertyName, and if so, return whatever the value is the list of
                        strings associated with it.
                    </p>
                    <p>Now I put a lock statement here because we're going to go and do some asynchronous work using a
                        separate thread, and that separate thread needs to be able to push errors into this PropertyErrors
                        collection.
                    </p>
                    <p>So if you're going to do that in a multi-threaded way, you want to make sure you protect that
                    ...
                    <p>Next, we need to implement this HasErrors method, so I'm going to flesh that out by calling a
                        method:</p>
                    <figure>
                <pre class="prettyprint"><code>public bool HasErrors
{
    ...
    foreach (var key in _PropertyErrors.Keys)
    {
        if (_PropertyErrors[key] != null)
        {
            ...
            break;
                    ...
                    <p>again, because I don't like putting complex logic in a get block, personally. And what this method
                    ...
                    <p>Now the whole point of this interface is to support asynchronous scenarios, places where you're going
                    ...
                    <figure>
                <pre class="prettyprint"><code>Task&lt;List&lt;string&gt;&gt; GetErrorsForPhone(string value)
{
    ...
    return Task.Factory.StartNew&lt;List&lt;string&gt;&gt;(() =&gt;
    {
        ...
        return null;
                    ...
                    <p>So this method is using the Task Parallel Library, it's defining a method called GetErrorsForPhone
                    ...
                    <figure>
                        <pre class="prettyprint"><code>return Task.Factory.StartNew&lt;List&lt;string&gt;&gt;(() =&gt;</code></pre>
                        <figcaption>Fig 07-240</figcaption>
                    </figure>
                    <p>on a separate thread.</p>
                    <p>The first thing I'm going to do is go sleep for 5 seconds, so that you can see when I run this that's
                ...
                <pre class="prettyprint"><code>Task&lt;List&lt;string&gt;&gt; GetErrorsForPhone(string value)
{
    ...
    return Task.Factory.StartNew&lt;List&lt;string&gt;&gt;(() =&gt;
    {
        ...
        return null;
                    ...
                    </figure>                <p>to be invoked at a point where I know that a phone value has changed. So, naturally, a good place for
        ...
        if (_CellPhone != value)
        {
            ...
            GetErrorsForPhone(CellPhone).ContinueWith((errorsTask) =&gt;
            {
                lock (_PropertyErrors)
                {
                    ...
                    <p>So I'm going to call that task-based method, which is non-blocking because it's asynchronous, and
                    ...
                    <p>But because this is all being done async, this is going to happen on a separate thread, so this is
                    ...
                    <figure>
                        <pre class="prettyprint"><code>_PropertyErrors[&quot;CellPhone&quot;] = errorsTask.Result;</code></pre>
                        <figcaption>Fig 07-243</figcaption>
                    </figure>
                    <p>of the task that come back in from our ContinueWith:</p>
                    <figure>
                        <pre class="prettyprint"><code>GetErrorsForPhone(CellPhone).ContinueWith((errorsTask) =&gt;</code></pre>
                    ...
                    <figure>
                        <pre class="prettyprint"><code>ErrorsChanged(this, new DataErrorsChangedEventArgs(&quot;CellPhone&quot;));</code></pre>
                        <figcaption>Fig 07-246</figcaption>
                    </figure>
                    <p>associated with the INotifyDataErrorInfo and tell it what property has changed. That's going to
                    ...
                    <p>We can go into the Cell Phone entry here, put in an invalid value, tab out of the field, count to
                        about 5, and we should see a red box up here.
                    </p>
                    <p>Boom, so that was that 5 second delay with the thread sleep, went out, evaluated the rule, returned
                    ...
                    <p>Now you can see if you implement it this way, GetErrors can certainly still execute synchronously.
                        All it's doing here:
                    </p>
                    <figure>
                <pre class="prettyprint"><code>public System.Collections.IEnumerable GetErrors(string propertyName)
        ...
        if (_CellPhone != value)
        {
            ...
            GetErrorsForPhone(CellPhone).ContinueWith((errorsTask) =&gt;
            {
                lock (_PropertyErrors)
                {
                    ...
                    <p>of the property and populate the PropertyErrors then, those errors will show up automatically as soon
                ...
                <pre class="prettyprint"><code> public System.Collections.IEnumerable GetErrors(string propertyName)
{
    ...
    return null;
                    ...
                    <p>Which happens after the PropertyChanged fires from the set block. Now notice that our demo worked
    ...
    Width=&quot;120&quot;</code></pre>
                ...
                <div class="panel-body">
                    ...
                    <p>Bindings offer a number of events that you can hook into for various scenarios, and one of those is
                    ...
                    <p>The one that's specific to validation is Validation.ValidationError. This is an attached event that
                        will fire whenever a validation error is added or removed from a control that has a binding hooked
                        up.
                    </p>
                    <p>Now none of these events fire by default, you're going to have to enable them on a binding, and you
                        do that with a series of properties on the Binding that are NotifyOn, and the appropriate event, and
                        you set that to true.
                    </p>
                    <p>So you first enable it with one of these flags, and then you hook up the events above to an event
                ...
                <div class="panel-body">
                    ...
                    <div class="example">
                        <div class="input-group">
                            <input id="Ex09-009" type="text" class="form-control"
                                   value="C:\Development Tutorials\Pluralsight-Courses\WPF Data Binding in Depth\Module 9\before\ValidationDemo\ValidationDemo.sln">
                            <span class="input-group-btn">
                	        <button class="btn" data-clipboard-target="#Ex09-009">
                        	    <img src="/./src/assets/clippy.svg" width="13" alt="Copy to clipboard">
                            </button>
        	            </span>
                        </div>
                    ...
                    <div class="example">
                        <div class="input-group">
                            <input id="Ex09-010" type="text" class="form-control"
                                   value="C:\Development Tutorials\Pluralsight-Courses\WPF Data Binding in Depth\Module 9\after\ValidationDemo\ValidationDemo.sln">
                            <span class="input-group-btn">
                	        <button class="btn" data-clipboard-target="#Ex09-010">
                        	    <img src="/./src/assets/clippy.svg" width="13" alt="Copy to clipboard">
                            </button>
        	            </span>
                        </div>
                    ...
                    <p>Another thing that comes in handy sometimes when working with validation is being able to hook events
    ...
    Width=&quot;120&quot;</code></pre>
                    ...
                    <p>These ValidationError events are attached events, so you can attach them to any element. So I could
                ...
                <pre class="prettyprint"><code>&lt;TextBox x:Name=&quot;cellPhoneTextBox&quot;
                    ...
                    <p>Because these are bubbling events, I could also put them further up the chain. So instead of on the
                    ...
                    <figure>
                <pre class="prettyprint"><code>private void cellPhoneTextBox_Error(object sender, ValidationErrorEventArgs e)
{
    Debug.WriteLine(e.Error.ErrorContent);
                    ...
                    <p>This thing has several properties on it. It's got a ValidationError is the main thing you're going to
                        look at and that error has a bunch of context on it.
                    </p>
                    <figure>
                <pre class="prettyprint"><code>public ValidationRule RuleInError { get; set; }
public object ErrorContent { get; set; }
public Exception Exception { get; set; }
public object BindingInError { get; }</code></pre>
                        <figcaption>Fig 07-256</figcaption>
                    </figure>
                    <p>The ErrorContent property is the main thing you're going to look at, it's generally going to be the
                    ...
                    <p>Now if we go ahead and run, watch the output window on the bottom, I'll invoke my asynchronous
                ...
                <div class="panel-body">
                    ...
                    <p>As you've seen so far, WPF controls have a default error template or rendering that it shows when
                    ...
                    <p>So the Validation Class has a number of attached properties that will allow you to drive the
                    ...
                    <p>Then there's the Errors collection, and this will get you all of the errors associated with the
                        control, generally based on a bound property, and validation errors that flowed through the binding.
                    </p>
                    <p>Finally, there's the ErrorTemplate. The ErrorTemplate is the custom ControlTemplate that is used when
                    ...
                    <p>So the basic process is to provide a custom display for the control, you're going to set the
                    ...
                    <p>You might also want to do a ToolTip to show the validation errors, and the standard way to do that is
                        to define a Style with a trigger on that Style that monitors the HasError property.
                    </p>
                    <p>When the HasError goes true, it sets a ToolTip on the control it's associated with and uses the
                ...
                <div class="panel-body">
                    ...
                    <div class="example">
                        <div class="input-group">
                            <input id="Ex09-011" type="text" class="form-control"
                                   value="C:\Development Tutorials\Pluralsight-Courses\WPF Data Binding in Depth\Module 9\before\ValidationDemo\ValidationDemo.sln">
                            <span class="input-group-btn">
                	        <button class="btn" data-clipboard-target="#Ex09-011">
                        	    <img src="/./src/assets/clippy.svg" width="13" alt="Copy to clipboard">
                            </button>
        	            </span>
                        </div>
                    ...
                    <div class="example">
                        <div class="input-group">
                            <input id="Ex09-012" type="text" class="form-control"
                                   value="C:\Development Tutorials\Pluralsight-Courses\WPF Data Binding in Depth\Module 9\after\ValidationDemo\ValidationDemo.sln">
                            <span class="input-group-btn">
                	        <button class="btn" data-clipboard-target="#Ex09-012">
                        	    <img src="/./src/assets/clippy.svg" width="13" alt="Copy to clipboard">
                            </button>
        	            </span>
                        </div>
                    ...
                    <p>So let's take a look at how we can provide a custom template to either customize the appearance of
                        how it highlights the box and also how we can get to the errors for display purposes.
                    </p>
                    <p>The first thing we might want to do is put a ToolTip on controls that have the red box so the user
...
&lt;/Window.Resources&gt;</code></pre>
                    ...
                    <p>so any FrameworkElement can have a ToolTip, we don't have to constrain it to a particular type. What
                    ...
                    <figure>
                        <pre class="prettyprint"><code>&lt;Style.Triggers&gt;</code></pre>
                        <figcaption>Fig 07-260</figcaption>
                    </figure>
                    <p>and we set up a Property Trigger on the attached property, Validation.HasError:</p>
                    <figure>
                <pre class="prettyprint"><code>&lt;Trigger Property=&quot;Validation.HasError&quot;
                         Value=&quot;true&quot;&gt;</code></pre>
                        <figcaption>Fig 07-261</figcaption>
                    </figure>
                    <p>So whichever FrameworkElement that we apply this to, basically, this is going to be monitoring the
                    ...
                    <figure>
                <pre class="prettyprint"><code>&lt;Setter Property=&quot;ToolTip&quot;
            Value=&quot;{Binding RelativeSource={x:Static RelativeSource.Self},
            Path=(Validation.Errors).CurrentItem.ErrorContent}&quot; /&gt;</code></pre>
                        <figcaption>Fig 07-262</figcaption>
                    </figure>
                    <p>This Setter is going to go set the ToolTip property of that FrameworkElement, and it's going to set
                        it to a value that it obtains from the Errors collection on the Validation Class.
                    </p>
                    <p>So it's using a RelativeSource Binding here to basically get a reference to the element itself, use
    ...
    Width=&quot;120&quot;
    Validation.Error=&quot;cellPhoneTextBox_Error&quot;
Style=&quot;{StaticResource ErrorStyle}&quot; /&gt;</code></pre>
                    ...
                    <p>Now you might also want to customize the way the control displays that error. Instead of a red box,
                    ...
                    <figure>
                <pre class="prettyprint"><code>&lt;ControlTemplate x:Key=&quot;TextBoxErrorTemplate&quot;&gt;
    &lt;DockPanel&gt;
...
&lt;/ControlTemplate&gt;</code></pre>
                    ...
                    <p>because what we're doing is we're defining a custom ControlTemplate that we're going to apply to our
                        input control.
                    </p>
                    <p>The structure of what you put in that ControlTemplate is totally up to you, but there's a magic
                    ...
                    <p>So what I've got here is I've got a DockPanel. The last element in a DockPanel will fill the
                    ...
                    <figure>
                        <pre class="prettyprint"><code>ToolTip=&quot;Contains invalid data&quot;</code></pre>
                        <figcaption>Fig 07-267</figcaption>
                    </figure>
                    <p>that we can hover over. So we've defined this ControlTemplate and now we have to associate it with
                    ...
                    <p>So we apply an implicit Style here which is going to apply to all TextBoxes that don't already have a
                        Style</p>
                    <figure>
                <pre class="prettyprint"><code>&lt;Style TargetType=&quot;TextBox&quot;&gt;
...
&lt;/Style&gt;</code></pre>
                    ...
                    <figure>
                <pre class="prettyprint"><code>&lt;Setter Property=&quot;Validation.ErrorTemplate&quot;
                    Value=&quot;{StaticResource TextBoxErrorTemplate}&quot; /&gt;</code></pre>
                        <figcaption>Fig 07-269</figcaption>
                    </figure>
                    <p>if there are validation errors. In addition, we want a ToolTip on these controls as well, so similar
                    ...
                    <p>and remember on the Cell Phone we had applied a Style explicitly for the ToolTip, but the other two
                    ...
                    <p>If we hover over the TextBox, we get the Invalid input format that's coming out of the ToolTip. So
                ...
                <div class="panel-body">
                    ...
                    <p>It's a common mechanism for expressing validation rules on properties, done through attributes.
                    ...
                    <p>Basically, you put these attributes on a property and they will be evaluated in validation scenarios.
                    ...
                    <p>Now WPF does not automatically evaluate these, so you have to write a little bit of code using two
                ...
                <div class="panel-body">
                    ...
                    <div class="example">
                        <div class="input-group">
                            <input id="Ex09-013" type="text" class="form-control"
                                   value="C:\Development Tutorials\Pluralsight-Courses\WPF Data Binding in Depth\Module 9\before\ValidationDemo\ValidationDemo.sln">
                            <span class="input-group-btn">
                	        <button class="btn" data-clipboard-target="#Ex09-013">
                        	    <img src="/./src/assets/clippy.svg" width="13" alt="Copy to clipboard">
                            </button>
        	            </span>
                        </div>
                    ...
                    <div class="example">
                        <div class="input-group">
                            <input id="Ex09-014" type="text" class="form-control"
                                   value="C:\Development Tutorials\Pluralsight-Courses\WPF Data Binding in Depth\Module 9\after\ValidationDemo\ValidationDemo.sln">
                            <span class="input-group-btn">
                	        <button class="btn" data-clipboard-target="#Ex09-014">
                        	    <img src="/./src/assets/clippy.svg" width="13" alt="Copy to clipboard">
                            </button>
        	            </span>
                        </div>
                    ...
                    <p>DataAnnotations are a capability that comes in from a reference that you'll add to your project
                    ...
                    <p>DataAnnotations are known by Entity Framework, ASP.NET, Silverlight RIA Services, and you can enable
                        them in WPF very easily.
                    </p>
                    <p>First what we're going to do is define a base class that we can put on our entities and I'll call
                        this EntityBase.
                    </p>
                    <figure>
                <pre class="prettyprint"><code>public class EntityBase
{
    protected virtual void ValidateProperty(string propertyName, object value)
    {
        ...
        Validator.ValidateProperty(value, context);
                    ...
                    <p>And we're using this to encapsulate just the few lines of code that we need to evaluate
                        DataAnnotations.</p>
                    <p>Specifically, there's a class called the ValidationContext.</p>
                    <figure>
                        <pre class="prettyprint"><code>ValidationContext context = new ValidationContext(this, null, null);</code></pre>
                    ...
                    <figure>
                        <pre class="prettyprint"><code>context.MemberName = propertyName;</code></pre>
                        <figcaption>Fig 07-275</figcaption>
                    </figure>
                    <p>and then you use the Validator class .ValidateProperty method, passing in the value that's being set
                        on that property, and the context object that you created up above:
                    </p>
                    <figure>
                        <pre class="prettyprint"><code>Validator.ValidateProperty(value, context);</code></pre>
                        <figcaption>Fig 07-276</figcaption>
                    </figure>
                    <p>that wraps the object being validated.</p>
                    <p>ValidateProperty is going to throw an exception if this value is not appropriate based on attributes
                    ...
                    <p>I'm going to comment out the ValidatePhone method call here in the HomePhone property, and I'm going
    ...
    set
    {
        if (_HomePhone != value)
        {
            ...
            PropertyChanged(this, new PropertyChangedEventArgs(&quot;HomePhone&quot;));
                    ...
                    <figure>
                        <pre class="prettyprint"><code>[RegularExpression(@&quot;^\D?(\d{3})\D?\D?(\d{3})\D?(\d{4})$&quot;)]</code></pre>
                        <figcaption>Fig 07-278</figcaption>
                    </figure>
                    <p>as one of the built-in attributes, so that we can just declaratively put the RegularExpression that
                    ...
                    <figure>
                        <pre class="prettyprint"><code>[Required]</code></pre>
                    ...
                    <figure>
                        <pre class="prettyprint"><code>[StringLength(12)]</code></pre>
                        <figcaption>Fig 07-280</figcaption>
                    </figure>
                    <p>If it was a numeric value, there is a Range attribute. There's also a CustomValidation attribute that
                    ...
                    <figure>
                        <pre class="prettyprint"><code>public class Customer : EntityBase, INotifyPropertyChanged, INotifyDataErrorInfo</code></pre>
                    ...
                    <figure>
                        <pre class="prettyprint"><code>ValidateProperty(&quot;HomePhone&quot;, value);</code></pre>
                ...
                <pre class="prettyprint"><code>public class EntityBase
{
    protected virtual void ValidateProperty(string propertyName, object value)
    {
        ...
        Validator.ValidateProperty(value, context);
                    ...
                    <p>to use the ValidationContext and Validator to basically reach back up to this property, pull off each
                    ...
                    <p>So it even gives you a nice error string there based off of the attribute, and those are localizable.
                    ...
                    <p>This will return a Boolean flag and it will populate a collection of validationResults with any
                    ...
                    <p>I'm just going to leave it with the ValidateProperty here, but generally just throwing the exception
                        and using ValidatesOnExceptions will work nicely.
                    </p>
                    <p>So that's all there is to using DataAnnotations, put the appropriate attributes on your properties,
                ...
                <div class="panel-body">
                    ...
                    <p>Next, we looked at IDataErrorInfo, which is the most common way of doing validation on business
                    ...
                    <p>Next we covered INotifyDataErrorInfo, which is an asynchronous version of IDataErrorInfo. It came in,
                    ...
                    <p>Next, we talked about some binding and validation events that are available, and you saw that these
                    ...
                    <p>Next, we looked at how you can provide custom ControlTemplates to change the way built-in controls
                    ...
                    <p>Finally, we finished off with DataAnnotations and explained that they are a standard way in .NET,
                ...
                <div class="panel-body">
                    <p>So that completes our course on WPF data binding in depth. You got a good end-to-end coverage of all
                    ...
                    <p>We got into Data Sources and what kinds of objects you can work with, what types of interfaces and
                    ...
                    <p>We got into the more advanced properties of data bindings and talked about things like doing Async
                    ...
                    <p>We covered DataTemplates in depth, and you saw that those really start to unleash the power of data
                    ...
                    <p>We looked at the Design Time features in Visual Studio and a little bit in Blend, that help you get
                    ...
                    <p>We dove under the covers a little bit and showed you what's really going on when data binding occurs
                        in building up the visual tree based on the data-bound objects.
                    </p>
                    <p>And we reviewed the common collection controls and made sure you really knew how to use all the
                    ...
                    <p>And finally, we finished off in this module with Validation. So that pretty much wraps it up. Thank
