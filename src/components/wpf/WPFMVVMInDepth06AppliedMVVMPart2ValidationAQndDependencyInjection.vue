<template>
        ...
        <div class="panel-group">
            <div class="panel panel-primary">
                ...
                <div class="panel-body">
                    ...
                    <p>First up, get rid of the TextBlock from the previous module's example and then in Visual Studio switch to the
                        Designer tab and select View>Other Windows>Data Sources:
                    </p>
                    <figure>
                        <img src="./images/wpfmvvmindepth/Fig06-001.png"/>
                    ...
                    <figure>
                        <img src="./images/wpfmvvmindepth/Fig06-002.png"/>
                    ...
                    <p>Select the Details option and expand the result. You'll see it has mappings for each of the properties based on
                        it's type. Now we're not going to use all these properties for this data entry so I can go and set some of
                        them to none:
                    </p>
                    <figure>
                        <img src="./images/wpfmvvmindepth/Fig06-002A.png"/>
                    ...
                    <figure>
                        <img src="./images/wpfmvvmindepth/Fig06-002B.png"/>
                        <figcaption>Fig06-002B</figcaption>
                    </figure>
                    <p>Now if we go into the XAML we can see that it wired up a number of things that we don't really want to be here
                    ...
                    <figure>
                        <pre class="prettyprint"><code>Loaded=&quot;UserControl_Loaded&quot;</code></pre>
    ...
    VerticalAlignment=&quot;Top&quot;
                    ...
                    <p>Now we just have to focus on the ViewModel and the interactions within that View which we'll start into in the
                        next demo.
                    </p>
                    ...
                    <figure>
                <pre class="prettyprint"><code>public class SimpleEditableCustomer : BindableBase
{
    private Guid _id;
    public Guid Id
    {
    ...
    public string FirstName
    {
        ...
        set { SetProperty(ref _firstName, value); }
                    ...
                    <figure>
                <pre class="prettyprint"><code>private SimpleEditableCustomer _Customer;
public SimpleEditableCustomer Customer
{
    ...
    set { SetProperty(ref _Customer, value); }
                    ...
                    <figure>
                <pre class="prettyprint"><code>public void SetCustomer(Customer cust)
{
...
private void CopyCustomer(Customer source, SimpleEditableCustomer target)
{
    ...
    if (EditMode)
    {
        ...
        target.Email = source.Email;
                    ...
                    <p>Now if you run the application, click on Customers, click on Edit for a Customer and you will see the data for
                        our Customer in our data entry form:
                    </p>
                    <figure>
                        <img src="./images/wpfmvvmindepth/Fig06-010.png"/>
                    ...
                    <figure>
                        <img src="./images/wpfmvvmindepth/Fig06-011.png"/>
...
&lt;/Grid&gt;</code></pre>
                    ...
                    <p>One thing that may immediately occur to you is why don’t we just have one Button and dynamically populate what
                    ...
                    <figure>
                <pre class="prettyprint"><code>&lt;Button Content=&quot;Save&quot;
    Command=&quot;{Binding SaveCommand}&quot;
    ...
    Width=&quot;75&quot; /&gt;</code></pre>
                    ...
                    <figure>
                <pre class="prettyprint"><code>public class NegatableBooleanToVisibilityConverter : IValueConverter
{
    ...
    public Visibility FalseVisibility { get; set; }
                    ...
                    <figure>
                        <pre class="prettyprint"><code>xmlns:conv=&quot;clr-namespace:ZzaDesktop.Converters&quot;</code></pre>
...
&lt;Button Content=&quot;Add&quot;
        ...
        Width=&quot;75&quot; /&gt;</code></pre>
                    ...
                    <figure>
                <pre class="prettyprint"><code>public RelayCommand CancelCommand { get; private set; }
public RelayCommand SaveCommand { get; private set; }</code></pre>
                    ...
                    <figure>
                <pre class="prettyprint"><code>public AddEditCustomerViewModel(ICustomersRepository repo)
{
    _repo = repo;
    ...
    SaveCommand = new RelayCommand(OnSave, CanSave);
                    ...
                    <figure>
                <pre class="prettyprint"><code>private void OnCancel()
{
}
...
private bool CanSave()
{
    return true;
                    ...
                    <figure>
                        <pre class="prettyprint"><code>public event Action Done = delegate { };</code></pre>
                ...
                <pre class="prettyprint"><code>private void OnCancel()
{
    Done();
                    ...
                    <figure>
                        <pre class="prettyprint"><code>_addEditViewModel.Done += NavToCustomerList;</code></pre>
                    ...
                    <figure>
                <pre class="prettyprint"><code>private void NavToCustomerList()
{
    CurrentViewModel = _customerListViewModel;
                    ...
                    <p>As soon as you start accepting data input from end users you need to consider validation. WPF has some great
                        built in features in the binding system for validating input and you can still leverage all those features
                        when doing MVVM.
                    <p>The only thing to keep in mind is that the logic that supports your validation and defines what rules exist for
                    ...
                    <figure>
                <pre class="prettyprint"><code>public class ValidatableBindableBase : BindableBase, INotifyDataErrorInfo
{
    ...
    public bool HasErrors
    {
        get { throw new NotImplementedException(); }
                    ...
                    <figure>
                        <pre class="prettyprint"><code>private Dictionary&lt;string, List&lt;string&gt;&gt; _errors = new Dictionary&lt;string, List&lt;string&gt;&gt;();</code></pre>
...
public bool HasErrors
{
    get { return _errors.Count &gt; 0; }
                    ...
                    <figure>
                <pre class="prettyprint"><code>protected override void SetProperty&lt;T&gt;(ref T member, T val, [CallerMemberName] string propertyName = null)
{
    ...
    ValidateProperty(propertyName, val);
                    ...
                    <figure>
                <pre class="prettyprint"><code>private void ValidateProperty&lt;T&gt;(string propertyName, T value)
{
    ...
    ErrorsChanged(this, new DataErrorsChangedEventArgs(propertyName));
                    ...
                    <p>Back in our AddEditCustomerView we can look at the bindings that were generated from our earlier drag and drop
                        operations:
                    </p>
                    <figure>
                <pre class="prettyprint"><code>&lt;TextBox x:Name=&quot;firstNameTextBox&quot;
     Grid.Column=&quot;1&quot;
     HorizontalAlignment=&quot;Left&quot;
     Height=&quot;23&quot;
     Margin=&quot;3&quot;
     ...
     Width=&quot;120&quot; /&gt;</code></pre>
                    ...
                    <p>If there are errors returned, then it will set certain Attached Properties on the associated Control such as:
                    </p>
                    <ul>
                    ...
                    <figure>
                <pre class="prettyprint"><code>private string _firstName;
[Required]
public string FirstName
{
    get { return _firstName; }
...
public string LastName
{
    ...
    set { SetProperty(ref _lastName, value); }
                    ...
                    <figure>
                <pre class="prettyprint"><code>private string _email;
[EmailAddress]
public string Email
{
    get { return _email; }
...
public string Phone
{
    ...
    set { SetProperty(ref _phone, value); }
                    ...
                    <figure>
                        <pre class="prettyprint"><code>public class SimpleEditableCustomer : ValidatableBindableBase</code></pre>
                        <figcaption>Fig 06-034</figcaption>
                    </figure>
                    <p>Now if we run the application, edit a Customer, try to Edit by clearing the FirstName field and we will see it
                        gets red boxed:
                    </p>
                    <figure>
                        <img src="./images/wpfmvvmindepth/Fig06-035.png"/>
                    ...
                    <p>To support disabling the SaveCommand we need to go to the AddEditCustomerViewModel and modify the SetCustomer
                        method:
                    </p>
                    <figure>
                <pre class="prettyprint"><code>public void SetCustomer(Customer cust)
...
private void RaiseCanExecuteChanged(object sender, EventArgs e)
{
    SaveCommand.RaiseCanExecuteChanged();
                    ...
                    <figure>
                <pre class="prettyprint"><code>private bool CanSave()
{
    return !Customer.HasErrors;
                    ...
                    <p>Adding a ToolTip for the validation error is as simple as defining a style for it to the AddEditCustomerView:
                    </p>
                    <figure>
...
&lt;/Style&gt;</code></pre>
                    ...
                    <p>Now if we run the application, edit a Customer, cause a validation error, tab off the field, we can see that
                        the button still disables but then when we hover over the field we and an error indication of what is going on
                        there:
                    </p>
                    <figure>
                        <img src="./images/wpfmvvmindepth/Fig06-039.png"/>
                    ...
                    <p>They can both be treated as a two phase approach to constructing. When you use a container the container takes
                        several responsibilities:
                    </p>
                    <ul>
                        <li>It constructs an object when asked. Instead of newing up objects yourself you will go to the container and
                            ask it to produce an object for you. </li>
                        <li>As part of the process the container will determine what that object depends on. Depending on the
                            container used it can do that based on parametrized constructors, properties or methods. </li>
                        <li>To provide those dependencies the container will then construct those dependencies and will inject them
                            into the object being constructed </li>
                    ...
                    <div class="example">
                        <div class="input-group">
                            <input id="Ex06-009" type="text"
                                   value="C:\Development Tutorials\Pluralsight-Courses\WPF MVVM In Depth\Module 6\ZzaDesktop-Start\ZzaDesktop.sln">
                            <span class="input-group-btn">
                	        <button class="btn" data-clipboard-target="#Ex06-009">
                        	    <img src="/./src/assets/clippy.svg" width="13" alt="Copy to clipboard">
                            </button>
        	            </span>
                        </div>
                    ...
                    <div class="example">
                        <div class="input-group">
                            <input id="Ex06-010" type="text"
                                   value="C:\Development Tutorials\Pluralsight-Courses\WPF MVVM In Depth\Module 6\ZzaDesktop-End\ZzaDesktop.sln">
                            <span class="input-group-btn">
                	        <button class="btn" data-clipboard-target="#Ex06-010">
                        	    <img src="/./src/assets/clippy.svg" width="13" alt="Copy to clipboard">
                            </button>
        	            </span>
                        </div>
                    ...
                    <ul>
                        <li>Testability – this is a hard coded, not exposed, dependency that the AddEditCustomerViewModel now has. Any
                            ...
                            substitute a mock object. </li>
                        <li>Another problem is that this ViewModel is newing up its own instance of the CustomersRepository and if you
                            ...
                            are going to use it. That is where Dependency Injection comes in. </li>
                    </ul>
                    <p>The first thing we want to do is change the code in our ViewModel to take in a reference to an
                        ICustomersRepository:
                    </p>
                    <figure>
                        <pre class="prettyprint"><code>public AddEditCustomerViewModel(ICustomersRepository repo)</code></pre>
                    ...
                    <figure>
                <pre class="prettyprint"><code>_customerListViewModel = new CustomerListviewModel(_repo);
_addEditViewModel = new AddEditCustomerViewModel(_repo);</code></pre>
                    ...
                    <figure>
                        <img src="./images/wpfmvvmindepth/Fig06-044.png"/>
                    ...
                    <figure>
                <pre class="prettyprint"><code>public static IUnityContainer Container
{
    get { return _container; }
                    ...
                    <figure>
                <pre class="prettyprint"><code>_container = new UnityContainer();
_container.RegisterType&lt;ICustomersRepository, CustomersRepository&gt;(
    new ContainerControlledLifetimeManager());</code></pre>
                    ...
                    <figure>
                        <pre class="prettyprint"><code>using Microsoft.Practices.Unity;</code></pre>
                    ...
                    <figure>
                <pre class="prettyprint"><code> _customerListViewModel = ContainerHelper.Container.Resolve&lt;CustomerListViewModel&gt;();
_addEditViewModel = ContainerHelper.Container.Resolve&lt;AddEditCustomerViewModel&gt;();</code></pre>
                    ...
                    <figure>
                <pre class="prettyprint"><code>private void UpdateCustomer(SimpleEditableCustomer source, Customer target)
{
    target.FirstName = source.FirstName;
    target.LastName = source.LastName;
    target.Phone = source.Phone;
    target.Email = source.Email;
                    ...
                    <figure>
                <pre class="prettyprint"><code>private async void OnSave()
{
    ...
    Done();
                    ...
                    <p>Now we have Dependency Injection and Validation wrapped up we will cover one more use case scenario – searching
                    ...
                    <div class="example">
                        <div class="input-group">
                            <input id="Ex06-011" type="text"
                                   value="C:\Development Tutorials\Pluralsight-Courses\WPF MVVM In Depth\Module 6\ZzaDesktop-Start\ZzaDesktop.sln">
                            <span class="input-group-btn">
                	        <button class="btn" data-clipboard-target="#Ex06-011">
                        	    <img src="/./src/assets/clippy.svg" width="13" alt="Copy to clipboard">
                            </button>
        	            </span>
                        </div>
                    ...
                    <div class="example">
                        <div class="input-group">
                            <input id="Ex06-012" type="text"
                                   value="C:\Development Tutorials\Pluralsight-Courses\WPF MVVM In Depth\Module 6\ZzaDesktop-End\ZzaDesktop.sln">
                            <span class="input-group-btn">
                	        <button class="btn" data-clipboard-target="#Ex06-012">
                        	    <img src="/./src/assets/clippy.svg" width="13" alt="Copy to clipboard">
                            </button>
        	            </span>
                        </div>
                    ...
                    <figure>
                <pre class="prettyprint"><code>&lt;Button Content=&quot;Add Customer&quot;
...
&lt;/Grid&gt;</code></pre>
                    ...
                    <ul>
                        <li>A property called SearchInput that we want to be set every time a keystroke happens so we use the
                            UpdateSourceTrigger on the binding and set that to PropertyChanged </li>
                        <li>We also need a ClearSearchCommand to clear out the search term</li>
                    </ul>
                    <figure>
                <pre class="prettyprint"><code>private string _SearchInput;
...
private void FilterCustomers(string searchInput)
{
    ...
    else
    {
        Customers = new ObservableCollection&lt;Customer&gt;(_allCustomers.Where(c =&gt; c.FullName.ToLower().Contains(searchInput.ToLower())));
                    ...
                    <p>So we just need to add the _allCustomers collection and we need to populate that when we load our Customers:
                    </p>
                    <figure>
                <pre class="prettyprint"><code>private List&lt;Customer&gt; _allCustomers;
...
public async void LoadCustomers()
{
    ...
    Customers = new ObservableCollection&lt;Customer&gt;(_allCustomers);
                    ...
                    <figure>
                        <pre class="prettyprint"><code>public RelayCommand ClearSearchCommand { get; private set; }</code></pre>
                        <figcaption>Fig 06-053</figcaption>
                    </figure>

                    <p>Construct the Command and point it to its handling method:</p>
                    <figure>
                        <pre class="prettyprint"><code>ClearSearchCommand = new RelayCommand(OnClearSearch);</code></pre>
                        <figcaption>Fig 06-054</figcaption>
                    </figure>
                    <p>And define that handling method in this case nulling out the SearchInput:</p>
                    <figure>
                <pre class="prettyprint"><code>private void OnClearSearch()
{
    SearchInput = null;
}</code></pre>
                    ...
                    <p>Another a small open source framework that helps you implement the MVVM pattern and also supports a number of
                    ...
                    <p>Brian Lagunas, one of the co-authors of the Prism framework has written a number of Pluralsight courses on the
                        subject.
                    </p>
                    <h3>Summary</h3>
                    <p>
                    ...
                    <p>Finally, we talked about MVVM frameworks and Toolkits out there and you should look into these and use one to
                        help you implement MVVM.
                    </p>
                    <p>This course has hopefully helped you understand the MVVM pattern in depth and you could go build your own MVVM
...
<script>
    export default {
        name: "WPFMVVMInDepth06AppliedMVVMPart2ValidationAQndDependencyInjection"
    }
</script>
