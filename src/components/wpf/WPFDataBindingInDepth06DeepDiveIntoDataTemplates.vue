        ...
        <div class="panel-group">
            <div class="panel panel-primary">
                ...
                <div class="panel-body">
                    <h3>Introduction</h3>
                    <p>Hi, this is Brian Noyes, and in this module we're going to dive into Data Templates. Data Templates
                    ...
                    <p>First we'll talk about the broad concepts of DataTemplates and how they work in general, and how they
                        get mapped out to the data objects that are being rendered out to provide the visual structure for
                        rendering on the screen.
                    </p>
                    <p>Next we'll talk about ExplicitDataTemplates, which are the most direct and simple way to use
                    ...
                    <p>Next we'll talk about ImplicitDataTemplates, which are a little more magical in the way that they
                    ...
                    <p>Next, we'll talk about DataTemplateSelectors, which give you a little more explicit control, but
                ...
                <div class="panel-body">
                    ...
                    <figure>
                        <pre class="prettyprint"><code>&lt;ContentControl Grid.Row=&quot;1&quot; Content=&quot;{Binding CurrentViewModel}&quot;/&gt;</code></pre>
                        <figcaption>Fig 05-001</figcaption>
                    </figure>
                    <p>And it's either going to be a ContentControl or some control derived from ItemsControl in general,
                        such as ListBox, ComboBox or DataGrid.
                    </p>
                    <p>Let's start with the ContentControl case. ContentControl, if you're not familiar, has a single
                    ...
                    <p>In this case, though, we're supplying the content as a data-bound object. It happens to be a
                    ...
                    <p>So we then supply a DataTemplate, and the DataTemplate has some structure to it. In this case we've
                        just got a child user control as the structure of that DataTemplate as the nested element:
                    </p>
                    <figure>
                <pre class="prettyprint"><code>&lt;DataTemplate DataType=&quot;{x:Type local:HierarchicalDataTemplatesViewModel}&quot;&gt;
    &lt;local:HierarchicalDataTemplatesView /&gt;
&lt;/DataTemplate&gt;</code></pre>
                        <figcaption>Fig 05-003</figcaption>
                    </figure>
                    <p>And when these come together in the data binding context, that DataTemplate provides the visual
                        structure for the data-bound object that is the content.
                    </p>
                    <p>Now a really important part of DataTemplates to understand is that when a DataTemplate gets rendered,
                    ...
                    <figure>
                <pre class="prettyprint"><code>&lt;ListBox x:Name=&quot;ProductsListBox&quot; ItemsSource=&quot;{Binding Products}&quot;
         ItemTemplate=&quot;{StaticResource ProductTemplate}&quot;&gt;
&lt;/ListBox&gt;</code></pre>
                        <figcaption>Fig 05-005</figcaption>
                    </figure>
                    <p>Now in the case of an ItemsControl, such as a ListBox, ComboBox or DataGrid, first off you're going
...
&lt;/ListBox&gt;</code></pre>
                    ...
                    <p>So here we have a grid with an image and a couple of rows in it to provide the name and description
                        for that product.
                    </p>
                    ...
                    <p>Within the DataTemplate we have individual bindings and you can see that they're just binding with
                        simple paths to properties on a single Product object.
                    </p>
                    <p>This works because, again, whenever a DataTemplate is rendered, the DataContext of the root element
                    ...
                    <p>And then in the case of an ItemsControl, this all comes together in that the ListBox, in this case,
                    ...
                    <p>And that's basically how DataTemplates work at a high level. So now let's look at the different ways
                        to get them hooked up and the different variants of DataTemplates.
                    </p>
                </div>
                <div class="panel-body">
                    <h3>Explicit DataTemplates</h3>
                    <p>So first let's look at ExplicitDataTemplates because they're the most straightforward in terms of the
                        way the DataTemplate gets hooked up to the data-bound control.
                    </p>
                    <p>First off, you're going to define a DataTemplate somewhere, and usually that's going to be in a
                    ...
                    <figure>
                <pre class="prettyprint"><code>&lt;UserControl&gt;
...
&lt;/UserControl&gt;</code></pre>
                    ...
                    <p>You're going to give that DataTemplate a key if it lives in the Resource dictionary, so that you can
                ...
                <div class="panel-body">
                    ...
                    <p>In this demo, I want to show you how you can use ExplicitDataTemplates to provide the visual
                        structure for a data-bound object that's being rendered in a control.
                    </p>
                    <p>Our starting point for this demo, and the rest of the demos in this module is this DataTemplates
                    ...
                    <p>And we've got a RelayCommand, which we've seen in previous modules for hooking up Command scenarios.
                    ...
                    <p>And then I'll add a class called ExplicitDataTemplatesViewModel. I'm going to replace this class with
                        a fleshed-out definition and just explain the code here quickly.
                    </p>
                    <figure>
                <pre class="prettyprint"><code>public class ExplicitDataTemplatesViewModel : INotifyPropertyChanged
{
        ...
        set
        {
            if (value != _Products)
            {
                ...
                PropertyChanged(this, new PropertyChangedEventArgs(&quot;Products&quot;));
                    ...
                    <p>Okay, so the fleshed out definition here is a class that implements INotifyPropertyChanged, so that
                        it can participate nicely in data binding.
                    </p>
                    <p>It exposes one property called Products, which is an observable collection of products suitable for
                        data binding, again:
                    </p>
                    <figure>
    ...
    set
    {
        if (value != _Products)
        {
            ...
            PropertyChanged(this, new PropertyChangedEventArgs(&quot;Products&quot;));
                    ...
                    <p>And then it has a Constructor that only when you're not in the DesignTime environment, and you can do
                        that by checking this Designer properties GetIsInDesignMode property.
                    </p>
                    <figure>
                        <pre class="prettyprint"><code>if (!DesignerProperties.GetIsInDesignMode(new DependencyObject()))</code></pre>
                        <figcaption>Fig 05-015</figcaption>
                    </figure>
                    <p>If it's true, then you're in the designer, if it's false you're not. So only if we're not in the
                        Designer, do we go out to the database and populate our products by going to the Products collection
                        of our Zza DBContext.
                    </p>
                    <figure>
                <pre class="prettyprint"><code>if (!DesignerProperties.GetIsInDesignMode(new DependencyObject()))
{
    ZzaDbContext context = new ZzaDbContext();
    Products = new ObservableCollection&lt;Product&gt;(context.Products.ToList());
    Products.ToList().ForEach(p =&gt; p.Image = &quot;Images/&quot; + p.Image);
                    ...
                    <p>And then we're looping over each product after we get it, and modifying the Image property to have
                        this relative path down into the Images folder that's part of our project.
                    </p>
                    <p>Now we need a View that's going to data bind to this, and whenever you do the MVVM pattern, the
                    ...
                    <p>So let's go into our View and add a local namespace so that we can get to the other objects defined
                        in our project
                    </p>
                    <figure>
                <pre class="prettyprint"><code>&lt;UserControl x:Class=&quot;DataTemplates.ExplicitDataTemplatesView&quot;
             ...
             mc:Ignorable=&quot;d&quot;
             d:DesignHeight=&quot;300&quot; d:DesignWidth=&quot;300&quot;&gt;</code></pre>
                        <figcaption>Fig 05-017</figcaption>
                    </figure>
                    <p>and then we're going to set the DataContext of this View declaratively by setting the DataContext
                        property to an instance of our new ViewModel
                    </p>
                    <figure>
                <pre class="prettyprint"><code>&lt;UserControl.DataContext&gt;
...
&lt;/UserControl.DataContext&gt;</code></pre>
                    ...
                    <p>So I'm going to add a ListBox that's bound to our Products collection. So I'll drop down here in the
...
&lt;/Grid&gt;</code></pre>
                    ...
                    <p>and because there is a Products property that exposes a collection on our ViewModel, we should be
                    ...
                    <p>I can drag and drop that into the Grid of the MainWindow. If I go ahead and run at this point, we can
                    ...
                    <p>but I'm just getting a ToString on the Product object, not very functional. So the simplest thing we
...
&lt;/Grid&gt;</code></pre>
                    ...
                    <p>If we run with that we get a slightly more functional ListBox where we see the Product name, but it's
                    ...
                    <p>Now normally when you define DataTemplates, you're going to want to make them reusable. That means
                    ...
                    <figure>
                <pre class="prettyprint"><code>&lt;UserControl.Resources&gt;
    &lt;DataTemplate x:Key=&quot;ProductTemplate&quot;&gt;
...
&lt;/Grid&gt;</code></pre>
                    ...
                    <p>And we can see what I've added here is a root Grid that has two columns. In the first column is an
                    ...
                    <p>An instance of this DataTemplate and its contents is going to be rendered out for each data-bound
                    ...
                    <p>And as part of that process, conceptually, it is setting the DataContext property to that Product
    ...
    &lt;Grid DataContext=&quot;Product&quot;&gt;
        ...
        &lt;/Grid.ColumnDefinitions&gt;</code></pre>
                    ...
                    <p>Now you don't explicitly put that there, that's going to happen automatically for you when the
                    ...
                    <p>Now that we have our template defined, we need to tie it into our ListBox, so we're going to get rid
                        of the DisplayMemberPath and instead use a property called ItemTemplate.
                    </p>
                    <figure>
                <pre class="prettyprint"><code>&lt;ListBox x:Name=&quot;ProductsListBox&quot;
     ItemsSource=&quot;{Binding Products}&quot;
     ItemTemplate=&quot;{StaticResource ProductTemplate} &quot; /&gt;</code></pre>
                    ...
                    <p>And we can put whatever structure we want into this DataTemplate to render out the data that is
                        associated with whatever visual elements we want to present them.
                    </p>
                    <p>Now we could make some further modifications to our DataTemplate. If I go to this nested grid and set
                    ...
                    <figure>
                <pre class="prettyprint"><code>&lt;Image x:Name=&quot;ProductImage&quot;
   Source=&quot;{Binding Image}&quot;
   Margin=&quot;5&quot;
   Width=&quot;100&quot; /&gt;
&lt;Grid Grid.Column=&quot;1&quot;
...
&lt;/Grid.RowDefinitions&gt;</code></pre>
                    ...
                    <figure>
                <pre class="prettyprint"><code>&lt;Grid Grid.Column=&quot;1&quot;
    Height=&quot;{Binding ElementName=ProductImage, Path=ActualHeight}&quot;&gt;</code></pre>
                        <figcaption>Fig 05-029</figcaption>
                    </figure>
                    <p>Whenever you bind to a Height or Width, make sure you're binding to the ActualHeight or
                        ActualWidth.</p>
                    ...
                    <figure>
                <pre class="prettyprint"><code>&lt;TextBlock Grid.Row=&quot;2&quot;
    Text=&quot;{Binding RelativeSource={RelativeSource AncestorType=ListBox}, Path=ItemsSource.Count}&quot; /&gt;
&lt;TextBlock Grid.Row=&quot;3&quot;
    Text=&quot;{Binding ElementName=ProductsListBox, Path=ItemsSource.Count}&quot; /&gt;</code></pre>
                        <figcaption>Fig 05-030</figcaption>
                    </figure>
                    <p>In this case, one of these is using a RelativeSource binding and you can see it's going to walk all
                    ...
                    <p>So we can walk all the way up there, use the ListBox as our Source object, and then we could us a
                    ...
                    <figure>
                <pre class="prettyprint"><code>&lt;TextBlock Grid.Row=&quot;2&quot;
    Text=&quot;{Binding RelativeSource={RelativeSource AncestorType=ListBox}, Path=ItemsSource.Count}&quot; /&gt;</code></pre>
                        <figcaption>Fig 05-031</figcaption>
                    </figure>
                    <p>Likewise, we can even use ElementName bindings that are outside of our CurrentDataTemplate. So the
                        name of our ListBox is ProductsListBox:
                    </p>
                    <figure>
                <pre class="prettyprint"><code>&lt;TextBlock Grid.Row=&quot;3&quot;
   Text=&quot;{Binding ElementName=ProductsListBox, Path=ItemsSource.Count&quot; /&gt;</code></pre>
...
&lt;/Grid&gt;</code></pre>
    ...
    Height=&quot;{Binding ElementName=ProductImage, Path=ActualHeight&quot;&gt;</code></pre>
                    ...
                    <p>I wanted to show that as an example, but it'll end up truncating those fields that we just added
                    ...
                    <p>41 products in our collection, getting one by walking up the Element Tree to the ListBox, and getting
        ...
        &lt;DataTemplate x:Key=&quot;ProductTemplate&quot;&gt;
            &lt;Grid&gt;
                ...
                &lt;Grid Grid.Column=&quot;1&quot;&gt;
                    &lt;Grid.RowDefinitions&gt;
                        &lt;RowDefinition Height=&quot;Auto&quot; /&gt;
                        &lt;RowDefinition Height=&quot;Auto&quot; /&gt;</code></pre>
                    ...
                    <p>Here we're really just setting the ItemTemplate property of the ListBox explicitly to an instance of
    ...
    &lt;ListBox.ItemTemplate&gt;
        &lt;DataTemplate x:Key=&quot;ProductTemplate&quot;&gt;
            &lt;Grid&gt;</code></pre>
                    ...
                    <p>Is no longer reusable, it's only available within the context of this ListBox. So generally, I'll
         ...
         ItemTemplate=&quot;{StaticResource ProductTemplate}&quot;/&gt;</code></pre>
                    ...
                    <p>So that's how you use ExplicitDataTemplates. Specifically point to one through the ItemTemplate
                ...
                <div class="panel-body">
                    ...
                    <p>Now let's talk about ImplicitDataTemplates. ImplicitDataTemplates give you a lot more power and
                    ...
                    <p>Basically, an ImplicitDataTemplate gets associated automatically with a data-bound object based on
                    ...
                    <p>It could be a Resource dictionary directly on the ContentControl, on the containing Grid, on the
                    ...
                    <p>Next we have to have a DataType property on our DataTemplate that matches up to the concrete type of
                    ...
                    <p>That CurrentViewModel can be one of several types and, in this case, it's checking the Type of that
                    ...
                    <p>Note the use of the x:Type markup extension, make sure you use that. It's very tempting to just put
                    ...
                    <p>So it could either be pushing a different ViewModel into this Content property as shown in this
...
&lt;/Grid&gt;</code></pre>
                    ...
                    <p>Basically what we're going to be setting up here is the ability to switch between child views,
...
&lt;/UserControl.DataContext&gt;</code></pre>
                    ...
                    <p>And we're going to first make this a proper ViewModel by implementing the INotifyPropertyChanged
                        interface:</p>
                    <figure>
                <pre class="prettyprint"><code>public class MainWindowViewModel : INotifyPropertyChanged
{
    public event PropertyChangedEventHandler PropertyChanged = delegate {};
                    ...
                    <figure>
                <pre class="prettyprint"><code>&lt;Window xmlns=&quot;http://schemas.microsoft.com/winfx/2006/xaml/presentation&quot;
    xmlns:x=&quot;http://schemas.microsoft.com/winfx/2006/xaml&quot;
    xmlns:local=&quot;clr-namespace:DataTemplates&quot;
...
&lt;/Window&gt;</code></pre>
                    ...
                    <p>And what we're going to do is drop back into our ViewModel and declare a property called
                        CurrentViewModel:</p>
                    <figure>
                <pre class="prettyprint"><code>public object CurrentViewModel
{
    get
    {
    ...
    set
    {
        ...
        PropertyChanged(this, new PropertyChangedEventArgs(&quot;CurrentViewModel&quot;));
                    ...
                    <p>So the idea is that we can set an instance of a ViewModel into this property and data bind to it from
                        the MainWindow, and that will cause the current rendered child view to switch out.
                    </p>
                    <p>So now we need some ViewModels to switch between. I'm going to pop in here two more MemberVariables,
                        one an instance of our ExplicitDataTemplatesViewModel and one an instance of our
                        DataTemplateSelectorsViewModel.
                    </p>
                    <figure>
                <pre class="prettyprint"><code>private object _CurrentViewModel;
ExplicitDataTemplatesViewModel _ViewModel1 = new ExplicitDataTemplatesViewModel();
DataTemplateSelectorsViewModel _ViewModel2 = new DataTemplateSelectorsViewModel();</code></pre>
                        <figcaption>Fig 05-053</figcaption>
                    </figure>
                    <p>Now we need some logic that's actually going to switch between those two and we're going to do that
                        with Commands.
                    </p>
                    <figure>
...
public RelayCommand View2Command { get; set; }</code></pre>
                    ...
                    <figure>
                <pre class="prettyprint"><code>private void ShowView1()
{
    CurrentViewModel = _ViewModel1;
}
...
public event PropertyChangedEventHandler PropertyChanged = delegate { };</code></pre>
...
&lt;/Grid&gt;</code></pre>
                    ...
                    <p>The ContentControl in WPF is a nice little container control that you set the content to some UI
...
&lt;/Window.Resources&gt;</code></pre>
                    ...
                    <p>And in that Resource section I define two DataTemplates, one that is mapped through the DataType
                    ...
                    <figure>
                <pre class="prettyprint"><code>&lt;ContentControl Grid.Row=&quot;1&quot;
    Content=&quot;{Binding CurrentViewModel&quot; /&gt;</code></pre>
                        <figcaption>Fig 05-060</figcaption>
                    </figure>
                    <p>that's binding content to one or the other of those ViewModel Types. The data binding is actually
                    ...
                    <p>The contents of this DataTemplate is nothing more than our User Control that represents our view in
                    ...
                    <p>So basically, the view's DataContext will be set to the instance of the ViewModel that it's being
...
private void ShowView2()
{
    CurrentViewModel = _ViewModel2;
                    ...
                    <p>With that code in place, I can go ahead and run, and now we can see the two buttons up at the top, I
                    ...
                    <figure>
                <pre class="prettyprint"><code>&lt;Button Content=&quot;Explicit DataTemplates View&quot;
                    Command=&quot;{Binding View1Command}&quot;/&gt;</code></pre>
                ...
                <pre class="prettyprint"><code>public object CurrentViewModel
{
    ...
    set
    {
        ...
        PropertyChanged(this, new PropertyChangedEventArgs(&quot;CurrentViewModel&quot;));
                    ...
                    <p>Now that a new DataObject has shown up in the Content property through a binding, Control goes out
...
&lt;/Window.Resources&gt;</code></pre>
                    ...
                    <p>ImplicitDataTemplates can also certainly be used for heterogeneous collections. Say that you had a
                ...
                <div class="panel-body">
                    ...
                    <p>As we just covered, if the criteria is just simply based on the type of the data-bound object, you
                    ...
                    <p>DataTemplateSelectors at a high level let you dynamically select the right DataTemplate based on
                    ...
                    <p>So what you do is you derive a class from a base class called DataTemplateSelector in the framework.
                        In that class, you override a SelectTemplate method so the simplest possible implementation would
                        look something like this:
                    </p>
                    <figure>
                <pre class="prettyprint"><code>public class ProductTemplateSelector : DataTemplateSelector
{
    public override DataTemplate SelectTemplate(object item, DependencyObject container)
    {
        ...
        return template;
                    ...
                    <p>Ultimately, you invoke some logic and you come up with a DataTemplate. That logic could dynamically
                    ...
                    <p>So basically, you use whatever logic is needed and come up with a DataTemplate and return it. And
                ...
                <div class="panel-body">
                    ...
                    <p>In this demo, I'm going to show you how to use DataTemplateSelectors to dynamically select an
                    ...
                    <figure>
                <pre class="prettyprint"><code>namespace DataTemplates
{
    public class ProductTemplateSelector : DataTemplateSelector
    {
        public override DataTemplate SelectTemplate(object item, DependencyObject container)
        {
            ...
            return template;
                    ...
                    <p>It takes in two arguments, the first of which is the data-bound object that's being rendered through
                    ...
                    <p>Now the implementation I'm showing here is a simple reusable version where I know that I'm going to
                        be using this for Products, so I can go ahead and cast the data-bound object to a Product object.
                    </p>
                    <figure>
                        <pre class="prettyprint"><code>Product product = item as Product;</code></pre>
                        <figcaption>Fig 05-072</figcaption>
                    </figure>
                    <p>Likewise, I can cast the container to a framework element.</p>
                    <figure>
                        <pre class="prettyprint"><code>FrameworkElement element = container as FrameworkElement;</code></pre>
                        <figcaption>Fig 05-073</figcaption>
                    </figure>
                    <p>As long as both of those casts succeeded, this TemplateSelector must be hooked up in the place I
                        expect it to be. If not, I can just punt here and return null.
                    </p>
                    <figure>
                        <pre class="prettyprint"><code>if (product == null || element == null) return null;</code></pre>
                        <figcaption>Fig 05-074</figcaption>
                    </figure>
                    <p>But as long as I got that far, then I can go and invoke some logic to figure out what DataTemplate to
                    ...
                    <figure>
                        <pre class="prettyprint"><code>DataTemplate template = element.FindResource(product.Type + &quot;Template&quot;) as DataTemplate;</code></pre>
                        <figcaption>Fig 05-075</figcaption>
                    </figure>
                    <p>and that Type is going to be pizza, salad or drink in our Zza data examples.</p>
                    <p>If I just concatenate Template onto the end of that and assume that out in the current Resource
                    ...
                    <figure>
                <pre class="prettyprint"><code>DataTemplate template = element.FindResource(product.Type + &quot;Template&quot;) as DataTemplate;
            return template;</code></pre>
                        <figcaption>Fig 05-076</figcaption>
                    </figure>
                    <p>you could new up a DataTemplate from scratch and dynamically populate its elements from programmatic
                    ...
                    <p>So what I'm going to do is go out to a View I created in a previous demo called
                    ...
                    <figure>
                <pre class="prettyprint"><code>    xmlns:local=&quot;clr-namespace:DataTemplates&quot;
    mc:Ignorable=&quot;d&quot;
    d:DesignHeight=&quot;300&quot;
    d:DesignWidth=&quot;300&quot;&gt;
&lt;UserControl.Resources&gt;
...
&lt;/UserControl.Resources&gt;</code></pre>
                    ...
                    <p>Now that I have an instance of that Selector available, I can use it in the data-bound Context. So
...
&lt;/Grid&gt;</code></pre>
                    ...
                    <p>Now we just need to get these products from somewhere, so I'm going to wire that up in the ViewModel
                    ...
                    <figure>
                <pre class="prettyprint"><code>public class DataTemplateSelectorsViewModel : INotifyPropertyChanged
{
    private ObservableCollection&lt;Product&gt; _Products;
    ...
    public ObservableCollection&lt;Product&gt; Products
    {
        ...
        set
        {
            if (value != _Products)
            {
                _Products = value;</code></pre>
                        <figcaption>Fig 05-079</figcaption>
                    </figure>
                    <p>We have a Products collection exposed. We only populate that Products collection from the database if
...
&lt;/Window.Resources&gt;</code></pre>
                    ...
                    <p>So the only thing we have left to do is define the DataTemplates that are going to be rendered out by
        ...
        &lt;DataTemplate x:Key=&quot;saladTemplate&quot;&gt;
            &lt;Grid&gt;
                &lt;Grid.ColumnDefinitions&gt;
                    &lt;ColumnDefinition Width=&quot;Auto&quot; /&gt;
                    &lt;ColumnDefinition Width=&quot;*&quot; /&gt;</code></pre>
                        <figcaption>Fig 05-081</figcaption>
                    </figure>

                    <p>One called pizzaTemplate, one called saladTemplate, and one called drinkTemplate, the Type property
                    ...
                    <figure>
                <pre class="prettyprint"><code>&lt;DataTemplate x:Key=&quot;pizzaTemplate&quot;&gt;
    &lt;Grid&gt;
        &lt;Grid.ColumnDefinitions&gt;
            &lt;ColumnDefinition Width=&quot;Auto&quot; /&gt;
            &lt;ColumnDefinition Width=&quot;*&quot; /&gt;
    ...
    &lt;/Grid&gt;
&lt;/DataTemplate&gt;
&lt;DataTemplate x:Key=&quot;saladTemplate&quot;&gt;</code></pre>
                    ...
                    <p>So by defining your own DataTemplateSelector, overriding the SelectTemplate, and invoking whatever
                ...
                <div class="panel-body">
                    ...
                    <p>The general idea is that similar to a DataTemplate, it contains a chunk of XAML that represents the
                    ...
                    <p>The first of those is the ItemsSource property on the HierarchicalDataTemplate. Basically, this
                    ...
                    <p>Then there is the ItemTemplate property on the HierarchicalDataTemplate. This provides a DataTemplate
                    ...
                    <p>You can also use ImplicitDataTemplates with HierarchicalDataTemplates, so it has a DataType property
                ...
                <div class="panel-body">
                    ...
                    <p>In this demo, I'm going to show you how to work with HierarchicalDataTemplates, which are a special
                ...
                <pre class="prettyprint"><code>public class MainWindowViewModel : INotifyPropertyChanged
{
    ...
    HierachicalDataTemplatesViewModel _ViewModel3 = new HierachicalDataTemplatesViewModel();</code></pre>
                ...
                <pre class="prettyprint"><code> public MainWindowViewModel()
{
    ...
    CurrentViewModel = _ViewModel1;
                    ...
                    <figure>
                <pre class="prettyprint"><code>private void ShowView3()
{
    CurrentViewModel = _ViewModel3;
}</code></pre>
                        <figcaption>Fig 05-089</figcaption>
                    </figure>
                    <p>So that should all look familiar from the ImplicitDataTemplates demo. So if I run at this point, I
                    ...
                    <p>and I can switch between the DataTemplateSelectorsView, the ExplicitDataTemplatesView, and this new
                    ...
                    <p>So the first thing we're going to want is some real data to work with, in our ViewModel, so I fleshed
                        out some code here. You can see that our ViewModel implements INotifyPropertyChanged, as usual:
                    </p>
                    <figure>
                        <pre class="prettyprint"><code>public class HierachicalDataTemplatesViewModel : INotifyPropertyChanged</code></pre>
                        <figcaption>Fig 05-092</figcaption>
                    </figure>
                    <p>We have a constructor that is, only when we're not in the Designer, going out to the database through
                        our DBContext and retrieving the Customers collection from our DBContext.
                    </p>
                    <figure>
                <pre class="prettyprint"><code>public HierachicalDataTemplatesViewModel()
{
    if (!DesignerProperties.GetIsInDesignMode(new DependencyObject()))
    {
        using (ZzaDbContext context = new ZzaDbContext())
        {
            Customers = new ObservableCollection&lt;Customer&gt;(
                ...
                .Include(&quot;Orders.OrderItems.Product&quot;).ToList());
                    ...
                    <p>In this case, however, it's also using the .Include method of Entity Framework to retrieve the
...
&lt;/Grid&gt;</code></pre>
                    ...
                    <p>What I added here in terms of the root structure of the View is a TreeView control. You can see that
                        the ItemsSource is pointing to our Customers collection, which will be on our DataContext that is
                        our ViewModel.
                    </p>
                    <p>Then we use the ItemTemplate as we did before with the ExplicitDataTemplates demo, to point to a
                    ...
                    <p>you can see that we are getting data rendered out based on our DataTemplate, but notice there's no
                    ...
                    <p>So I'm going to change out this normal DataTemplate for a HierarchicalDataTemplate for Customers that
...
&lt;/UserControl.Resources&gt;</code></pre>
                    ...
                    <p>It still has the same key as before, so that the ItemTemplate can point to it. It's got the same root
                    ...
                    <figure>
                <pre class="prettyprint"><code>&lt;HierarchicalDataTemplate x:Key=&quot;CustomerTemplate&quot;
        ItemsSource=&quot;{Binding Orders}&quot;&gt;
                </code></pre>
                        <figcaption>Fig 05-098</figcaption>
                    </figure>
                    <p>The ItemsSource property on a HierarchicalDataTemplate points to some property hanging off of the
                    ...
                    <figure>
                <pre class="prettyprint"><code>&lt;HierarchicalDataTemplate.ItemTemplate&gt;
    &lt;DataTemplate&gt;
        &lt;StackPanel Orientation=&quot;Horizontal&quot;&gt;
            &lt;Image Source=&quot;Images/checkmark.jpg&quot;
                   Margin=&quot;3&quot;
...
&lt;/HierarchicalDataTemplate.ItemTemplate&gt;
                    ...
                    <p>we can see we can just use a normal DataTemplate at that level and we have some simple structure
                    ...
                    <p>With that in place, if we run, we'll get one level of expansion based on that Orders collection, on
                    ...
                    <p>Now we can take this even deeper and I can replace this HierarchicalDataTemplate with an even bigger
        ...
        &lt;HierarchicalDataTemplate.ItemTemplate&gt;
            &lt;HierarchicalDataTemplate ItemsSource=&quot;{Binding OrderItems}&quot;&gt;
                &lt;StackPanel Orientation=&quot;Horizontal&quot;&gt;
                    ...
                    <p>It's got the binding to the Orders collection on each customer and the structure for each customer.
        ...
        &lt;HierarchicalDataTemplate.ItemTemplate&gt;
            &lt;DataTemplate&gt;
                &lt;StackPanel Orientation=&quot;Horizontal&quot;&gt;
                    ...
                    <p>For each OrderItem, what is the template for that? We go another level down and specify the
    ...
    &lt;DataTemplate&gt;
        ...
        &lt;/StackPanel&gt;
    &lt;/DataTemplate&gt;
&lt;/HierarchicalDataTemplate.ItemTemplate&gt;</code></pre>
                        <figcaption>Fig 05-103</figcaption>
                    </figure>
                    <p>And in this case, if this is our leaf node, we can just use a standard DataTemplate. So if we run
                    ...
                    <p>Now the problem with what we've shown so far is, this works if there's a single child collection per
                    ...
                    <p>So I'm going to replace all of these HierarchicalDataTemplates with a series of three DataTemplates
...
&lt;/Grid&gt;</code></pre>
                    ...
                    <figure>
                <pre class="prettyprint"><code>&lt;HierarchicalDataTemplate DataType=&quot;{x:Type data:Customer}&quot; ItemsSource=&quot;{Binding Orders}&quot;&gt;
    &lt;TextBlock Text=&quot;{Binding LastName}&quot;/&gt;
&lt;/HierarchicalDataTemplate&gt;
                </code></pre>
                        <figcaption>Fig 05-106</figcaption>
                    </figure>
                    <p>as our DataTemplate and we can have an ItemSource for that. Now we don't have to specify the
                        ItemTemplate here for those orders, because that can be matched up with a separate
                        ImplicitDataTemplate:
                    </p>
                    <figure>
                <pre class="prettyprint"><code>&lt;HierarchicalDataTemplate DataType=&quot;{x:Type data:Order}&quot; ItemsSource=&quot;{Binding OrderItems}&quot;&gt;
    &lt;TextBlock Text=&quot;{Binding OrderDate}&quot;/&gt;
&lt;/HierarchicalDataTemplate&gt;</code></pre>
                        <figcaption>Fig 05-107</figcaption>
                    </figure>
                    <p>so here we have another ImplicitDataTemplate that maps to Order objects and it knows how to render
                    ...
                    <figure>
                <pre class="prettyprint"><code>&lt;DataTemplate DataType=&quot;{x:Type data:OrderItem}&quot;&gt;
    &lt;TextBlock Text=&quot;{Binding Product.Name}&quot;/&gt;
&lt;/DataTemplate&gt;
                </code></pre>
...
&lt;/Grid&gt;</code></pre>
                    ...
                    <p>and go ahead and run, switch to the HierarchicalDataTemplatesView, and now you can see the rendering
                        is simpler.
                    </p>
                    <p>I don't have images involved in the templates now, but you can see we've got the name of the
                    ...
                    <p>So it's able to match up the right DataTemplate for the right node in the Tree based on the type of
                        the data object being rendered.
                    </p>
                    <p>And again, this would allow us to have multiple derived types in a heterogeneous collection in those
                ...
                <div class="panel-body">
                    <h3>Summary</h3>
                    <p>In this module, we saw how you could use DataTemplates to start to put together a much more complex
                        visual structure for the way you render out individual data-bound objects.
                    </p>
                    <p>We saw that at a high level, DataTemplates supply that visual structure for the way you're going to
                    ...
                    <p>First we dove into ExplicitDataTemplates where you saw you define a DataTemplate, you give it a name,
                        and you explicitly hook it up to the data-bound control through the ItemTemplate property or the
                        ContentTemplate property.
                    </p>
                    <p>Next, we looked at ImplicitDataTemplates, which can auto-associate an appropriate DataTemplate based
                    ...
                    <p>Then we covered DataTemplateSelectors, which give you a chunk of logic that you can write to
                    ...
                    <p>Finally, we finished off with HierarchicalDataTemplates, which apply the same capabilities to define
