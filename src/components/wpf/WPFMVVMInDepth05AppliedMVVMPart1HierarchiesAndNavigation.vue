            ...
            <div class="panel panel-primary">
                ...
                <div class="panel-body">
                    ...
                    <ul>
                        ...
                        <li>Hierarchical MVVM and how it ties in with Navigation where compose your app from hierarchies of
                            Views and ViewModels.
                        </li>
                        <li>From there we'll dive straight into writing code and start building out several use cases for a
                    ...
                    <p>Most often your Views will be implemented as a UserControl that will be hosted or contained within
                    ...
                    <p>If you have a View that’s name doesn't end in View, then append ViewModel to the end of the View name
                    ...
                    <p>But then you go look at your Model object definitions and realize that you need to get the Product
                    ...
                    <p>In fact, in the majority of real applications out there the Model types were often in a separate
                        library altogether:
                    </p>
                    <figure>
                    ...
                    <ul>
                        ...
                        <li>We'll also have a separate notional requirement, just to show some navigation, that we need to
                    ...
                    <div class="example">
                        <div class="input-group">
                            <input id="Ex05-001" type="text"
                                   value="C:\Development Tutorials\Pluralsight-Courses\WPF MVVM In Depth\Module 5\ZzaDesktop-Start\ZzaDesktop.sln">
                    ...
                    <div class="example">
                        <div class="input-group">
                            <input id="Ex05-002" type="text"
                                   value="C:\Development Tutorials\Pluralsight-Courses\WPF MVVM In Depth\Module 5\ZzaDesktop-End\ZzaDesktop.sln">
                            <span class="input-group-btn">
                    ...
                    <p>Now we will add a View and ViewModel to each one of those feature folders where the Views are
...
&lt;/Grid&gt;</code></pre>
                    ...
                    <figure>
                <pre class="prettyprint"><code>xmlns:x=&quot;http://schemas.microsoft.com/winfx/2006/xaml&quot;
...
&lt;/Window.DataContext&gt;</code></pre>
                    ...
                    <figure>
                        <pre class="prettyprint"><code>public object CurrentViewModel { get; set; }</code></pre>
                    ...
                    <figure>
                <pre class="prettyprint"><code>private CustomerListViewModel _customerListViewModel = new CustomerListViewModel();
private OrderViewModel _orderViewModel = new OrderViewModel();
private OrderPrepViewModel _orderPrepViewModel = new OrderPrepViewModel();</code></pre>
                    ...
                    <figure>
                <pre class="prettyprint"><code>namespace ZzaDesktop
{
    public class BindableBase : INotifyPropertyChanged
    {
        public event PropertyChangedEventHandler PropertyChanged = delegate { };
                    ...
                    <p>and then it needs to expose a way for the derived classes to trigger that PropertyChanged event at
                    ...
                    <figure>
                <pre class="prettyprint"><code>protected virtual void SetProperty&lt;T&gt;(ref T member, T val, [CallerMemberName] string propertyName = null)
{
...
protected virtual void OnPropertyChanged(string propertyName)
{
    PropertyChanged(this, new PropertyChangedEventArgs(propertyName));
                    ...
                    <p>The SetProperty method is intended to be set from the set block of each property and encapsulates the
                    ...
                    <figure>
                <pre class="prettyprint"><code>class MainWindowViewModel : BindableBase
{
    ...
    public BindableBase CurrentViewModel
    {
        ...
        set { SetProperty(ref _CurrentViewModel, value); }
                    ...
                    <figure>
                <pre class="prettyprint"><code>public class BindableBase : INotifyPropertyChanged
{
    protected virtual void SetProperty&lt;T&gt;(ref T member, T val,
                [CallerMemberName] string propertyName = null)</code></pre>
                        <figcaption>Fig 05-008A</figcaption>
                    </figure>
                    <p>The by ref member variable comes in in the first argument, the value comes in second and the third
                        one gets set with a nice feature of C# which is the CallerMemberName attribute:
                    </p>
                    <figure>
                        <pre class="prettyprint"><code>[CallerMemberName] string propertyName = null</code></pre>
                        <figcaption>Fig 05-008B</figcaption>
                    </figure>
                    <p>That attribute makes it so it can automatically pick off the name of the thing that called in here –
                ...
                <pre class="prettyprint"><code>class MainWindowViewModel : BindableBase
{
    ...
    public BindableBase CurrentViewModel
    {
        ...
        set { SetProperty(ref _CurrentViewModel, value); }
                    ...
                    <figure>
                <pre class="prettyprint"><code>namespace ZzaDesktop.Customers
{
    ...
    {</code></pre>
                    ...
                    <figure>
                <pre class="prettyprint"><code>namespace ZzaDesktop.Orders
{
    ...
    {</code></pre>
                    ...
                    <figure>
                <pre class="prettyprint"><code>namespace ZzaDesktop.OrderPrep
{
    class OrderPrepViewModel : BindableBase
    {
                    ...
                    <figure>
                <pre class="prettyprint"><code>private void OnNav(string destination)
{
    switch (destination)
    {
        case &quot;orderPrep&quot;:
        ...
        default:
            ...
            break;
                    ...
                    <figure>
                        <pre class="prettyprint"><code>public RelayCommand&lt;string&gt; NavCommand { get; private set; }</code></pre>
...
&lt;/Grid&gt;</code></pre>
                    ...
                    <p>Now to start making this more real we're going to flesh out the CustomerListView cause the only way
...
&lt;/Grid&gt;</code></pre>
                    ...
                    <figure>
                <pre class="prettyprint"><code> &lt;Button Content=&quot;Place Order&quot;
    Command=&quot;{Binding DataContext.PlaceOrderCommand, RelativeSource={RelativeSource AncestorType={x:Type DataGrid}}}&quot;
    CommandParameter=&quot;{Binding}&quot;
    Margin=&quot;5&quot; /&gt;</code></pre>
                    ...
                    <p>In the CustomerListViewModel we will start by adding the Customers Collection and the
                        PlaceOrderCommand:
                    </p>
                    <figure>
...
public ObservableCollection&lt;Customer&gt; Customers
{
    ...
    set { SetProperty(ref _customers, value); }
                    ...
                    <figure>
                <pre class="prettyprint"><code>public async void LoadCustomers()
{
    Customers = new ObservableCollection&lt;Customer&gt;(
        await _repo.GetCustomersAsync());
                    ...
                    <figure>
                <pre class="prettyprint"><code>&lt;i:Interaction.Triggers&gt;
...
&lt;/i:Interaction.Triggers&gt;</code></pre>
                    ...
                    <figure>
                        <pre class="prettyprint"><code>public RelayCommand&lt;Customer&gt; PlaceOrderCommand { get; private set; }</code></pre>
                        <figcaption>Fig 05-021</figcaption>
                    </figure>
                    <p>In the constructor for the ViewModel initialize the Command property and point it to a handling
                        method:
                    </p>
                    <figure>
                <pre class="prettyprint"><code> public CustomerListViewModel()
{
    PlaceOrderCommand = new RelayCommand&lt;Customer&gt;(OnPlaceOrder);
                    ...
                    <figure>
                <pre class="prettyprint"><code>private void OnPlaceOrder(Customer customer)
{
    PlaceOrderRequested(customer.Id);
}</code></pre>
                        <figcaption>Fig 05-023</figcaption>
                    </figure>
                    <p>Just to make sure that is all setup correctly we can set a break point on the OnPlaceOrder method and
                    ...
                    <p>
                        ...
                        child ViewModel needs to be able to communicate up to its parent ViewModel:</p>
                    <figure>
                        <img src="./images/wpfmvvmindepth/Fig05-023A.png"/>
                        <figcaption>Fig 05-023A</figcaption>
                    </figure>
                    <p>
                        which is the MainWindowViewModel because that is where the navigation is being managed.</p>
                    <p>
                    ...
                    <figure>
                        <pre class="prettyprint"><code>public event Action&lt;Guid&gt; PlaceOrderRequested = delegate { };</code></pre>
                    ...
                    <p>
                        ...
                        the Customer Id of the Customer the Command was raised for:</p>
                    <figure>
                <pre class="prettyprint"><code>private void OnPlaceOrder(Customer customer)
{
    PlaceOrderRequested(customer.Id);
                ...
                <pre class="prettyprint"><code> public MainWindowViewModel()
        {
            ...
            _customerListViewModel.PlaceOrderRequested += NavToOrder;
                    ...
                    <figure>
                        <pre class="prettyprint"><code>private void NavToOrder(Guid customerId)</code></pre>
                    ...
                    <figure>
                <pre class="prettyprint"><code>private void NavToOrder(Guid customerId)
        {
            ...
            CurrentViewModel = _orderViewModel;
                    ...
                    <figure>
                <pre class="prettyprint"><code>private Guid _CustomerId;
...
public Guid CustomerId
{
    ...
    set { SetProperty(ref _CustomerId, value); }
                    ...
                    <figure>
                <pre class="prettyprint"><code>&lt;StackPanel Orientation=&quot;Horizontal&quot;&gt;
...
&lt;/StackPanel&gt;</code></pre>
                    ...
                    <div class="example">
                        <div class="input-group">
                            <input id="Ex05-011" type="text"
                                   value="C:\Development Tutorials\Pluralsight-Courses\WPF MVVM In Depth\Module 5\ZzaDesktop-Start\ZzaDesktop.sln">
                            <span class="input-group-btn">
                	        <button class="btn" data-clipboard-target="#Ex05-011">
                        	    <img src="/./src/assets/clippy.svg" width="13" alt="Copy to clipboard">
                            </button>
        	            </span>
                        </div>
                    ...
                    <div class="example">
                        <div class="input-group">
                            <input id="Ex05-012" type="text"
                                   value="C:\Development Tutorials\Pluralsight-Courses\WPF MVVM In Depth\Module 5\ZzaDesktop-Start\ZzaDesktop.sln">
                            <span class="input-group-btn">
                	        <button class="btn" data-clipboard-target="#Ex05-012">
                        	    <img src="/./src/assets/clippy.svg" width="13" alt="Copy to clipboard">
                            </button>
        	            </span>
                        </div>
    ...
    &lt;DataGrid ItemsSource=&quot;{Binding Customers}&quot;
              ...
              Grid.Row=&quot;1&quot;&gt;</code></pre>
                    ...
                    <figure>
                <pre class="prettyprint"><code>&lt;DataGridTemplateColumn Width=&quot;Auto&quot;&gt;
    &lt;DataGridTemplateColumn.CellTemplate&gt;
        &lt;DataTemplate&gt;
            &lt;Button Content=&quot;Place Order&quot;
                    Command=&quot;{Binding DataContext.PlaceOrderCommand, RelativeSource={RelativeSource AncestorType={x:Type DataGrid}}}&quot;
...
&lt;/DataGridTemplateColumn&gt;</code></pre>
                    ...
                    <figure>
                <pre class="prettyprint"><code>public RelayCommand AddCustomerCommand { get; private set; }
public RelayCommand&lt;Customer&gt; EditCustomerCommand { get; private set; }</code></pre>
                ...
                <pre class="prettyprint"><code>public CustomerListViewModel()
{
    ...
    EditCustomerCommand = new RelayCommand&lt;Customer&gt;(OnEditCustomer);
                    ...
                    <figure>
                <pre class="prettyprint"><code>private void OnAddCustomer()
{
...
private void OnEditCustomer(Customer cust)
{
    EditCustomerRequested(cust);
                    ...
                    <figure>
                <pre class="prettyprint"><code>public event Action&lt;Customer&gt; AddCustomerRequested = delegate { };
public event Action&lt;Customer&gt; EditCustomerRequested = delegate { };</code></pre>
...
private void OnEditCustomer(Customer cust)
{
    EditCustomerRequested(cust);
                    ...
                    <figure>
                        <pre class="prettyprint"><code>private AddEditCustomerViewModel _addEditViewModel = new AddEditCustomerViewModel();</code></pre>
                    ...
                    <figure>
                <pre class="prettyprint"><code>_customerListViewModel.AddCustomerRequested += NavToAddCustomer;
_customerListViewModel.EditCustomerRequested += NavToEditCustomer;</code></pre>
                        <figcaption>Fig 05-039</figcaption>
                    </figure>
                    <p>Now we just need to implement the handling methods:</p>
                    <figure>
                <pre class="prettyprint"><code>private void NavToAddCustomer(Customer cust)
{
...
private void NavToEditCustomer(Customer cust)
{
    ...
    CurrentViewModel = _addEditViewModel;
                    ...
                    <figure>
                <pre class="prettyprint"><code>class AddEditCustomerViewModel : BindableBase
{
...
&lt;/Grid&gt;</code></pre>
                    ...
                    <figure>
                <pre class="prettyprint"><code>&lt;DataTemplate DataType=&quot;{x:Type cust:AddEditCustomerViewModel}&quot;&gt;
    &lt;cust:AddEditCustomerView /&gt;
&lt;/DataTemplate&gt;</code></pre>
                        <figcaption>Fig 05-043</figcaption>
                    </figure>
                    <p>
                        This maps the AddEditCustomerViewModel to the AddEditCustomerView so it can be rendered out.</p>
                    <p>
                    ...
                    <ul>
                        <li>You could have a client service that manages navigation for a given container or set of
                            containers.
                        </li>
                        <li>If you use a framework like Prism you could do something like region navigation</li>
                    </ul>
                    <p>However, this approach is simple and straightforward and it only takes moments to stub each
                        navigation out.
                    </p>
                    <p>
                    ...
                    <ul>
                        <li>Naming and location of components is important for maintainability and you want to put some
                        ...
                        <li>We saw how hierarchical MVVM works and how it can mirror the way you can compose a complex
                        ...
                        <li>And then you started to see through the demos that building out an MVVM app is a progressive
                    ...
                    <p>In the next module we'll continue building out those use cases and we'll look at things like
