    ...
    <div class="panel-group">
      <div class="panel panel-primary">
        ...
        <div class="panel-body">
          ...
          <p>So above is an illustration of how we were running when we used CodePen in the previous couple of examples.
            We automatically loaded up the Vue JS file by adding it as a dependency to the project. We also then added a
            little bit of code directly on CodePen. The two together formed a usable application. The key thing there to
          ...
          <p>Essentially webpack's purpose is to look at a big collection of files and possibly dependencies as well
            (like Vue JS) and combine them all down to one single file which can then be sent down to your users browser
            when they try to make use of your application.
          </p>
          <p>The webpack tool is one of the big reasons that we're making use of Vue CLI. webpack is somewhat notorious
            for being a little difficult to set up out of the box. So by making use of Vue CLI it automatically sets up
          ...
          <p>Along with Vue CLI we also get access to Babel preconfigured for us right of the box. You may have heard of
            ...
            Javascript which we might want to use. Just like webpack, Babel is also included automatically inside of Vue
          ...
          <p>Let's continue in the next section where we're going to start up our application and get a better sense of
            all the different files and folders that were created for us automatically when we just generated the
            project.
          </p>
          <h3>Project Walkthrough</h3>
          <p>In the last section we spoke about how we get webpack and Babel setup for us for free whenever we make use
            of Vue CLI.
          </p>
          ...
          <p>Let's now open up our code editor inside of the project directory and take a look at some of the different
...
new Vue({
    ...
    render: h =&gt; h(App)
          ...
          </span> rather than being <span class="filename">.js</span> or anything like that. Let's open up the file and
            have a look:
          </p>
          <figure>
...
&lt;/style&gt;</pre>
          ...
          <p>This figure demonstrates one possible way of structuring a Vue app. We might define our Vue template inside
            of some Html like an actual Html document and we might create our Vue instance over in some Javascript code.
          </p>
...
&lt;/style&gt;
          ...
          <p>So in this scenario we are making use of a paradigm called Vue files. The idea behind a Vue file is that
            you'll have exactly one file that is responsible for creating a single component inside of your application.
          </p>
          <p>A single component is created as a reusable piece of code that can be used all over your application many
            times. Inside the single Vue file you'll find not only the template that your Vue file or your Vue component
          ...
          <p>What I want to point out here is that even when we start making use of Vue files. Even though we've got our
          ...
          <p>
            ...
            Html, Javascript and CSS. That means that we don't have to go hunting around different directories inside of
          ...
          <p>It might take a little bit of time to get used to this Vue file syntax but over time I'm very confident
            that you'll come to enjoy it. We've spoken a little bit about the purpose of the Vue file at this point but
            we have't really spoken about how this strange syntax is used to build an actual application.
          </p>
          <p>In the next section we're going to examine some of the behind the scenes stuff that occurs to get the code
            you create inside your Vue files to run inside of your browser.
          </p>
          <h3>Behind the Scenes of Vue Files</h3>
          <p>In the last section we started talking about Vue files. Inside of a single Vue file we will locate all the
          ...
          <p>So in this section we will run through a quick demonstration of what this file turns into when it gets sent
          ...
          <p>Then I'm going to go over to my code editor and make one very small change to the Vue file and then save
            it. Go back over to the request log and you should see a whole bunch of Javascript code that represents that
    ...
    { attrs: { id: \&quot;app\&quot; } },
        ...
        1
          ...
          <p>So even though this syntax still might look very strange - it might not look like any Vue code we've
            written or looked at so far - I hope that you can at least get the sense that everything inside the Vue file
          ...
          <p>Ok, so now that we've got a better idea of what's happening inside of a Vue file, let's continue in the
            next section and do a little bit more of a walk through of some of the code that we have inside the project
          ...
          <p>You can think of a Vue component as a blueprint or a set of rules on how to create something that can be
            ...
            file we are going to define exactly one component inside of it and that component is going to have this set
          ...
          <p>On the other hand is a Vue instance which is what we were making back inside of CodePen. A Vue instance is
          ...
          <p>So like I said we are always going to create one Vue file for every Vue component. One of the critical
            steps that we're going to undergo in every Vue application that we put together is to plan out the different
            components that we might want to make. In general we like to look at mockups of the applications that we are
            going to make and do some quick brainstorming on how we might assemble a set of different components go make
            up that application.
          </p>
          <p>So with that in mind I took the liberty of taking our mockup for the video-browser application and thinking
          ...
          <p>So you'll notice I've added some colored boxes on this mockup and then on every box I've attached a label.
            So the label that you see here is the name of a component that we're going to make to implement the
            video-browser application. So you and I are going to make one Vue file called <span class="filename">SearchBar.vue</span>.
            This is going to have a single Vue component that is going to contain a text input field and anytime a user
          ...
          <p>So the VideoList component will contain many VideoListItems inside of it because the VideoListItem
            ...
            VideoDetail which is responsible for showing details about one single video and then all of these components
            will be assembled underneath one component that we will call the App component. This component is kind of
            like the central brains of our entire application and it represents the single point that kind of organizes
          ...
          <p>So this is the overall structure that you and I are going to eventually head towards as we start to build
            ...
            App component will show the VideoDetail, the SearchBar, and the VideoList. In turn, the VideoList component
          ...
          <p>So that's how all these different pieces are going to be wired together. Again we're going to make one
            separate Vue file for each of these different components and each of those Vue files is going to contain all
