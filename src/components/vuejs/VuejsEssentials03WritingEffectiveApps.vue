...
&lt;/script&gt;</pre>
   ...
   render: h =&gt; h(App)
          ...
          <p>Now, anytime this file is executed, we will try to create a new Vue instance. When that Vue instance is
            created the <code class="prettyprint">render</code> function will be called with the
            <code class="prettyprint">createElement</code> function. We then pass in our App component to
            <code class="prettyprint">createElement</code> and the entire result is return from the <code
              class="prettyprint">render</code> function.
          </p>
          <p>Our App component is created and then the result of that is stuck into the DOM. We're going to come back
            in the next section and make sure that this entire file actually gets some content on the screen of our
            browser.
          </p>
          <h3>Multiple Ways of DOM Attachment</h3>
          <p>In the last section we started working on our main.js file. So we've now created a new Vue instance and
            we've told this instance that we wanted to do to get some content on the screen. So when we eventually
            render this thing to our actual DOM or try to get it to show up inside the browser - this render function
            will be called and we are telling that render function that we want to display an instance of our App
            component:
          </p>
          <figure>
<pre class="prettyprint">new Vue({
   render: h =&gt; h(App)
          ...
          <p>Now the very last thing we need to do inside of here is to make sure that we actually try to take this
            ...
            tells Vue where it should stick the rendered Html into inside of our Html structure:
          </p>
          <figure>
            <code class="prettyprint">el: '#app',</code>
            <figcaption>Fig 03-020</figcaption>
          </figure>
          <p>And so the <code class="prettyprint">#app</code> above means go and find some element inside of our DOM
            with an <code class="prettyprint">id</code> of app. The same thing has to be provided to our Vue instance in
            our main.js file as well. So if we ran this code right now we would create our application but it would not
            show up on the screen because we have not yet told Vue where to put this Html.
          </p>
          <p>You'll recall that inside the public directory we've got that index.html file and inside there is a <code
            class="prettyprint">div</code> with an <code class="prettyprint">id</code> of <code
            class="prettyprint">app</code>. So when we generated Vue CLI we already got a place created right out of the
            box for us to render our application to:
          </p>
          <figure>
            <pre class="prettyprint">&lt;div id=&quot;app&quot;&gt;&lt;/div&gt;</pre>
            <figcaption>Fig 03-021</figcaption>
          </figure>
          <p>So to get our main.js file's Vue instance to actually show up inside the browser we need to provide an
            <code class="prettyprint">el</code> property:
          </p>
          <figure>
                    <pre class="prettyprint">new Vue({
    el: '#app',
    render: h =&gt; h(App)
          ...
          <p>One thing I want to quickly mention here is back when we first generated our Vue CLI project, like just a
            moment ago, you might recall that when we looked at the main.js file we did not have an object or a property
            in here called <code class="prettyprint">el</code>.
          </p>
          <p>So when we first generate this project main.js looked a little bit different and previously, out of the
            box, it actually had a different way of specifying the element that we wanted to render our application to.
          </p>
          <p>In particular it created the new Vue instance and then it called a function on that instance that was
            created:
          </p>
          <figure>
                    <pre class="prettyprint">new Vue({
    render: h =&gt; h(App)
          ...
          <p>Now let me be clear. In the Vue JS world there's like two or three or four or five ways of doing any given
            operation. We have already seen many different ways in which we can define templates that are tied to our
            application. This entire idea, described above, of mounting or somehow rendering our components in Vue
            instances to the DOM is no different. So <code
              class="prettyprint">#mount</code> and then passing in <code
              class="prettyprint">'#app'</code> is identical to writing out <code class="prettyprint">el: '#app',</code>
            It's just two different ways of doing the exact same operation. Like I said, this is a pattern that you are
            going to notic in the Vue world over and over again - there's more than one way to do just about everything.
          </p>
          <p>So how do you decide which approach to use? It's pretty much down to personal preference but just for the
            sake of convention, and sticking with what Vue CLI does by default, we'll stay with the <code
              class="prettyprint">#mount</code> option rather than using <code
              class="prettyprint">el</code>
          </p>
          <p>So that's pretty much it to get some content on the screen. Now last thing we have to do is go back over to
            the browser to just test our application and make sure that we can see the text we specified appear on the
            screen.
          </p>
          <p>So I can go back over to my browser to localhost:8080 and do a forceable refresh of the application and you
            should see the text "Hi there!"
          </p>
          <p>So in the next section we're going to start really diving into our application and building out some more
            of the guts and different components that we had spoken about earlier like the search bar and the video list
            and so on.
          </p>
          <h3>Making the SearchBar</h3>
          <p>In the last section we put together our app component. We're now going to move forward to the second big
            component that we're going to work on inside the application which is the search bar.
          </p>
          <p>Remember the idea here is that this SearchBar component is going to show an input filed and any time a user
            types in there we will trigger a search on the YouTub API to attempt to go and find some list of videos
            related to that search term.
          </p>
          <p>So let's flip back over to our code editor where we're going to create a new Vue file to contain the
            SearchBar component. So back inside my code editor I'll find the src directory. We create a components
            directory inside of the src directory and inside of that directory we will locate all the different
            components we have. So inside the components folder we create a new file called SearchBar.vue. Inside the
            SearchBar.vue file we're going to pu down some different boilerplate tags inside here that are going to
            become very familiar over time. So we'll first begin by placing a <code class="prettyprint">template</code>
            tag and then underneath that we'll add a <code class="prettyprint">script</code> tag. Finally we'll add a
            <code
              class="prettyprint">style</code> tag which will eventually contain some CSS that is solely related to this
            one component and no other component inside of our application.
            <figure>
                        <pre class="prettyprint">&lt;template&gt;
&lt;/template&gt;
...
&lt;/style&gt;</pre>
          ...
          <p>Back up inside the <code class="prettyprint">template</code> tag we'll immediately create an <code
            class="prettyprint">input</code> element and we'll wrap it with a <code
...
&lt;/template&gt;</pre>
          ...
          <p>Honestly that's really just for styling. By default a <code class="prettyprint">div</code> has a CSS
            display property ot block which will make sure that this input element appears in the browser on it's own
            individual line.
          </p>
          <p>So the <code class="prettyprint">div</code> here is really just kind of for layout purposes not necessarily
            because this component actually requires it from any technical standpoint or functionality standpoint.
          </p>
          <p>Next we're going to start working inside the <code class="prettyprint">script</code> tag. We'll add the
            following code:
          </p>
          <figure>
<pre class="prettyprint">&lt;script&gt;
...
&lt;/script&gt;</pre>
          ...
          <p>Let's get started by adding the name parameter which we spoke about a couple of sections back . So I'm
            going to provide a name of simply SearchBar like so:
          </p>
          <figure>
                    <pre class="prettyprint">&lt;script&gt;
export default {
...
&lt;/script&gt;</pre>
          ...
          <p>So if you go back over to the browser and refresh the page you'll notice that the search bar doesn't appear
            at all - which kind of makes sense.
          </p>
          <p>If you actually want to get any component to show up inside of a Vue application your have to manually wire
            it up to another component. So to get that search bar to appear we have to wire it up to the App component.
            To do so, go back to the code editor and add the following code to the App.vue file inside the <code
              class="prettyprint">script</code> tag:
          </p>
          <figure>
...
&lt;/template&gt;</pre>
          ...
          <p>Ok so let's save this file and go back to the browser to see what happens. In the Console log you might
            notice an error message similar to:
          </p>
          <figure>
          ...
          <p>You see whenever you want to nest a component like this or show one component inside of another it's not
            quite enough just to import that component and then place it into the template. There's actually one other
            step that we have to go through. I illustrated this error because the last step is very easy to forget.
          </p>
          <p>Ok so back over inside of our code editor, still inside the App.vue file, we imported the SearchBar (step
            1) and we used it inside the template (step 2). Step number 3 involves adding a components property. This is
            going to be an object that lists all the different components that are going to be used inside of the app's
            template:
          </p>
          <figure>
    ...
    components: {
        SearchBar: SearchBar
          ...
          <p>We can use a little bit of ES 2015 syntax to shorten this up because the key and the value are identical:
          </p>
          <figure>
<pre class="prettyprint">export default {
    name: 'App',
    components: {
        SearchBar
          ...
          <p>Save the App.vue file and flip back over to the browser and you should notice that we our input field
            displayed on screen.
          </p>
          ...
          <p>In the next we'll start thinking about how we're going to get our SearchBar file to recognize the user
            typing some text in the input text field and as a result performing a search on YouTube using the YouTube
            API.
          </p>
          <h3>Event Directives</h3>
          <p>I'm inside of our SearchBar.vue file which is now visible inside of our application. So our goal now is to
            make sure that anytime the user types inside the input text we trigger some type of search in the YouTube
            API. Let's first get started by just making sure that we can get some event or some notification anytime the
            user types in the input text field. We already did this previously in one of our CodePen exercises. You'll
            recall that we added a directive to the <code
              class="prettyprint">input</code> tag in the <code class="prettyprint">template</code> and then we'll also
            add a <code class="prettyprint">method</code> object to our component definition.
          </p>
...
&lt;/script&gt;</pre>
          ...
          <p>So I'm going to make a method called onInput. The onInput function will be called with an event which will
...
&lt;/template&gt;</pre>
          ...
          <p>So let's save this and back in the browser refresh the page and enter some text into the text input field
            and you'll see the console logs start to appear.
          </p>
          <figure>
            <img src="./images/vuejsessentials/Fig03-038.png"/>
            <figcaption>Fig 03-038</figcaption>
          </figure>
          <p>In the next we'll start thinking about how we can use the text entered as a search term in the YouTube API.
          </p>
          ...
          <p>In the last section we wired up the <code class="prettyprint">onInput</code> directive to our <code
            ...
            <code class="prettyprint">v-on</code> directive we used on the <code
          ...
          <p>So we have replaced the <code class="prettyprint">v-on:input</code> directive with a more concise syntax
            <code class="prettyprint">@input</code>. So this can be read as anytime someone triggers an input event on
            this element run the <code class="prettyprint">onInput</code> function.
          </p>
          <h3>Data Sharing Between Components</h3>
          <p>The SearchBar component is now aware of what a user is typing into your application thanks to the onInput
            method. So now we can get a search term to use with the YouTube API. But where inside of our application
            should locate the code to actually execute that search?
          </p>
          ...
          <p>In our application we are trying to display a list of videos to our user and we're also trying to allow
            ...
            VideoDetail and VideoList components.
          </p>
          <p>On the other hand the SearchBar component does'nt really care about the list of videos that are being
          ...
          <p>So anytime that we want to communicate information (data or arbitrary information) throughout our app it is
            ...
            VideDetail component can easily communicate data with the App component and likewise the App component can
          ...
          <p>In the next section we'll figure out how we're going to make sure that the SearchBar can somehow tell the
            App component that it needs to execute a search of the YouTube API.
          </p>
          <h3>Communication with Props and Events</h3>
          <p>In the last section we talked about how the App component is the best location to put some logic to search
            the YouTube API. The next thing we have to do is to figure out how to get the SearchBar to somehow tell the
            App component that it needs to execute a search in the first place. At present the SearchBar component is
            just reading in input that is provided by the user. So the SearchBar has to somehow say to the App - "Hey
            we've got some new input it's time for you to execute a search."
          </p>
          <p>So let's now talk about how we can communicate from a child component up to a parent one. To communicate
            between parent and child components varies on the direction we are trying to communicate.
          </p>
          <p>So if we want the App component to provide some information or data or just communicate something down to
            ...
            anytime we want to communicate from the App down to the SearchBar we will pass a prop.
          </p>
          <p>A prop can be anything from an array to an object, string or function - all types of values. We will see
          ...
          <p> In the next section we will wire up some events between the SearchBar component and the App component.
          </p>
          ...
          <p>In the last section we spoke about how a parent component can communicate to the child by passing props and
            how the child can communicate with the parent by emitting events.
          </p>
          <p>So let's now go back over to our code editor and add some code to the SearchBar component to make sure that
            it emits an event anytime someone change that <code class="prettyprint">input</code> element.
          </p>
          <p>So in SearchBar.vue every single time that the onInput function is called we are going to want to emit an
            event to the inform that App component that there is a new term to search the YouTube API with.
          </p>
    ...
    onInput: function(event) {
        this.$emit();
          ...
          <p>The first argument to this function is the name of the event we want to emit. In this case I'll use a name
            of termChange to indicate that our search term just changed:
          </p>
          <figure>
                    <pre class="prettyprint">methods: {
    onInput: function(event) {
        this.$emit('termChange');
          ...
          <p>As an aside I want to mention that when we were listening for an <code class="prettyprint"> input</code>
            event on the <code class="prettyprint">input</code> text field in the <code
              class="prettyprint">template</code> input is a special event name. In other words input elements emit
            events called input anytime someone inputs something into them. However when you and I are emitting events
            we have complete control over the name that we choose.
          </p>
          <p>I used termChange because one convention is to use the name of what is about to occur and then a verb that
            indicates what just changed.
          </p>
          <p>The second argument can be used to provide some additional information about the event that just occurred.
            ...
            available on <code class="prettyprint">event.target .value</code>. So let's add that as the second argument:
          </p>
          <figure>
                    <pre class="prettyprint">methods: {
    onInput: function(event) {
        this.$emit('termChange', event.target.value);
          ...
          <p>The last thing I want to mention here is the dollar sign in the emit function. There's nothing special
            about using the dollar sign with a function name - it is valid Javascript. So the creators of the Vue
            library could just have easily have called this emit and left out the dollar sign. The reason they added the
            dollar sign in there is just in case you ever decided to add in some additional property to your component
            called emit as well.
          </p>
          <p>In the next section we will work on our App component and make sure it listens for the SearchBar to emit an
            event. Anytime that occurs we'll then trigger a search on the YouTube API.
          </p>
          ...
          <p>Our SearchBar component is now emitting an event anytime someone enters some new text. We're now going to
            open our App component and make sure that anytime the SearchBar emits the termChange event the App has the
            ability to run some code that will do a search on the YouTube API.
          </p>
          ...
          <p>We specify a method name of <code class="prettyprint">onTermChanged</code> to run anytime the <code
            class="prettyprint">termChange</code> event is triggered.
          </p>
    ...
    methods: {
        onTermChange: function() {
          ...
          <p>One quick thing I want to mention about these function declarations that we've been writing is that we've
            been using the function keyword (see Fig 03-045 above). If we wanted to we could use a little bit of ES2015
...
<pre class="prettyprint">export default {
        ...
        methods: {
          ...
          <p>Because we add <code class="prettyprint">event.target.value</code> as the second argument it will show up
            as the first argument to our callback function or event handler:
          </p>
          <figure>
<pre class="prettyprint">methods: {
...
<pre class="prettyprint">methods: {
    onTermChange(searchTerm) {
        console.log(searchTerm);
          ...
          <p>Ok, let's save and then go back to the browser to test our code. Refresh the page and if you type some text
            you should see it logged in the console as before. The difference this time round is the console log is
            being called in the App.vue onTermChange method.
          </p>
          ...
          <p>Our App component is now aware of anytime the user enters in a new search term. So inside the App.vue
            file's onTermChange function is where we'll probably want to locate a little bit of code to somehow initiate
            a search for some videos.
          </p>
          <p>Let's take a look at a diagram that will give us a better idea how we will use the YouTube API:
          </p>
          <p>So every single time the user types into the input text field we are going to use a library called axios to
            make a Ajax request over to the YouTube API. In the Ajax request we will provide the search term we are
            looking for. The YouTube API will then execute a search on it's own server and then respond to us with a
            list of videos that match that search term. In order to make use of the YouTube API we first have to sign up
            to it and get an API key.
          </p>
          <p>Browse to <a target="_blank" href="console.developers.google.com">console.developers.google.com</a>, sign
            in if required, and create a new project (I did this by clicking the drop down next to the Google APIs logo
            and clicking the new project button).
          </p>
          <p>Take the default random name (My Project 32866 in my case) and click Create. Next click the Enable APIS and
            Services button. Search for the YouTube Data API v3 and open it up. Click the enable button.
          </p>
          <p>After the API is enabled we need to get an API key to allow us to access the API itself. Click on the
            ...
            browser, and that we only want to see public data.
          </p>
          <p>After you have entered all the relevant information click the "What credentials do I need?" button. At this
            point you should see the API key. Copy the key and return to your editor.
          </p>
          ...
          <p>You'll notice that I use capital letters with the underscore right here to declare the variable name.
            That's because this is constant variable and I never expect this to be changed.
          </p>
          <p>Next we will install the Axios library which is what we are going to use to make an Ajax request to the
            YouTube API. From a terminal in the video-browser directory issue the following command:
          </p>
          <figure>
          ...
          <p>In the last section we signed up for YouTube API key and installed axios into our project. In the terminal
            start the web application up again:
          </p>
          <figure>
          ...
          <p>We will add the following code to the onTermChange function to perform the YouTube search (You can find the
            YouTube API documentation by googling YouTube API):
          </p>
          <figure>
    ...
    onTermChange(searchTerm) {
        axios.get('https://googleapis.com/youtube/v3/search', {
          ...
          <p>We use the <code class="prettyprint">axios.get</code> method to call the YouTube API passing the Url as the
            first argument. A params object is passed as the the second object which specifies the following settings:
          </p>
          <ul>
          ...
          <p>We then chain a <code class="prettyprint">.then</code> to the axios request. This statement accepts a
            ...
            argument and then for now just write the response to the console.
          </p>
          ...
          <p>In the last section we got a nasty error message that complained with a status of 404 for this. This is
            ...
            should have been <a target="blank"
          ...
          <p>In the last section we were able to execute a search on the YouTube API for some given search term. We got
            a response that contained a list of videos that were found during that search.
          </p>
          <p>So we now need to take this list of videos and somehow display them on the screen.</p>
          <p>Create a new file in the components directory called VideoList.vue - remember we always make one separate
            Vue file for every component that we create. Add the following code:
          </p>
          <figure>
<pre class="prettyprint">&lt;template&gt;
  &lt;ul&gt;
...
&lt;/style&gt;</pre>
          ...
          <p>The <code class="prettyprint">script</code> tag contains <code class="prettyprint">export default</code>
            which contains a name property of VideoList.
          </p>
          <p>Before we go any further let's wire up the VideoList component to our App component to get it visible on
            the screen.
          </p>
          <p>In App.vue we will go through the three step process that we follow to show one component inside of
            another.
          </p>
          <ul>
          ...
          <p>Now if you save the file and go to the browser you should see the text VideoList in the browser window.
          </p>
          <p>In the next section we will look at how we can communicate our list of videos that was just retrieved from
            the YouTube API down to the VideoList component and render them.
          </p>
          ...
          <p>Our <code class="prettyprint">onTermChange</code> method is now making a request to the YouTube API to
            fetch a list of videos. We have also setup a VideoList component to actually display the list of videos.
          </p>
          <p>The next thing we need to do is communicate the list of videos that we fetch down from the parent App.vue
            to the child VideoList.vue component. VideoList.vue is the child because it is being display from within
            App.vue.
          </p>
          ...
          <p>This is a timeline - starting from the top and ending up at the bottom. So with our workflow for getting a
            list of videos everything start with that OnTermChange method being called.
          </p>
          <p>When that method is called we make a request to the YouTube API. The YouTube API will then respond with a
            list of objects that represents a list of videos.
          </p>
          ...
          <p>Remember that any time that we have some type of variable or information tied to a Vue instance or a
            component we store that information on the <code class="prettyprint">data</code> property. So we use <code
              class="prettyprint"> methods</code> to update our <code
              class="prettyprint">data</code>. Whenever we update our <code
              class="prettyprint">data</code> the component is automatically re-rendered. We can then optionally make
            use of that <code class="prettyprint">computed</code> property to somehow twist those values into something
            that can be displayed on the screen.
          </p>
          <p>So we're going to take that list of videos that are returned and we're going to assign it to a property on
            our <code class="prettyprint">data</code> object inside of the App component. Because we are updating the
            <code class="prettyprint">data</code> property we are going to cause the App component to automatically
            re-render it's template
          </p>
          <p>Now the important thing to realize here is that when we show one component inside of another, as we are
            ...
            new data that causes all of the child components to be re-rendered as well. So whenever we update <code
              class="prettyprint">data</code> in the <code
              class="prettyprint">App</code> component <code class="prettyprint">VideoList</code> will be updated at the
            same time - all of which happens automatically.
          </p>
          <p>So the plan is to take the list of videos and store it on data. When we do that causes everything to
            re-render. Then we can get our list of videos inside of VideoList and render them out in the template.
          </p>
          ...
          <p>In the last section we spoke about how we're going to take a our list of videos and store them on the <code
            class="prettyprint">data</code> property of our <code class="prettyprint">app</code> component. When we
            store the list of videos on the <code class="prettyprint">data</code> property it will cause the <code
              class="prettyprint">App</code> component to render and automatically render the <code class="prettyprint">VideoList</code>
            at the same time.
          </p>
          <p>The <code class="prettyprint">data</code> property is defined in slightly different way depending on
...
<pre class="prettyprint">data: function() {
    return {
        textInput: ''
          ...
          <p>Now, just for a moment, I want you to imagine that we break the rules and make an App component within
            which we define our data property as an object rather than as a function. When we define the data property
            as an object you can imagine that it essentially gets created inside our Computer's Memory:
          </p>
          <figure>
          ...
          <p>In the last section we spoke about how data inside of a component has to be defined as a function that
            ...
            property that will return an object that represents our <code
          ...
          <p>So we define the data property and use the shorter ES2015 syntax. Inside the data function we return an
            object which will initialize our data. This is going to be like the starting state or the initial
            ingredients for our component. So we have to start thinking about what different properties our data is
            going to contain.
          </p>
          <p>Well we definitely know that we need to have a list of videos because that's what we are retrieving from
            our onTermChange function - so let's add a videos property which should be an array of objects where every
            object represents one video:
          </p>
          <figure>
                    <pre class="prettyprint">data() {
    return { videos: [] };
    ...
    onTermChange(searchTerm) {
        axios.get('https://www.googleapis.com/youtube/v3/search', {
          ...
          <p>Remember the data property that is returned in the response is not at all related to the component instance
            - this is the data property that is tied to our response object that comes back from YouTube.
          </p>
          <p>Because videos is a data property when the above line is executed this will cause the entire template
            inside the App component to automatically re-render.
          </p>
          <p>So, as a quick test, let's add in a counter inside of our template that counts the number of videos that
            have been found on the YouTube API:
          </p>
          <figure>
...
&lt;/div&gt;</pre>
          ...
          <p>Usually we would use a computed function here but because this is just a temporary thing we won't bother a
            computed function. So we reference the videos data property and retrieve it's length.
          </p>
          <p>Remember that when we are referencing a data property or function that is tied to our component from within
            our template we do not have to prefix it with <code class="prettyprint">this</code> we can just write out
            the name of the property that we are trying to reference.
          </p>
          ...
          <p>In the last section we updated our <code class="prettyprint">data</code> property of <code
            ...
            a property that was defined on our <code class="prettyprint">data</code> object that cause our App component
            to automatically re-render.
          </p>
          <p>Now that we have that in place we want to communicate down the list of videos down to the VideoList
            component because the VideoList is responsible for rendering out information about each particular video on
            the screen.
          </p>
          <p>We've spoken a little bit about communicating from a parent to a child component previously - so instead of
            emitting events (that happens in the other direction) we pass props.
          </p>
          <p>This is a two step process. The first thing we're going to do is add a little bit of code to the parent
            component:
          </p>
          <figure>
          ...
          <p>The <code class="prettyprint">videos</code> property we specify above (to the left of the = operator) will
            ...
            myVideoArray). The value to the right specifies the name of the <code
              class="prettyprint">data</code> property (so here the name is important) that we want to share from the
            App component - the parent. In this case that's the <code class="prettyprint">videos</code> property.
          </p>
          <p>The purpose of <code class="prettyprint">v-bind</code> directive is to specify that anytime the <code
          ...
          <p>In the last section we added some configuration to our App component to take our list of videos and pass it
            down to the VideoList.
          </p>
          <p>Step two of passing props involves adding some configuration to our child component to tell it about what
            data it should expect to receive from it's parent.
          </p>
  ...
  props: ['videos']
          ...
          <p>We assign an array to the <code class="prettyprint">props</code> property that contains a number of strings
            which match the exact property names that the component should expect to receive from the parent - videos in
            our case.
          </p>
          <p>So defining our <code class="prettyprint">props</code> list can be as simple as listing out the names of
            all the different properties that we expect to receive. We can also add in some amount of configuration or
            validation to make sure that the child component is receiving the correct typeo of property as well.
          </p>
          <p>So we might want to validate to make sure that the parent is passing down an array, or a string, or an
            object. This is optional, but recommended. So instead of an array of strings we can pass in an object where
            the keys are the name of the properties we expect to receive and the values are the type of the property
            that we expect.
          </p>
          <p>In the case of the videos object we expect it to be an array of objects so we can write the following:
          </p>
          <figure>
                    <pre class="prettyprint">export default {
  name: 'VideoList',
  props: {
      videos: Array
          ...
          <p>This component will then expect to receive a property called videos that should be an array. If an array is
            not passed then the component will throw an error
          </p>
          <p>So, now that we completed step number 2, we can make use of the prop that has been provided anywhere inside
            of our component. So we could use it in a computed function, or methods, or we could access it directly
            inside our template. We'll do the last one now.
          </p>
          <p>So, inside the template, I'm going to try to print out the number of videos that we have been passed again:
          </p>
          <figure>
...
&lt;/template&gt;</pre>
          ...
          <p>This syntax is identical to the earlier example when we accessed the <code
            class="prettyprint">videos</code> data property in the <code class="prettyprint">App</code> component.
          </p>
          <p>We do the exact same thing when we are accessing a prop as well - the common theme being we just use the
            name.
          </p>
          ...
          <p>Now that we have got some props we can start to add some logic to the VideoList component to render those
            out as a list.
          </p>
          ...
          <p>Our VideoList component now knows about the list of videos that it needs to render to the screen. As a
            quick reminder remember that the entire list itself is going to be comprised of two separate components.
          </p>
          <p>The first is the VideoList - which is what we are working on now. The purpose of VideoList is to take the
            list of videos that we pass down to it and then render one VideoListItem component for each video that it
            receives.
          </p>
          <p>Create a new file called VideoListItem.vue inside the components directory. Add the following boilerplate
            code:
          </p>
          <figure>
<pre class="prettyprint">&lt;template&gt;
...
&lt;/template&gt;</pre>
          ...
          <p>We have now created the VideoListItem component. Let's now import it into the VideoList and wire it up as a
...
<pre class="prettyprint">&lt;script&gt;
      import VideoListItem from './VideoListItem';</pre>
          ...
          <p>Remember that anytime we want to the parent component that it has access to the child we have to add the
            components property to that parent component itself. To do this add the following code inside of the
            VideoList configuration object:
          </p>
          <figure>
<pre class="prettyprint">export default {
  name: 'VideoList',
  ...
  props: {
      videos: Array
          ...
          <p>So now inside of our VideoList component we access to the VideoListItem component. Now we need to figure
            out how to create one VideoListItem component for every video that is contained within our videos property.
          </p>
          <figure>
...
<pre class="prettyprint">&lt;template&gt;
  ...
  &lt;/template&gt;</pre>
          ...
          <p>You are seeing this output because, at present, the VideoListItem component is simply outputting an <code
            class="prettyprint">li</code> element containing the text VideoListItem for each <code
          ...
          <p>We now need figure out some way to say that for every video in this list of videos we want to communicate
            this video in the <code class="prettyprint">v-for</code> directive down to the <code
              class="prettyprint">VideoListItem</code> component so the component instance knows what video it is
            supposed to be displaying information about on the screen.
          </p>
...
<pre class="prettyprint">&lt;template&gt;
  ...
  &lt;/template&gt;</pre>
          ...
          <p>So we write <code class="prettyprint">v-bind:</code> and then the name of the prop: <code
            ...
            VideoListComponent that needs to be passed down to the child. <code
              class="prettyprint">video</code> is referencing the current video that we are iterating over inside of our
            list of videos.
          </p>
...
&lt;/template&gt;</pre>
          ...
          <p>We get the name of the video from the <code v-pre class="prettyprint">{{ video.snippet.title }} </code>.
            The video object has a <code class="prettyprint">snippet</code> property which, in turn, contains <code
              class="prettyprint">title, description</code> and <code
...
&lt;/template&gt;</pre>
          ...
          <p>It's worth pointing out that we don't try and add styles directly to the <code
            class="prettyprint">&lt;VideoListItem&gt;</code> tag in the VideoList component. Instead we open up the
            component itself and add the styles to the <code class="prettyprint"> li</code> element located within.
          </p>
...
&lt;/template&gt;</pre>
          ...
          <p>The <code class="prettyprint">this</code> keyword in the code above is present because we are referencing a
            <code class="prettyprint">prop</code> called <code
...
&lt;/template&gt;</pre>
          ...
          <p>Remember, to add an event handler we write out <code class="prettyprint">@</code> and then the name of the
            event that we want to listen for - in the case <code
              class="prettyprint">videoSelect </code> then after the <code
              class="prettyprint">=</code> assignment operator we specify what we want to do when the event is
            ...
            different function in it's own <code
    ...
    onTermChange(searchTerm) {
        axios.get('https://www.googleapis.com/youtube/v3/search', {
          ...
          <p>So, just for the moment, we are writing the video object to the console. The <code
            ...
            object because in the <code class="prettyprint"> VideoList</code> component when we emitted our <code
          ...
          <p>Back in the browser, refresh, select a video and then check the console. As the video object gets written
            to the console you should see an observer object being written out that contains <code
...
&lt;/template&gt;</pre>
          ...
          <p>So that's the idea, we're going to make use of the data system of the <code
            class="prettyprint">App</code> component to communicate the newly clicked video over to the <code
...
<pre class="prettyprint">onVideoSelect(video) {
    this.selectedVideo = video;
          ...
          <p>When our application first renders our <code class="prettyprint">App</code> component has a data property
            for <code class="prettyprint">selectedVideo</code> which is set to null:</p>
          <figure>
<pre class="prettyprint">        data() {
    return { videos: [], selectedVideo: null };
},</pre>
          ...
          <p>We take the <code class="prettyprint">selectedVideo</code> property and we pass it down to the <code
            class="prettyprint">VideoDetail</code> component:</p>
          <figure>
            <pre class="prettyprint">&lt;VideoDetail :video=&quot;selectedVideo&quot; /&gt;</pre>
            <figcaption>Fig 03-107</figcaption>
          </figure>
          <p>Inside <code class="prettyprint">VideoDetail</code> our template tries to read the <code
            class="prettyprint">snippet </code>property of the <code class="prettyprint">video</code> prop:</p>
          <figure>
<pre v-pre class="prettyprint">&lt;template&gt;
    &lt;div&gt;
        {{ video.snippet.title }}
...
&lt;/template&gt;</pre>
          ...
          <p>Add some code to make sure that if the <code class="prettyprint">video</code> prop is null we do not
            attempt to read the <code class="prettyprint">snippet</code> property on it: </p>
          <figure>
<pre v-pre class="prettyprint">&lt;template&gt;
    &lt;div v-if=&quot;video&quot;&gt;
        {{ video.snippet.title }}
...
&lt;/template&gt;</pre>
          ...
          <p>Because Vue doesn't render the <code class="prettyprint">div</code> it also skips the line of code
            contained within - and therefore the error is not thrown.</p>
          ...
          <p>You've now communicated the selected video to the VideoDetail component. We're now ready to get the video
            ...
            user has clicked on.</p>
          <p>We will start with the video title and description. Add the following code to the <code
            class="prettyprint">template</code> of the <code class="prettyprint">VideoDetail</code> component.</p>
          <figure>
<pre v-pre class="prettyprint">&lt;template&gt;
    &lt;div v-if=&quot;video&quot;&gt;
        &lt;div class=&quot;details&quot;&gt;
...
&lt;/style&gt;</pre>
          ...
          <p>We've got a lot of information about the video now visible on the screen but we need to make sure the
            actual video player displays as well. </p>
          ...
          <p>When you embed a YouTube video you can add it directly to another web application. Add the following code
            above the <code class="prettyprint">div</code> with a class of <code
    ...
    videoUrl() {
        ...
        return `https://www.youtube.com/embed/${videoId}`;
          ...
          <p>Because the <code class="prettyprint">videoId</code> <code class="prettyprint">const</code> that we used
            ...
            destructuring assignment syntax and surround our <code
          ...
          <p>We've now got our YouTube embed visible on the screen but it's a little bit small. We will use the
            ...
            it. The documentation provides an example:</p>
          <figure>
...
&lt;/div&gt;</pre>
          ...
          <p>Back in the browser you should see the VideoDetail now displays a larger video window which will adjust as
            we resize the browser window.</p>
          ...
          <p>Our video <code class="prettyprint">iframe</code> is now plainly visible on the screen but it's still
            stacked on top of our <code class="prettyprint">VideoList</code>.</p>
          ...
          <p>To accomplish this we will apply some layout CSS provided by Bootstrap. In the App.vue file add a div that
            wraps both the VideoDetail and VideoList tags:</p>
          <figure>
...
&lt;/div&gt;</pre>
          ...
          <p>If you viewed this in the browser now you would still see the VideoDetail and the VideoList being displayed
            ...
            (width wise) as it possibly can.</p>
          ...
          <p>In the <code class="prettyprint">VideoList</code> component add the following class to the root <code
            class="prettyprint">ul</code> element:</p>
          <figure>
          ...
          <p>Our application is complete but there are a couple of items we should review that are important to keep in
            mind when you work on future Vue applications. </p>
          <p>The first thing is how we communicate information between different components that we create. Anytime that
            we want to communicate information between components it's always between parent and child:</p>
          <figure>
          ...
          <p>In our application we had the <code class="prettyprint">VideoDetail</code>, the <code
            class="prettyprint">SearchBar</code> and the <code class="prettyprint">VideoList</code> as sibling
            components - in other words they were all being displayed by the <code class="prettyprint">App</code>
            component.</p>
          ...
          <p>Instead we choose to communicate directly from, for example, the <code
            ...
            <code class="prettyprint">App</code> down to the <code class="prettyprint">VideoDetail</code>. </p>
          ...
          <p>So if you want to go from a parent down to a child we refer to that as passing props (short for
            properties). This is a two step process:</p>
          <figure>
          ...
          <p>We make sure that the parent components tag for showing that component has a <code
            class="prettyprint">v-bind</code> directive. This makes sure that the parent component is going to send some
            information down to the child. Likewise the child also has to be told about the props that it should expect
            to receive from the parent.</p>
