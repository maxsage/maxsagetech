    ...
    <div class="panel-group">
      <div class="panel panel-primary">
        ...
        <div class="panel-body">
          ...
          <code class="language-javascript">import Vue from 'vue';</code>
          <p>As a quick reminder of how <code class="language-javascript">import</code> statements work webpack is going
            to interpret this line of code when it processes all of our code. It's going to see that we're trying to
            import something called Vue. Now we don't currently have any files inside of our <span
              class="filename">src</span> directory called Vue so instead webpack is going to automatically look into
          ...
          <p>So when we write out <code class="language-javascript">import Vue from 'vue';</code> webpack is going to
            look into that folder and pull in some amount of Javascript code to support our project.
          </p>
          <p>Now the next thing we're going to do is to create a new file called <span class="filename">App.vue</span>.
            This is going to function very similarly to the <span class="filename">App.vue</span> file that we just
            deleted. Inside that file you and I are going to make a single Vue component which is going to serve as,
            kind of, the brains of our application.
          </p>
          ...
          <p>At the very top we're going to place that <code class="language-html">&lt;template&gt;</code> tag which
...
&lt;/template&gt;</code></pre>
          ...
          <p>Inside the <code class="language-html">&lt;template&gt;</code> tag itself we are only allowed to have one
...
&lt;/template&gt;</code></pre>
          ...
          <p>Let's add some content to the <code class="language-html">&lt;template&gt;</code> tag:</p>
          <figure>
<pre><code class="language-html">&lt;template&gt;
    &lt;div&gt;
        Hi there!
...
&lt;/template&gt;</code></pre>
          ...
          <p>Underneath the <code class="language-html">&lt;template&gt;</code> tag we will add a <code
            class="language-html">&lt;script&gt;</code> tag:
          </p>
          <figure>
...
&lt;/script&gt;</code></pre>
          ...
          <p>Inside of the <code class="language-html">&lt;script&gt;</code> tag add the following code:</p>
          <figure>
<pre><code class="language-html">&lt;script&gt;
    export default {
    }
&lt;/script&gt;</code></pre>
          ...
          <p>Now we haven't really spoken a lot about this <code class="language-javascript">export default</code>
            syntax so far
            even though we saw it in the previous implementation of the <span class="filename">App.vue</span> file. In
            the next section we'll talk about what the purpose of this <code class="language-javascript">export
              default</code>
            syntax is.
          </p>
          ...
          <p>We finished off the last section working on the <code class="language-html">&lt;script&gt;</code> tag
            right here
            where
            we wrote <code class="language-javascript">export default</code> then placed a set of curly braces:
          </p>
          <figure>
<pre><code class="language-html">&lt;script&gt;
    export default {
...
&lt;/script&gt;</code></pre>
          ...
          <p>Recall we also added the template directly into the Javascript as well. So the object that we passed to
            <code class="language-javascript">new Vue</code> had some number of properties that customized the instance.
            The same exact thing is true of this object that you pass into <code class="language-javascript">export
              default</code>. So this object is going to contain a couple of different properties that customize this
            Vue component and how it behaves.</p>
          <p>So, for example, we might eventually have a <code class="language-javascript">data</code> property on here,
            some <code class="language-javascript">computed</code> functions and eventually have some different <code
              class="class=language-javascript">methods</code> tied to it as well:
          </p>
          <figure>
<pre><code class="language-html">&lt;script&gt;
export default {
  ...
  methods: {}
          ...
          <p>Right now, there is only one property that we are going to assign inside of this object - a property we did
            not discuss previously when we were working back inside of CodePen - <code
              class="language-javascript">name</code>:
          </p>
          <figure>
<pre><code class="language-html">&lt;script&gt;
    export default {
...
&lt;/script&gt;</code></pre>
          ...
          <p>The <code class="language-javascript">name</code> property is not strictly required for Vue to work
            properly. We put in the <code class="language-javascript">name</code> property just to make our lives, as
            developers, a little bit easier. You see, there's a couple of debugging tools out there that you can
            download, free of charge, that help you debug Vue applications inside the browser. Many of these debug tools
            that we use with Vue look for different components that have specific names tied to them. So simply
            providing the name property is going to make those debugging tools work a little bit better by allowing you
            the developer to more easily identify different components that are in use in your application.
          </p>
          <p>So that's really all we need for right now to get this component started. We don't have to provide anything
            like the <code class="language-javascript">data</code>, <code class="language-javascript">methods</code> or
            <code
              class="language-javascript">computed</code> properties in order to get some content on the screen. We're
            only going to start to add those extra properties once our component actually needs their behaviour.
          </p>
          <p>One other thing I'll mention here is you'll recall that back in our Vue instance, that we created over in
            CodePen, we provided the <code class="language-javascript">el</code> argument:
          </p>
          <figure>
<pre><code class="language-javascript">new Vue({
    ...
    data: { // Initialize our list of 'ingredients'
        textInput: ''
          ...
          <p>The <code class="language-javascript">el</code> argument is short for element and it specifies to Vue where
            our Vue template is located in the DOM.
          </p>
          <p>In the case of the Vue component that we're putting together right now Vue is going to automatically
            understand that our template is created in the same file. We don't need to specify any <code
              class="language-javascript">el</code> property to tell this component where it's template is or where it
          ...
          <figure>
            <pre><code class="language-javascript">import App from './App';</code></pre>
            <figcaption>Fig 03-013</figcaption>
          </figure>
          <p>Just a quick reminder on <code class="language-javascript">import</code> statements - if we are importing a
            file that we have created then we have to provide the relative path to that file. So <code
              class="language-javascript">./</code> in the <code class="language-javascript">import</code> statement
            above means in the same folder look for a file called <span class="filename">App</span>. I think webpack
            allows you to include or omit the file extension.
          </p>
...
<pre><code class="language-javascript">new Vue({
          ...
          <p>We write out <code class="language-javascript">new Vue();</code> and then pass in an object with some
            number of configuration options. Now the first option that we're going to put in here is going to look a bit
            strange:
          </p>
          <figure>
<pre><code class="language-javascript">new Vue({
   render: function(createElement) {
       return createElement(App);
          ...
          <p>At this point, we have seen more than one way to specify the <code class="language-html">template</code>
            for a give Vue instance. We've seen how we can put our <code class="language-html">template</code> in the
            Html. We've also seen how we can attach it directly to our Vue instance. What you're seeing in Fig 03-015 is
            another way, yes another way, to tell a Vue instance how to show some content on the screen.
          </p>
          <p>We define a <code class="language-javascript">render</code> function that gets called with another function
            called <code class="language-javascript">createElement</code>. We call the <code
              class="language-javascript">createElement</code> function and we pass in the component that we just
            created and then we return the result of that function call.
          </p>
          <p>You can kind of imagine that this takes the App component and turns it into a Vue instance which can then
            ...
            going on with the <code class="language-javascript">render</code> function.
          </p>
          <p>We are going perform one quick refactor that will make this look more similar to what you'll see in real
            applications. Usually we do not write out the entire function name of <code class="language-javascript">createElement</code>:
          </p>
          <figure>
<pre><code class="language-javascript">function(createElement) {
    return createElement(App);
          ...
          <p>Instead we usually abbreviate the <code class="language-javascript">createElement</code> function name as
            simply <code class="language-javascript">h</code>:
          </p>
          <figure>
<pre><code class="language-javascript">new Vue({
   render: function(h) {
       return h(App);
          ...
          <p>Don't ask me why we use <code class="language-javascript">h</code> I honestly don't know why we came up
            with that convention. The other key factor that you'll see very commonly around this <code
              class="language-javascript">render</code> function is not to use the entire <code
              class="language-javascript">function</code> keyword but instead use an ES 2015 arrow function:
          </p>
          <figure>
<pre><code class="language-javascript">new Vue({
   render: h =&gt; h(App)
          ...
          <p>Now, anytime this file is executed, we will try to create a new Vue instance. When that Vue instance is
            created the <code class="language-javascript">render</code> function will be called with the <code
              class="language-javascript">createElement</code> function. We then pass in our App component to <code
              class="language-javascript">createElement</code> and the entire result is return from the <code
              class="language-javascript">render</code> function.
          </p>
          ...
          <p>In the last section we started working on our <span class="filename">main.js</span> file. We've now created
            ...
            eventually render to our actual DOM or try to get it to show up inside the browser - the <code
              class="language-javascript">render</code> function will be called. We are telling the <code
              class="language-javascript"> render</code> function that we want to display an instance of our App
            component:
          </p>
          <figure>
<pre><code class="language-javascript">new Vue({
   render: h =&gt; h(App)
          ...
          <p>Now the very last thing we need to do inside of here is to make sure that we actually try to take this
            ...
            CodePen identicon project we used the <code class="language-javascript">el</code> property. The <code
              class="language-">el</code> property not only tells Vue where it can find it's template but it also
            tells Vue where it should place the rendered Html inside of our Html structure:
          </p>
          <figure>
            <pre><code class="language-javascript">el: '#app',</code></pre>
            <figcaption>Fig 03-020</figcaption>
          </figure>
          <p>In the code snippet above <code class="language-javascript">#app</code> means go and find some element
            inside of our DOM with an <code class="language-html">id</code> of <code class="language-html">app</code>.
            The same thing has to be provided to our Vue instance in our <span class="filename">main.js</span> file as
            well. If we ran this code right now we would create our application but it would not show up on the screen
            because we have not yet told Vue where to put this Html.
          </p>
          <p>You'll recall that inside the <span class="filename">public</span> directory we've got that <span
            class="filename">index.html</span> file and inside there is a <code class="language-html">div</code> with an
            <code class="language-html">id</code> of <code class="language-html">app</code>. When we generated Vue CLI
            we already got a place created right out of the box for us to render our application to:
          </p>
          <figure>
          ...
          <p>To get our <span class="filename">main.js</span> file's Vue instance to actually show up inside the browser
            we need to provide an <code class="language-html">el</code> property:
          </p>
          <figure>
<pre><code class="language-javascript">new Vue({
    el: '#app',
    render: h =&gt; h(App)
          ...
          <p>One thing I want to quickly mention here is back when we first generated our Vue CLI project, like just a
            ...
            not have an object or a property in here called <code class="language-javascript">el</code>. When we first
            generated this project <span class="filename">main.js</span> had a different way of specifying the element
            that we wanted to render our application to. It created the new Vue instance and then it called a function
            on that instance:
          </p>
          <figure>
<pre><code class="language-javascript">new Vue({
    render: h =&gt; h(App)
          ...
          <p>The line of code, containing the <code class="language-javascript">mount</code> chained function, tells Vue
            where it should render itself to.
          </p>
          <p>In the Vue JS world there's several different ways of doing any given operation. We have already seen many
            different ways in which we can define templates that are tied to our application. Calling <code
              class="language-javascript">#mount</code> and then passing in <code class="language-javascript">'#app
              '</code> is identical to writing out <code class="language-javascript">el: '#app'</code>, It's just two
            different ways of doing the exact same operation. This is a pattern that you are going to notice in the Vue
            world over and over again - there's more than one way to do just about everything.
          </p>
          <p>How do you decide which approach to use? It's pretty much down to personal preference but just for the sake
            of convention, and sticking with what Vue CLI does by default, we'll stay with the <code
              class="language-javascript">#mount</code> option rather than using <code
              class="language-javascript">el </code>
          </p>
          ...
          <p>Flip back over to our code editor where we're going to create a new Vue file to contain the SearchBar
            ...
            boilerplate tags that are going to become very familiar over time. We'll begin by placing a <code
              class="language-html">template</code> tag and then underneath that we'll add a <code
              class="language-html">script</code> tag. Finally we'll add a <code class="language-html">style</code> tag
            which will eventually contain some CSS that is solely related to this one component and no other component
...
&lt;/style&gt;</code></pre>
          ...
          <p></p>
          <p>Back up inside the <code class="language-html">template</code> tag we'll immediately create an <code
            class="language-html">input</code> element and we'll wrap it with a <code class="language-html">div</code>
            tag.
          </p>
          <figure>
<pre><code class="language-html">&lt;template&gt;
...
&lt;/template&gt;</code></pre>
          ...
          <p>Now you might be a bit curious why we were wrapping this with a <code class="language-html">div</code> tag.
            Honestly that's really just for styling. By default a <code class="language-html">div</code> has a CSS
            display property ot <code class="language-html">block</code> which will make sure that this input element
            appears in the browser on it's own individual line. The <code class="language-html">div</code> here is
            really just kind of for layout purposes not necessarily because this component actually requires it from any
            technical standpoint or functionality standpoint. Add the following code to the <code
              class="language-html">script</code> tag:
          </p>
          <figure>
<pre><code class="language-html">&lt;script&gt;
...
&lt;/script&gt;</code></pre>
          ...
          <p>We just put together our SearchBar component and now we're ready to start adding a couple of options. Let's
            start by adding the <code class="language-javascript">name</code> parameter which we spoke about a couple of
            sections back . So I'm going to provide a name of simply <code class="language-javascript">SearchBar</code>
            like so:
          </p>
          <figure>
<pre><code class="language-html">&lt;script&gt;
...
&lt;/script&gt;</code></pre>
          ...
          <p>Now before we go any further I think that we should try to get the SearchBar component to be displayed
            ...
            the <code class="language-html">script</code> tag. First we <code class="language-javascript">import</code>
            the component:
          </p>
          <figure>
            <pre><code class="language-javascript">import SearchBar from './components/SearchBar.vue';</code></pre>
          ...
          <p>To get the SearchBar component to actually render inside the App component we add a <code
            class="language-html">SearchBar</code> tag inside our template - replacing the "Hi there!" placeholder text
...
&lt;/template&gt;</code></pre>
          ...
          <figure>
<pre>vue.runtime.esm.js?2b0e:587 [Vue warn]: Unknown custom element:
  &lt;SearchBar&gt; - did you register the component correctly? For recursive components, make sure to provide the &quot;name&quot; option.
  found in ---&gt; &lt;App&gt; at src\App.vue &lt;Root&gt;</pre>
          ...
          <p>Whenever you want to nest a component like this or show one component inside of another it's not quite
            enough just to <code class="language-javascript">import</code> that component and then place it into the
            <code
              class="language-html">template</code>. There's actually one other step that we have to go through. I
          ...
          <p>In the <span class="filename">App.vue</span> file, we imported the SearchBar (step 1) and we used it inside
            the template (step 2). Step number 3 involves adding a <code class="language-javascript">components</code>
            property. This is going to be an object that lists all the different components that are going to be used
            inside of the app's <code class="language-html">template</code>:
          </p>
          <figure>
...
<pre><code class="language-javascript">export default {
    ...
    components: {
        SearchBar: SearchBar
          ...
          <p>This let's our App component know that it might see a <code class="language-html">SearchBar</code> tag
            inside of it's <code class="language-html">template</code>. We can use a little bit of ES 2015 syntax to
            shorten this up because the key and the value are identical:
          </p>
          <figure>
<pre><code class="language-javascript">export default {
    name: 'App',
    components: {
        SearchBar
          ...
          <p>In summary, to show one component inside of another we add the <code
            class="language-javascript">import</code> statement, we use a <code class="language-html">SearchBar</code>
            tag inside of the <code
              class="language-html">template</code> and we declare a <code class="language-javascript">components</code>
            property that specifies a component called <code class="language-javascript">SearchBar</code>.
          </p>
          ...
          <p>Our goal now is to make sure that anytime the user types inside the input text field we trigger some type
            ...
            CodePen exercises. You'll recall that we added a directive to the <code class="language-html">input</code>
            tag in the <code class="language-html">template</code>. We will also add a <code
              class="language-javascript">method</code> object to our component definition.
          </p>
          <p>Let's begin by adding in the <code class="language-javascript">methods</code> object to our component
            definition:
          </p>
          <figure>
<pre><code class="language-html">&lt;script&gt;
...
&lt;/script&gt;</code></pre>
          ...
          <p>Remember methods give us the ability to change data inside of our application. So I'm going to make a
            method called <code class="language-javascript">onInput</code> that will be called with an event which will
            contain the text that the user just entered into the input. For the moment we will just write this out to
            the console:
          </p>
          <figure>
<pre><code class="language-html">&lt;script&gt;
...
&lt;/script&gt;</code></pre>
          ...
          <p>We will also wire that method up to our <code class="language-html">input</code> element in the <code
            class="language-html">template</code>:
          </p>
          <figure>
<pre><code class="language-html">&lt;template&gt;
    &lt;div&gt;
...
&lt;/template&gt;</code></pre>
          ...
          <p>In the last section we wired up the <code class="language-javascript">onInput</code> directive to our <code
            class="language-html">input</code> tag. Before we move on to talking about how we're going to use this
            information to actually do a search on the YouTube API there is one little refactor I want to do to the
            <code class="language-html">v-on</code> directive we used on the <code class="language-html">input</code>
            tag.
          </p>
          ...
          <figure>
            <code class="language-html">&lt;input @input=&quot;onInput&quot; /&gt;</code>
            <figcaption>Fig 03-039</figcaption>
          </figure>
          <p>We have replaced the <code class="language-html">v-on:input</code> directive with a more concise syntax
            <code
              class="language-html">@input</code>. This can be read as anytime someone triggers an input event on this
            element run the <code class="language-html">onInput</code> function.
          </p>
          ...
          <p>The SearchBar component is now aware of what a user is typing into your application thanks to the <code
            class="language-html">onInput</code> method - we can get a search term to use with the YouTube API. But
            where inside of our application should we locate the code to actually execute that search? Going back to our
          ...
          <p>In the last section we spoke about how a parent component can communicate to the child by passing props and
            ...
            component to make sure that it emits an event anytime someone changes that <code
              class="language-html">input</code> element. In <span class="filename">SearchBar.vue</span> every single
            time that the <code class="language-html">onInput</code> function is called we are going to want to emit an
            event to inform that App component that there is a new term to search the YouTube API with.
          </p>
          <p>First, replace the <code class="language-javascript">console.log</code> statement with a call the <code
            class="language-javascript">$emit</code> function:
          </p>
          <figure>
    ...
    onInput: function(event) {
        this.$emit();
          ...
          <p>The first argument to this function is the name of the event we want to emit. In this case I'll use a name
            of <code class="language-javascript">termChange</code> to indicate that our search term just changed:
          </p>
          <figure>
<pre><code class="language-javascript">methods: {
    onInput: function(event) {
        this.$emit('termChange');
          ...
          <p>As an aside I want to mention that when we were listening for an <code class="language-html">input</code>
            event on the input text field in the <code class="language-html">template</code> - input is a special event
            ...
            <code class="language-javascript">termChange</code> because one convention is to use the name of what is
            about to occur and then a verb that indicates what just changed.
          </p>
          <p>The second argument can be used to provide some additional information about the event that just occurred.
            ...
            available on <code class="language-javascript">event.target .value</code>:
          </p>
          <figure>
<pre><code class="language-javascript">methods: {
    onInput: function(event) {
        this.$emit('termChange', event.target.value);
          ...
          <p>The last thing I want to mention here is the <code class="language-javascript">$</code> sign in the <code
            class="language-javascript">emit</code> function. There's nothing special about using the dollar sign with a
            function name - it is valid Javascript. So the creators of the Vue library could just have easily have
            called the function <code class="language-javascript">emit</code> and left out the dollar sign. The reason
            they added the dollar sign in there is just in case you ever decided to add in some additional property to
            your component called emit as well.
          </p>
          ...
          <p>Our SearchBar component is now emitting an event anytime someone enters some new text. We're now going to
            open our App component and make sure that anytime the SearchBar emits the <code class="language-javascript">
              termChange</code> event the App has the ability to run some code that will do a search on the YouTube API.
            Add the following code to the <code class="language-javascript">SearchBar</code> tag that we added to <span
              class="filename">App.vue</span>:
          </p>
          <figure>
            <pre><code class="language-html">&lt;SearchBar v-on:termChange=&quot;onTermChange&quot;&gt;&lt;/SearchBar&gt;</code></pre>
            <figcaption>Fig 03-044</figcaption>
          </figure>
          <p>We specify a method name of <code class="language-html">onTermChange</code> to run anytime the <code
            class="language-html">termChange</code> event is triggered. Inside the component definition we'll add
            another key value pair of <code class="language-javascript">methods</code> and define a function of <code
              class="language-html">onTermChange</code>
          </p>
          <figure>
<pre><code class="language-javascript">export default {
    name: 'App',
    ...
    methods: {
        onTermChange: function() {
          ...
          <p>One quick thing I want to mention about these function declarations that we've been writing is that we've
            been using the <code class="language-javascript">function</code> keyword (see Fig 03-045 above). If we
            wanted to we could use a little bit of ES2015 syntax to shorten this to:
          </p>
          <figure>
<pre><code class="language-javascript">export default {
    name: 'App',
    ...
    methods: {
        onTermChange() {
          ...
          <p>Ok so, once again, anytime the <code class="language-javascript">v-on:termChange</code> event is triggered
            we will run the <code class="language-html">onTermChange</code> function.
          </p>
          <p>You'll remember that back inside of SearchBar whenever we emit <code
            class="language-html">termChange</code> we also pass along the new search term that the user just entered by
            passing <code
              class="language-javascript">event.target.value</code> as the second argument. Because we add <code
              class="language-javascript">event.target.value</code> as the second argument it will show up as the first
            argument to our callback function or event handler:
          </p>
          <figure>
            <code class="language-javascript">methods: { onTermChange(searchTerm) {
          ...
          <figure>
            <code class="language-javascript">methods: { onTermChange(searchTerm) { console.log(searchTerm); } }</code>
          ...
          <p>Now before we go test this out I want to draw your attention to the listener we added to the <code
            class="language-html">SearchBar</code> tag:
          </p>
          <figure>
<pre><code class="language-html">&lt;SearchBar v-on:termChange=&quot;onTermChange&quot;&gt;&lt;
  /SearchBar&gt;</code></pre>
            <figcaption>Fig 03-049</figcaption>
          </figure>
          <p>The syntax is almost identical to the syntax we used to listen to an <code
            class="language-html">input </code> event on an <code class="language-html">input</code> element earlier.
            So, as before, we can shorten the syntax:
          </p>
          <figure>
            <pre><code
              class="language-html">&lt;SearchBar @termChange=&quot;onTermChange&quot;&gt;&lt;/SearchBar&gt;</code></pre>
            <figcaption>Fig 03-041</figcaption>
          </figure>
          <p>In the browser refresh the page and if you type some text you should see it logged in the console as
            before. The difference this time round is the console log is being called in the App component's <code
              class="language-javascript">onTermChange</code> method. This confirms that the SearchBar component is
          ...
          <p>Our App component is now aware of anytime the user enters in a new search term. We will locate a bit of
            code in the <span class="filename">App.vue</span> file's <code
              class="language-javascript">onTermChange </code> function to initiate a search for some videos. Let's take
            a look at a diagram that will give us a better idea how we will use the YouTube API:
          </p>
          <figure>
          ...
          <p>Inside the <span class="filename">App.vue</span> file directly underneath our <code
            class="language-javascript">import</code> statement add the following code:
          </p>
          <figure>
            <pre><code class="language-javascript">const API_KEY = 'your key here';</code></pre>
            <figcaption>Fig 03-042</figcaption>
          </figure>
          ...
          <p>We will add the following code to the <code class="language-javascript">onTermChange</code> function to
            perform the YouTube search (You can find the YouTube API documentation by Googling YouTube API):
          </p>
          <figure>
<pre><code v-pre class="language-javascript">methods: {
    onTermChange(searchTerm) {
        axios.get('https://googleapis.com/youtube/v3/search', {
          ...
          <p>We use the <code class="language-javascript">axios.get</code> method to call the YouTube API passing the
            Url as the first argument. A <code class="language-javascript">params</code> object is passed as the the
            second object which specifies the following settings:
          </p>
          <ul>
          ...
          <p>We then chain a <code class="language-javascript">.then</code> to the axios request. This statement accepts
            a function that gets called with the response that we get back from YouTube. So we can accept that as an
          ...
          <p>In the last section we got a nasty error message that complained with a status of 404 for this. This is
            because of a typo in the Url that we specified to the <code class="language-javascript">axios.get</code>
            method. It should have been <a target="blank" href="https://www.googleapis.com/youtube/v3/search">https
...
&lt;/style&gt;</code></pre>
          ...
          <p>The unordered list will be used to render the list of videos - for the moment we have just added some
            placecholder text. The <code class="language-javascript">script</code> tag contains <code
              class="language-javascript">export default</code> which contains a <code
              class="language-javascript">name</code> property of VideoList.
          </p>
          ...
          <ul>
            <li>Add the <code class="language-javascript">import</code> statement - <code
              class="language-javascript">import VideoList from './components/VideoList.vue';</code>
            </li>
            <li>Add a tag for VideoList in the template - <code
              class="language-html">&lt;VideoList&gt;&lt;/VideoList&gt;</code>
            </li>
            <li>Add it to the <code class="language-javascript">components</code> property on the component
              configuration - <code class="language-javascript">VideoList</code></li>
          </ul>
          ...
          <p>Our <code class="language-javascript">onTermChange</code> method is now making a request to the YouTube API
            to fetch a list of videos. We have also setup a VideoList component to actually display the list of videos.
            The next thing we need to do is communicate the list of videos that we fetch down from the parent App
            component to the child VideoList component. VideoList is the child because it is being display from within
            the App component.
          </p>
          ...
          <p>This is a timeline - starting from the top and ending up at the bottom. So with our workflow for getting a
            list of videos everything starts with the <code class="language-javascript">onTermChange</code> method being
            called. When that method is called we make a request to the YouTube API which responds with a list of
            objects that represents a list of videos. The next step is where things get interesting.
          </p>
          <p>Remember that any time that we have some type of variable or information tied to a Vue instance or a
            component we store that information on the <code class="language-javascript">data</code> property. So we use
            <code
              class="language-javascript">methods</code> to update our <code class="language-javascript">data</code>.
            Whenever we update our <code class="language-javascript">data</code> the component is automatically
            re-rendered. We can then optionally make use of that <code class="language-javascript">computed</code>
            property to somehow twist those values into something that can be displayed on the screen. We're going to
            take that list of videos that are returned and we're going to assign it to a property on our <code
              class="language-javascript">data</code> object inside of the App component. Because we are updating the
            <code class="language-javascript">data</code> property we are going to cause the App component to
            automatically re-render it's template
          </p>
          <p>Now the important thing to realize here is that when we show one component inside of another, as we are
            doing here with the <code class="language-html">VideoList</code> inside the <code
              class="language-html">App</code> component, whenever the parent component re-renders it's template with
            some new data that causes all of the child components to be re-rendered as well. So whenever we update <code
              class="language-javascript">data</code> in the <code class="language-javascript">App</code> component
            <code
              class="language-javascript">VideoList</code> will be updated at the same time - all of which happens
            automatically.
          </p>
          <p>So the plan is to take the list of videos and store it on <code class="language-javascript">data</code> .
            When we do that causes everything to re-render. Then we can get our list of videos inside of VideoList and
            render them out in the template.
          </p>
          <p>In the next section, we will take our list of videos that are being retrieved when the <code
            class="language-javascript">onTermChange</code> function is executed and store them on the <code
            class="language-javascript">data</code> property.
          </p>
          <h3>Data in Components vs Instances</h3>
          <p>The <code class="language-javascript">data</code> property is defined in a slightly different way depending
            on whether we are working with a Vue instance or a Vue component. The following diagram describes this
            scenario:
          </p>
          <figure>
          ...
          <p>So if we are working with a Vue instance then we can define the <code
            class="language-javascript">data</code> property as an object:
          </p>
          <figure>
...
<pre><code class="language-javascript">data: function() {
    return {
        textInput: ''
          ...
          <p>Once we start working with Vue components the <code class="language-javascript">data</code> property <span
    ...
    return {
        textInput: ''
          ...
          <p>Now, just for a moment, I want you to imagine that we break the rules and make an App component within
            which we define our <code class="language-javascript">data</code> property as an object rather than as a
            function. When we define the <code class="language-javascript">data</code> property as an object you can
            imagine that it essentially gets created inside our Computer's Memory:
          </p>
          <figure>
          ...
          <p>So, essentially, because of this data sharing issue we have to create functions that create objects when
            we're working with <code class="language-javascript">data</code> inside of a component.
          </p>
          ...
          <p>In the last section we spoke about how data inside of a component has to be defined as a function that
            returns an object. So inside of my App component between our list of <code
              class="language-javascript">components</code> and our <code class="language-javascript">methods</code>
            we'll add a new property that will return an object that represents our <code class="language-javascript">data</code>:
          </p>
          <figure>
<pre><code class="language-javascript">data() {
    return {};
          ...
          <p>So we define the <code class="language-javascript">data</code> property and use the shorter ES2015 syntax.
            Inside the <code class="language-javascript">data</code> function we return an object which will initialize
            our data. This is going to be like the starting state or the initial ingredients for our component. So we
            have to start thinking about what different properties our <code class="language-javascript">data</code> is
            going to contain.
          </p>
          <p>We definitely know that we need to have a list of videos because that's what we are retrieving from our
            <code class="language-javascript">onTermChange</code> function - so let's add a videos property which should
            be an array of objects where every object represents one video:
          </p>
          <figure>
<pre><code class="language-javascript">data() {
    return { videos: [] };
          ...
          <p>Now inside the <code class="language-javascript">onTermChange</code> function where we have the <code
            class="language-javascript">.then</code> chained function to wait for a response from the YouTube API we can
            replace the console log with some logic that will take the list of videos that are contained inside of the
            response and update our <code class="language-javascript">data</code> property - specifically the videos
            property:
          </p>
          <figure>
<pre v-pre><code class="language-javascript">methods: {
    onTermChange(searchTerm) {
        axios.get('https://www.googleapis.com/youtube/v3/search', {
          ...
          <p>Remember the <code class="language-javascript">data</code> property that is returned in the response is not
            at all related to the component instance - it is the data property that is tied to our response object that
            comes back from YouTube.
          </p>
          <p>Because <code class="language-javascript">videos</code> is a <code class="language-javascript">data</code>
            property when the above line is executed it will cause the entire template inside the App component to
            automatically re-render. As a quick test, let's add in a counter inside of our template that counts the
            number of videos that have been found on the YouTube API:
          </p>
          <figure>
<pre v-pre><code class="language-html">&lt;div&gt;
  ...
  &lt;/div&gt;</code></pre>
          ...
          <p>Usually we would use a computed function here but because this is just a temporary thing we won't bother
            with a computed function. We reference the <code class="language-javascript">videos data</code> property and
            retrieve it's <code class="language-javascript">length</code> .
          </p>
          <p>Remember that when we are referencing a <code class="language-javascript">data</code> property or function
            that is tied to our component from within our template we do not have to prefix it with <code
              class="language-javascript">this</code> we can just write out the name of the property that we are trying
            to reference.
          </p>
          ...
          <p>The next step is to take the list of videos stored on the <code class="language-javascript">data</code>
            property and pass it down to the <code class="language-javascript">VideoList</code> component.
          </p>
          <h3>Communicating from Parent to Child</h3>
          <p>In the last section we updated our <code class="language-javascript">data</code> property of <code
            class="language-javascript">videos</code> whenever we got a response back from the YouTube API. Because we
            updated a property that was defined on our <code class="language-javascript">data</code> object that causes
            our App component to automatically re-render.
          </p>
          ...
          <figure>
            <pre><code class="language-html">&lt;VideoList v-bind:videos=&quot;videos&quot;&gt;&lt;/VideoList&gt;</code></pre>
            <figcaption>Fig 03-060</figcaption>
          </figure>
          <p>The <code class="language-javascript">videos</code> property we specify above (to the left of the =
            operator) will be available in the child component - there is no restriction on what you call this property
            (e.g. myVideoArray). The value to the right specifies the name of the <code
              class="language-javascript>">data</code>property (so here the name is important) that we want to share
            from the App component - the parent. In this case that's the <code class="language-javascript">videos</code>
            property.
          </p>
          <p>The purpose of the <code class="language-html">v-bind</code> directive is to specify that anytime the <code
            class="language-javascript">videos</code> property is updated inside of the parent it should automatically
            try to re-render the <code class="language-html">VideoList</code> and provide that new list of videos to the
            <code
              class="language-javascript">VideoList</code> component.
          </p>
          <p>We can also use a shorthand form for the v-bind directive as well:</p>
          <figure>
            <pre><code class="language-html">&lt;VideoList :videos=&quot;videos&quot;&gt;&lt;/VideoList&gt;</code></pre>
          ...
          <p>Open the <span class="filename">VideoList.vue</span> file and add the following code to the <code
            class="language-html">script</code> section:
          </p>
          <figure>
<pre><code class="language-javascript">export default {
  name: 'VideoList',
  props: ['videos']
          ...
          <p>We assign an array to the <code class="language-javascript">props</code> property that contains a number of
            strings which match the exact property names that the component should expect to receive from the parent -
            <code
              class="language-javascript">videos</code> in our case. Defining our <code
              class="language-javascript">props</code> list can be as simple as listing out the names of all the
            different properties that we expect to receive. We can also add in some amount of configuration or
            validation to make sure that the child component is receiving the correct type of property as well. We might
            want to validate to make sure that the parent is passing down an array, or a string, or an object. This is
            optional, but recommended. So instead of an array of strings we can pass in an object where the keys are the
            name of the properties we expect to receive and the values are the type of the property that we expect. In
            the case of the videos object we expect it to be an array of objects so we can write the following:
          </p>
          <figure>
<pre><code class="language-javascript">export default {
  name: 'VideoList',
  props: {
      videos: Array
          ...
          <p>This component will then expect to receive a property called <code
            class="language-javascript">videos</code> that should be an array. If an array is not passed then the
...
<pre v-pre><code class="language-html">&lt;template&gt;
  ...
  &lt;/template&gt;</code></pre>
          ...
          <p>This syntax is identical to the earlier example when we accessed the <code
            class="language-javascript">videos data</code> property in the <code class="language-javascript">App</code>
            component. We do the exact same thing when we are accessing a prop as well - the common theme being we just
            use the name.
          </p>
          ...
          <p>Now that we have got some <code class="language-javascript">props</code> we can start to add some logic to
            the VideoList component to render those out as a list.
          </p>
...
&lt;/script&gt;</code></pre>
          ...
          <p>Now, since we are using a <code class="language-html">ul</code> element inside of VideoList we will make
            each individual list item an <code class="language-html">li</code>.
          </p>
          <figure>
<pre><code class="language-html">&lt;template&gt;
    &lt;li&gt;
...
&lt;/template&gt;</code></pre>
          ...
          <figure>
<pre><code class="language-html">&lt;script&gt;
  import VideoListItem from './VideoListItem';</code></pre>
          ...
          <p>Remember that anytime we want to tell the parent component that it has access to the child we have to add
            the <code class="language-javascript">components</code> property to that parent component itself and supply
            the name of the child component. Add the following code to the VideoList component configuration:
          </p>
          <figure>
<pre><code class="language-javascript">export default {
  name: 'VideoList',
  ...
  props: {
      videos: Array
          ...
          <p>Now inside of our <code class="language-javascript">VideoList</code> component we have access to the <code
            class="language-javascript">VideoListItem</code> component. Now we need to figure out how to create one
            <code class="language-javascript">VideoListItem</code> component for every video that is contained within
            our <code class="language-javascript">videos</code> property.
          </p>
...
&lt;/template&gt;</code></pre>
          ...
          <p>The <code class="language-html">v-for</code> directive is the directive you will use every time that you
            want to build a list of components out. We can interpret this directive as - for every single object or
            every single video inside of the list of videos that was provided as a <code
              class="language-javascript">prop</code> to our VideoList component make exactly one VideoListItem
            component.
          </p>
          <p>Notice also how we declared <code class="language-html">video</code> in the statement <code
            class="language-">video in videos</code>. This allows use to access an individual <code
            class="language-">video</code> from within the <code class="language-html">VideoListItem</code> component
            tag.
          </p>
          ...
          <p>You are seeing this output because, at present, the <code class="language-html">VideoListItem</code> component is simply outputting an <code
            class="language-">li</code> element containing the text VideoListItem for each <code
            class="language-html">video</code>
            in the <code class="language-html">videos</code> prop.
          </p>
          ...
          <p>In the last section we were able to get a collection of VideoListItem components to print out on the screen
            by using the <code class="language-">v-for</code> directive.
          </p>
          <p>We now need figure out some way to say that for every video in this list of videos we want to communicate
            this video in the <code class="language-">v-for</code> directive down to the <code
              class="language-">VideoListItem</code>
          ...
          <p>We just went through this process but as a reminder we first have to add a <code class="language-">
          ...
          <p>In particular, how do we get access to one individual video to pass down to the VideoListItem? Well, when
            we write out <code class="language-">v-for=&quot;video in videos&quot;</code> a temporary variable of
            <code class="language-">video</code> is declared that we can use inside of our template. <code
              class="language-">video</code> represents one video or one object in our array.
          </p>
          <p>Let's add some configuration to the VideoListItem tag underneath the <code class="language-">v-for</code>
            directive:
          </p>
          <figure>
          ...
          <p>So we write <code class="language-">v-bind:</code> and then the name of the prop <code
            class="language-">video</code> as we want it to show up inside of the child component. So now
            VideoListItem is going to have access to a variable called <code class="language-">video</code> . After
            that we write <code
              class="language-">="video"</code> which specifies the piece of data or the variable from the VideoList
            component that needs to be passed down to the child. <code class="language-">video</code> is referencing
            the current video that we are iterating over inside of our list of videos. If we had specified a different
            name in the <code class="language-">v-for</code> directive we would need to use that in our <code
              class="language-">v-bind</code> directive also:
          </p>
          <figure>
            <code class="language-">&lt;VideoListItem
          ...
          <p>We will stick with <code class="language-">video</code> to keep the naming terminology consistent. So now
            that we are passing some information, as a <code class="language-">prop</code> , from the parent to the
            child we need to tell the child component to expect to receive the <code class="language-">prop</code> .
          </p>
          <p>In the VideoListItem component add a <code class="language-">props</code> property and, in this case, we
            will supply the <code class="language-">prop</code> names as an array of strings instead of an object,
            although either way is valid.
          </p>
          <figure>
            <code class="language-">export default {
              name: 'VideoListItem',
              props: ['video']
              };</code>
          ...
          <p>Now, in the template section of the VideoListItem component, replace the existing code with a reference to
            the <code class="language-">video</code> prop:
          </p>
          <figure>
<pre v-pre><code class="language-html">&lt;template&gt;
    &lt;li&gt;
  ...
  <figcaption>Fig 03-074</figcaption>
          ...
          <p>We get the name of the video from the <code v-pre class="language-">{{ video.snippet.title }} </code>.
            The video object has a <code class="language-">snippet</code> property which, in turn, contains <code
              class="language-">title, description</code> and <code class="language-">id</code> properties. Re-run
            the app inside of the browser and you should see a list of video titles appearing in our list.
          </p>
          <h3>Keys with v-for</h3>
          <p>In the last section we made use of the <code class="language-">v-for</code> directive to render out a
            list of VideoListItems. Anytime we use this <code class="language-">v-for</code> directive to build out a
            list of items there is one other property that we should (although it is optional) provide to the element or
            the component that we are making a list out of - the <code class="language-">key</code> property:
          </p>
          <figure>
            <code class="language-">&lt;VideoListItem
              v-for=&quot;video in videos&quot;
          ...
          <p>The <code class="language-">key</code> property is used whenever our list is updated. In particular it's
            ...
            individual record that we are rendering. In our case this is <code class="language-">video .etag</code>.
          </p>
          ...
          <p>Currently we are just outputting the title of the video in our list. We are going to add styling (using
            ...
            tag under the <code class="language-">title</code> element. Back in the browser if you refresh the page
          ...
          <p>In the last section we installed the Bootstrap CSS library into our project. We're now going to use some
            ...
            a class of <code class="language-">container</code> - which is used throughout Bootstrap to better style
            and align different items inside of our project. The <code class="language-">div</code> element in the
            template of the App component is one of the absolute root elements inside of our application so we'll add a
            <code class="language-">class</code> of <code class="language-">container</code> to this element.
          ...
          <p>Next add some styling to the SearchBar component in the empty style tag. We use the <code
            class="language-">scoped</code> attribute to ensure that the CSS we add here only affects this component:
          </p>
          <figure>
            <code class="language-">&lt;style scoped&gt;
              input {
              width: 75%;
              }
          ...
          <p>We will apply the <code class="language-">list-group</code> bootstrap style to the <code
            class="language-">ul</code> element in the VideoList component:
          </p>
          <figure>
            <code class="language-">&lt;template&gt;
              &lt;ul class=&quot;list-group&quot;&gt;
          ...
          <p>and the <code class="language-">list-group-item</code>style to the child <code
            class="language-">li</code> elements in the VideoListItem component:
          </p>
          <figure>
<pre v-pre><code class="language-html">&lt;template&gt;
  ...
  <figcaption>Fig 03-077</figcaption>
          ...
          <p>It's worth pointing out that we don't try and add styles directly to the <code
            ...
            tag in the VideoList component. Instead we open up the component itself and add the styles to the <code
              class="language-"> li</code> element located within. If you refresh the browser you should see the list
          ...
          <p>These properties refer to the size of the image. Copy the value of the url property in the default image.
            In the template for the VideoListItem component add an <code class="language-">img</code> tag:
          </p>
          <figure>
<pre v-pre><code class="language-html">&lt;template&gt;
    &lt;li class=&quot;list-group-item&quot;&gt;
...
&lt;/template&gt;</code></pre>
  <figcaption>Fig 03-078</figcaption>
          ...
          <p>Here we bind the <code class="language-">src</code> property to the <code class="language-">url</code>
          ...
          <p>In the last section we displayed our thumbnail image on the screen. However, we did add a very long
            property reference: <code class="language-">video.snippet.thumbnails.default.url</code> directly into our
            ...
            inside of a component. Underneath <code class="language-">props</code> add the <code
              class="language-">computed</code> object with a <code class="language-">thumbnailUrl</code> function:
          </p>
          <figure>
<pre v-pre><code class="language-javascript">export default {
  ...
  <figcaption>Fig 03-079</figcaption>
          ...
          <p>Back in the <code class="language-">img</code> tag in the <code class="language-">template</code> we
...
&lt;/template&gt;</code></pre>
  <figcaption>Fig 03-080</figcaption>
          ...
          <p>In the browser the application should work as before. To recap, we can use computed functions to work with
            or format data before we reference it inside our template. The <code class="language-">this</code> keyword
            in the computed function above is present because we are referencing a <code class="language-">prop</code>
            called <code class="language-">video</code> inside of our component. We then add the properties: <code
              class="language-">snippet.thumbnails.default.url</code> and make that the return value of our function.
            We specified a function name of <code class="language-">thumbnailUrl</code> inside of the computed object.
            ...
            computed function we do not have to specify any parentheses or prefix the function with <code
              class="language-">this.</code> - we just use the computed function's name and Vue takes over from there.
          </p>
          ...
          <p>To achieve this layout we add a class of <code class="language-">media</code> to our <code
            class="language-">li</code> element and a class of <code class="language-">mr-3</code> to our <code
            class="language-">img</code> element. We also have to use a <code
            class="language-">div</code> element with a class of <code class="language-">media-body</code> which
...
&lt;/template&gt;</code></pre>
  <figcaption>Fig 03-081</figcaption>
          ...
          <p>Ok, so we made a difference but we didn't get our desired result. Now the title just appears underneath the
            thumbnail. This is because the <code class="language-">list-group-item</code> and the <code
              class="language-">media</code> class conflict on how the item should be style. To workaround this we
            will add some custom CSS. We add the CSS to a <code class="language-">scoped style</code> tag in
            VideoListItem:</p>
          <figure>
            <code class="language-">&lt;style scoped&gt;
              li {
          ...
          <p>The <code class="language-">display: flex;</code> styling will cause the <code
            class="language-">img</code> and the <code class="language-">div</code> to display on the same line. If
          ...
          <p>In the VideoListItem component let's add some code to watch for a click event on the <code
            class="language-">li</code> element. So anytime someone clicks on the li element we want to emit a Vue
            specific event:
          </p>
          <figure>
<pre v-pre><code class="language-html">&lt;template&gt;
...
&lt;/template&gt;</code></pre>
  <figcaption>Fig 03-088</figcaption>
          ...
          <p>Now let's create the <code class="language-">onVideoSelect</code> method by adding it to a new <code
            class="language-">methods</code> object in the component definition:
          </p>
          <figure>
<pre v-pre><code class="language-javascript">export default {
    name: 'VideoListItem',
  ...
  <figcaption>Fig 03-089</figcaption>
          ...
          <figure>
            <code class="language-">this.$emit('termChange', event.target.value);</code>
          ...
          <p>The first argument is the event name we want to emit and the second argument is a the video that has been
            clicked on that belongs to this particular VideoListItem component. Add the following code inside the <code
              class="language-">onVideoSelect</code> method:</p>
          <figure>
            <code class="language-">methods: {
              onVideoSelect() {
          ...
          <p>Remember <code class="language-">this.video</code> is the prop that was passed in from the parent
          ...
          <p>Remember, to add an event handler we write out <code class="language-">@</code> and then the name of the
            event that we want to listen for - in the case <code class="language-">videoSelect </code> then after the
            <code class="language-">=</code> assignment operator we specify what we want to do when the event is
            triggered. We will again create a new method which we will call <code
              class="language-">onVideoSelect</code>. We are using the same name for this method that we used in the
            VideoListItem component but remember this is going to be a completely different function in it's own <code
              class="language-">methods</code> object. Add the following code to the VideoList component:</p>
          <figure>
            <code class="language-">methods: {
              onVideoSelect(video) {
          ...
          <p>We use the same event name of <code class="language-">videoSelect</code> and pass along the video object.
          ...
          <p>We use the same <code class="language-">onVideoSelect</code> method name which means we need to add this
            method to the <code class="language-">methods</code> object of the App component:
          </p>
          <figure>
<pre v-pre><code class="language-javascript">methods: {
    onVideoSelect(video) {
  ...
  <figcaption>Fig 03-095</figcaption>
          ...
          <p>So, just for the moment, we are writing the video object to the console. The <code class="language-">
            onVideoSelect</code>
            method has access to the <code class="language-">video</code> object because in the <code
              class="language-"> VideoList</code> component when we emitted our <code
              class="language-">videoSelect</code> event we passed along the video object. Back in the browser,
            refresh, select a video and then check the console. As the video object gets written to the console you
            should see an observer object being written out that contains <code class="language-">etag, id, kind and
          ...
          <p>So we will use the <code class="language-">video prop</code> to store the selected <code
            class="language-">video</code> that the user just clicked on. To verify this is working modify the
            template to display the <code class="language-">name</code> of the selected <code
              class="language-">video</code>
            :</p>
          <figure>
<pre v-pre><code class="language-html">&lt;template&gt;
  &lt;div&gt;
...
&lt;/template&gt;</code></pre>
  <figcaption>Fig 03-098</figcaption>
          ...
          <figure>
            <code class="language-">import VideoDetail from './components/VideoDetail.vue';</code>
          ...
          <figure>
            <code class="language-">components: {
              SearchBar,
              VideoList,
              VideoDetail
              },</code>
          ...
          <p>Finally, I'm going to make sure that I add the <code class="language-">VideoDetail</code> component
            itself to the template of the <code class="language-">App</code> component.
          </p>
          <figure>
            <code class="language-">&lt;template&gt;
              &lt;div class=&quot;container&quot;&gt;
          ...
          <p>We add a (shorthand) v-bind expression to pass down the <code class="language-">selectedVideo</code> .
          </p>
          <h3>Updating Data</h3>
          <p>In the last section we said that the App component needed to somehow communicate the selected <code
            class="language-">video</code> from the <code class="language-">onVideoSelect</code> method to our <code
            class="language-">VideoDetail</code> component. Let's take a look at a quick diagram that is going to help
          ...
          <p>Here we can see the entire video selection process in diagram format. You can see the chain of listening
            for and emitting events all the way up to the App. At present, inside the <code
              class="language-">App</code> component, whenever that event finally comes up and triggers our <code
              class="language-">onVideoSelect</code> function we are currently only logging details about the <code
              class="language-">video</code> object out to the console. We need to add a new property to the <code
              class="language-">data</code> object in the <code
              class="language-">App</code> component to get the <code class="language-">video</code> object
            communicated over to the <code class="language-">VideoDetail</code> component. We will call the new
            property <code class="language-">selectedVideo</code> and it will reflect the current video that the user
            has selected.
          </p>
          <p>So inside of the <code class="language-">App</code> components handler for listening for that click event
            of <code class="language-">onVideoSelect</code> we will place some code to update the <code
              class="language-">selectedVideo</code> prop. Anytime that data property is updated it will cause our
            ...
            video down to the <code
              class="language-">VideoDetail</code> component as a prop. So that's the idea, we're going to make use of
            the data system of the <code class="language-">App</code> component to communicate the newly clicked video
            over to the <code class="language-">VideoDetail</code> component. Add the <code
              class="language-">selectedVideo</code>
            prop to the <code class="language-">data</code> object:
          </p>
          <figure>
          ...
          <p>The default value of <code class="language-">selectedVideo</code> is <code
            class="language-">null</code> because when our application first starts there will be no video selected.
            Inside of <code class="language-">onVideoSelect</code> we will add some code to update this property:</p>
          <figure>
            <code class="language-">onVideoSelect(video) {
          ...
          <p>Recall, that anytime we update a data property it causes our component to instantly re-render with that new
            piece of data. Now inside the <code class="language-">template</code> of the <code
              class="language-">App</code> component we can modify the <code class="language-">VideoDetail</code>
            tag:
          </p>
          <figure>
            <code class="language-">&lt;VideoDetail :video=&quot;selectedVideo&quot; /&gt;</code>
            <figcaption>Fig 03-105</figcaption>
          </figure>
          <p>This will assign the <code class="language-">selectedVideo</code> to the <code
            class="language-">video</code> prop. Again remember that, inside of a template, to reference a data
            property we do not have to append the data property name with <code class="language-">this.</code> or
            anything of that nature - we just put the data properties name, in our case <code class="language-">
              selectedVideo</code>
          </p>
          ...
          <p>You will notice an error message: <code class="language-">Cannot read property 'snippet' of null</code>
            being emitted by our <code class="language-">VideoDetail</code> component. In the next section we'll
          ...
          <p>In the last section we added some code to the VideoDetail component to show the title of the selected
            video. However we are getting an error message: <code class="language-">Cannot read property 'snippet' of
            null</code> When our application first renders our <code class="language-">App</code> component has a
            data property for <code class="language-">selectedVideo</code> which is set to null:
          </p>
          <figure>
            <code class="language-">data() {
              return { videos: [], selectedVideo: null };
          ...
          <p>We take the <code class="language-">selectedVideo</code> property and we pass it down to the <code
            class="language-">VideoDetail</code> component:
          </p>
          <figure>
            <code class="language-">&lt;VideoDetail :video=&quot;selectedVideo&quot; /&gt;</code>
            <figcaption>Fig 03-107</figcaption>
          </figure>
          <p>Inside <code class="language-">VideoDetail</code> our template tries to read the <code
            class="language-">snippet</code> property of the <code class="language-">video</code> prop:
          </p>
          <figure>
<pre v-pre><code class="language-html">&lt;template&gt;
    &lt;div&gt;
...
&lt;/template&gt;</code></pre>
  <figcaption>Fig 03-108</figcaption>
          ...
          <p>At this point it is null which is why we are seeing the error message above. Add some code to make sure
            that if the <code class="language-">video</code> prop is null we do not attempt to read the <code
              class="language-">snippet</code> property on it:
          </p>
          <figure>
<pre v-pre><code class="language-html">&lt;template&gt;
    &lt;div v-if=&quot;video&quot;&gt;
...
&lt;/template&gt;</code></pre>
  <figcaption>Fig 03-109</figcaption>
          ...
          <p>The <code class="language-">v-if</code> directive can be thought of as a conditional rendering statement
            ...
            render the <code class="language-">div</code> or anything inside it. Because Vue doesn't render the <code
              class="language-">div</code> it also skips the line of code contained within - and therefore the error
            is not thrown.
          </p>
          <p>You'll most frequently see <code class="language-">v-if</code> around anytime you're loading up some data
            from some remote API because it always takes some amount of time to retrieve that data. So with <code
              class="language-">v-if</code> we can very easily tell our application to wait until data is available
            and only then try and render the relevant content. In the next section we will figure out we are going to
            get this <code
              class="language-">VideoDetail</code> component, that will eventually show a video player, to display on
            the screen.
          </p>
          ...
          <p>We will start with the video title and description. Add the following code to the <code
            class="language-">template</code> of the <code class="language-">VideoDetail</code> component.
          </p>
          <figure>
<pre v-pre><code class="language-html">&lt;template&gt;
    &lt;div v-if=&quot;video&quot;&gt;
...
&lt;/template&gt;</code></pre>
  <figcaption>Fig 03-110</figcaption>
          ...
          <p>YouTube provides some <code class="language-">html</code> which includes an <code
            class="language-">iframe</code>
            with a <code class="language-">src</code> of <code
              class="language-">https://www.youtube.com/embed/</code> followed by the unique video id:
          </p>
          <figure>
          ...
          <p>When you embed a YouTube video you can add it directly to another web application. Add the following code
            above the <code class="language-">div</code> with a class of <code class="language-">details</code>:
          </p>
          <figure>
            <code class="language-">&lt;div&gt;
              &lt;iframe :src=&quot;videoUrl&quot;/&gt;
          ...
          <p>The <code class="language-">src</code> property is a dynamic value provided by the <code
            class="language-">videoUrl</code> computed function:
          </p>
          <figure>
            <code class="language-">computed: {
              videoUrl() {
          ...
          <p>Because the <code class="language-">videoId</code> <code class="language-">const</code> that we used
            and the prop name of <code class="language-">this.video.id.videoId</code> are identical we use ES2015
            destructuring assignment syntax and surround our <code class="language-">const</code> with curly braces.
          ...
          <p>So a box appears but there doesn't look like there is any content inside of it. If I go over to the console
            you might notice a couple of warnings and even errors over here such as <code class="language-">Error
                                                                                          parsing header</code> or
            <code class="language-">ERR_BLOCKED_BY_CLIENT</code> These errors are thrown by
            the YouTube <code class="language-">iframe</code> - we don't have any control over them. In the next
          ...
          <p>We've now got our YouTube embed visible on the screen but it's a little bit small. We will use the
            ...
            will cause the <code class="language-">iframe</code> to expand to more suitably fit the space available to
            it. The documentation provides an example:
          </p>
          <figure>
            <code class="language-">&lt;div class=&quot;embed-responsive embed-responsive-16by9&quot;&gt;
              &lt;iframe class=&quot;embed-responsive-item&quot; src=&quot;https://www.youtube.com/embed/zpOULjyy-n8?rel=0&quot; allowfullscreen&gt;&lt;/iframe&gt;
              &lt;/div&gt;</code>
            <figcaption>Fig 03-117</figcaption>
          </figure>
          <p>We have to place a <code class="language-">div</code> round our <code class="language-">iframe</code>
            with a couple of different classes applied to it. We also add one class on the <code class="language-">
              iframe</code>
          ...
          <p>Our video <code class="language-">iframe</code> is now plainly visible on the screen but it's still
          ...
          <figure>
            <code class="language-">&lt;div class=&quot;row&quot;&gt;
              &lt;VideoDetail :video=&quot;selectedVideo&quot; /&gt;
              &lt;VideoList @videoSelect=&quot;onVideoSelect&quot; :videos=&quot;videos&quot;&gt;&lt;/VideoList&gt;
              &lt;/div&gt;</code>
          ...
          <figure>
            <code class="language-">&lt;div v-if=&quot;video&quot; class=&quot;col-md-8&quot;&gt;</code>
            <figcaption>Fig 03-120</figcaption>
          </figure>
          <p>This will create a column out of the <code class="language-">div</code> element that at most can only
            ...
            the VideoList component add the following class to the root <code
              class="language-">ul</code> element:
          </p>
          <figure>
            <code class="language-">&lt;ul class=&quot;list-group col-md-4&quot;&gt;</code>
          ...
          <p>We make sure that the parent components tag for showing that component has a <code class="language-">
            v-bind</code>
