    ...
    <div class="panel-group">
      <div class="panel panel-primary">
        ...
        <div class="panel-body">
          ...
          <p>The blue box represents a <code class="prettyprint">div</code> and we're going to be applying all our
            styling to that <code class="prettyprint">div</code> - so the border, border radius, dimensions, text
            centering etc. Inside the <code class="prettyprint">div</code> there will two child elements. One will be a
            <code class="prettyprint">span</code> that will contain some amount of text inside it. The other will be an
            invisible input element which will provide the drag and drop functionality and click the element. Add the
            following code to the <code class="prettyprint">template</code> of the UploadForm component:</p>
          <figure>
...
&lt;/style&gt;</pre>
          ...
          <p>The height of 30vh refers to the view height or the view window exposed in the browser which means it will
            essentially occupy 30 percent of the overall height of the browser. After that we setup our <code
              class="prettyprint">border</code> and <code class="prettyprint">border-radius</code>. Refresh the browser
            and you should see the styling:</p>
          <figure>
            <img src="./images/vuejsessentials/Fig08-005.png"/>
            <figcaption>Fig 08-005</figcaption>
          </figure>
          <p>So now we've got the <code class="prettyprint">div</code> that is taking up pretty much the whole width of
            the screen. Also, notice that if I reduce the height of the browser the height of the <code
              class="prettyprint">div</code> will reduce as well to approx 30% of the available height.
          </p>
...
&lt;/template&gt;</pre>
          ...
          <p>We wrapped the <code class="prettyprint">router-view</code> in a single <code
            class="prettyprint">div</code> with a class of <code class="prettyprint">ui container</code> which is
            provided by SemanticUI and should add the desired styling. Next we will fix up the Upload panel in
            UploadForm. In the <code class="prettyprint">style</code> tag at the bottom add the following:
          </p>
          <figure>
<pre class="prettyprint">&lt;style&gt;
    .dropper {
...
&lt;/style&gt;</pre>
          ...
          <p>If you preview in the browser you should see the content is now centered. The <code
            class="prettyprint">input</code> tag is still visible. We will add some styling to make the <code
            class="prettyprint">input</code> invisible and stretch it to fill the whole dropper <code
...
&lt;/style&gt;</pre>
          ...
          <p>We add some styling rules for <code class="prettyprint">input</code> to resize it's <code
            class="prettyprint">width</code> and <code class="prettyprint">height</code> . We add a <code
            class="prettyprint">position</code> of <code class="prettyprint"> absolute</code> and then on the dropper
            class we add a <code
          ...
          <p>and you'll see that the <code class="prettyprint">input</code> has moved to the top left-hand side. If you
            hover over anywhere on the dropper <code class="prettyprint">div</code> you will see the "No file chosen"
            notification. That means the <code class="prettyprint">input</code> element is now filling up the entire
            height and width of the <code class="prettyprint">div</code>. If you click anywhere in the <code
              class="prettyprint">div</code> a file selector will be displayed. To change the visibility of the <code
              class="prettyprint">input</code> element add an opacity of 0 to it's styling:</p>
          <figure>
<pre class="prettyprint">input {
    ...
    opacity: 0;
          ...
          <p>We will add a background color to our dropper <code class="prettyprint">div</code> to make it clear that
    ...
    background-color: #eee;
          ...
          <p>We will do some work on the Image module design. Inside <span class="filename">images.js</span> add the
            <code class="prettyprint">uploadImages</code> action:
          </p>
          <figure>
<pre class="prettyprint">async uploadImages({ commit }, images) {
    console.log(images);
          ...
          <p>We specify our first options argument, as usual, specifying <code class="prettyprint">commit</code> to
            allow us to execute mutations. As a second argument add in an argument called <code
              class="prettyprint">images</code> which we will upload to Imgur. We add a temporary <code
              class="prettyprint">console.log</code> to give us some information about the <code class="prettyprint">images</code>
            object. In the UploadForm component we will map the <code class="prettyprint">uploadImages</code> action so
            we can call it anytime we select a file. Firstly, import <code class="prettyprint">mapActions</code>:
          </p>
          <figure>
            <pre class="prettyprint">import { mapActions } from 'vuex';</pre>
            <figcaption>Fig 08-012</figcaption>
          </figure>
          <p>Then add the following code to our <code class="prettyprint">methods</code> property to give us access to
            the <code class="prettyprint"> uploadImages</code> action:
          </p>
          <figure>
    ...
    methods: mapActions(['uploadImages']);
          ...
          <p>Now that we've added in our <code class="prettyprint">uploadImages</code> action we can make sure that this
            action gets called anytime someone selects a file from our <code class="prettyprint">input</code> tag. Add
...
&lt;/template&gt;</pre>
          ...
          <p>So we watch for a <code class="prettyprint">change</code> event which signals the selection of that <code
          ...
          <p>Inside the <code class="prettyprint">event</code> object is a property called <code class="prettyprint">target</code>
            with a value of <code class="prettyprint">input</code> which contains a property called <code
              class="prettyprint">files</code> . This is a file list object which contains a reference to every file
          ...
          <p>One other point to consider is that in addition to clicking on the upload element we also want the user to
            be able to drag and drop files to upload. If we try dragging and dropping a file onto the element the <code
              class="prettyprint">changed</code> event will still fire and we still get the same level of information so
          ...
          <p>Throughout this course we've used the exact same syntax every time - the <code
            class="prettyprint">@</code> sign followed by the name of the event followed by the method to call when
            triggered. We can change the syntax of this statement to send back just the part of the <code
              class="prettyprint">event</code> object that we are interested in:
          </p>
          <figure>
          ...
          <p>Let's explain the reason for the <code class="prettyprint">$</code> sign. If ommitted the <code
            class="prettyprint">$</code> sign and just wrote out <code class="prettyprint">event .target.files</code>
            then anytime the <code class="prettyprint">change</code> event is triggered Vue would try to look for an
            ...
            throw an error. The <code
              class="prettyprint">$</code> sign tells Vue that we want to take the <code
              class="prettyprint">event</code> object from triggering the <code
              class="prettyprint">change</code> event and pass through the <code class="prettyprint"> data.files</code>
            property to the <code class="prettyprint">uploadImages</code> action. In the browser, if we do a refresh,
          ...
          <p>Next we will add the functionality to upload multiple images. In UploadForm add the following code to the
            <code class="prettyprint">input</code> tag:
          </p>
          <figure>
                    <pre class="prettyprint">&lt;template&gt;
    &lt;div class=&quot;dropper&quot;&gt;
...
&lt;/template&gt;</pre>
          ...
          <p>We're now inside of our <code class="prettyprint">uploadImages</code> action and we're just about ready to
            ...
            API call is the Bearer with the <code class="prettyprint">access_token</code> property on it. You can ignore
            ...
            to a user's account - so we will use the Authorization Bearer with the <code class="prettyprint">access_token</code>
            property.
          </p>
          <p>So inside the <code class="prettyprint">uploadImages</code> action in the Image module we are going to
          ...
          <p>We are not going to write the actual upload logic inside of the <code
            class="prettyprint">uploadImages</code> action, instead we will put the code in the api <span
            class="filename">imgur.js</span> file which is where we've been placing all of our logic that reaches out to
            Imgur so far. First let's get the <code class="prettyprint">access_token</code> from the Auth module,
            exactly like we did in the <code
              class="prettyprint">fetchImages</code> action. Rather than making use of the <code
              class="prettyprint">commit</code> function we will swap it for <code class="prettyprint"> rootState</code>
            which gives us access to all the state that contained inside of our Vuex store. We're going to reach over to
            the Auth module and get our <code class="prettyprint">access_token</code> out of that:</p>
          <figure>
          ...
          <p>Next we're going to call our api imgur module to do the actual file upload. We will define a function
            called <code class="prettyprint">uploadImages</code> shortly but for the moment let's just add the function
            call to the <code class="prettyprint">uploadImages</code> action:
          </p>
          <figure>
          ...
          <p>In the <span class="filename">imgur.js</span> api file add a new function called <code class="prettyprint">uploadImages</code>
            :</p>
          <figure>
        ...
        return axios.post(`$(ROOT_URL}/3/image`, formData, {
          ...
          <p>First we take the images file list object and turn it into an array - currently it is an array like object
            (this is what it is known as in the JavaScript world). We use <code
              class="prettyprint">Array.from</code> and pass in the images file list object. We then call the <code
              class="prettyprint">map</code> function and use it to iterate over all the images stored inside of the
            file list object. Inside the <code class="prettyprint">map</code> function we're going to take every image
            ...
            notice that we have to provide a key of <code
              class="prettyprint">image</code> which should be a binary file or base64 data - we have to attach the
            ...
            <code
              class="prettyprint">formData</code> object. This object is part of the standard vanilla Javascript spec
          ...
          <p>Then directly after that we're going to take the <code class="prettyprint">formData</code> object and we're
            going to attach the reference to the image that was provided to our <code class="prettyprint">map</code>
            function: </p>
          <figure>
            <pre class="prettyprint">formData.append('image', image);</pre>
            <figcaption>Fig 08-024</figcaption>
          </figure>
          <p>I'm using <code class="prettyprint">image</code> because if you look back at the Imgur API documentation it
            specifically stated that we need to supply a key of <code class="prettyprint">image</code> that contains the
            actual image file itself. Next we make the actual POST request to the Imgur API that's going to take that
            <code class="prettyprint">formData</code> object and send off the entire object:</p>
          <figure>
<pre class="prettyprint">return axios.post(`$(ROOT_URL}/3/image`, formData, {
          ...
          <p>First we use the <code class="prettyprint">return</code> keyword to make sure we return the request object.
            Then we write <code class="prettyprint">axios.post</code> and supply the Url as the first argument. As the
            second argument we supply the <code class="prettyprint">formData</code> object. Then, as the third object,
            we pass in the headers, as we did previously, along with the <code class="prettyprint">access_token</code>
          ...
          <p>That's just about it. The code above is going to take all of our images from inside the images list, it's
            going to create a <code class="prettyprint">formData</code> object, attach our image to that object which is
            going to turn the image reference to a file itself, and then it's going to take the entire <code
              class="prettyprint">formData</code> and post it off to the Imgur API. It will then repeat the same process
          ...
          <p>In the last section we put together the <code class="prettyprint">uploadImages</code> function that loops
            ...
            time. That's good, but there is one point we need to be aware of. We need to make sure that this <code
              class="prettyprint">uploadImages</code>

            function returns exactly one promise that represents all of these separate image requests or all these
            separate uploads so that any other function that calls <code class="prettyprint">uploadImages</code> will
            ...
            returned from the <code
              class="prettyprint">map</code> function:
          </p>
          <figure>
            <pre class="prettyprint">Array.from(images).map(image =&gt;</pre>
            <figcaption>Fig 08-026</figcaption>
          </figure>
          <p>So notice how we are running <code class="prettyprint">map</code> function and inside the body we return
    ...
    const promises = Array.from(images).map(image =&gt; {
        ...
        return axios.post(`${ROOT_URL}/3/image`, formData, {
          ...
          <p><code class="prettyprint">Promise.all</code> is a built-in Javascript function that takes an array of
            ...
            that after all the work in <code class="prettyprint">uploadImages</code> is completed and then we will
          ...
          <p>Test this in the browser. You will notice (if you have your Network tab open in the console) one or more
            POST requests that represent each of the individual file uploads. We then get redirected back over to <code
              class="prettyprint">locahost:8080</code> and we should see our uploaded image(s) on the screen.
          </p>
          ...
          <p>We are now able to see a list of the images that we have uploaded using the Imgur API but they're
            ...
            moment. In the ImageList component you'll recall we have our root <code class="prettyprint">div</code> and
...
&lt;/style&gt;</pre>
          ...
          <p>We are going to use something called CSS grids to style the list of images. We create three separate
            ...
            styling to our class which we added to the <code class="prettyprint">div</code> above. We add <code
              class="prettyprint">column-count</code> of 3 and <code class="prettyprint">column-gap</code> of 0. This
          ...
          <p>Directly after that we add some styling to the <code class="prettyprint">img</code> tag for each of the
            images. We set the <code class="prettyprint">max-width</code> to <code class="prettyprint">100%</code> to
            make sure that the images dont break out of the column width which they have been assigned. Then we add a
            <code
              class="prettyprint">padding</code> of 5 pixels which works closely with the <code
              class="prettyprint">column-gap</code> of <code class="prettyprint">0</code>. It's going to make sure that
            there's five pixels between every <code class="prettyprint">img</code> tag both above and to the side. Now
...
&lt;/template&gt;</pre>
          ...
          <p>We add a <code class="prettyprint">v-if</code> expression to check the value of <code class="prettyprint">isLoggedIn</code>
            . If they are render the list of images as before. If they are not then render the <code
              class="prettyprint">h2</code> element with the <code
              class="prettyprint">v-else</code> directive.
          </p>
