        ...
        <div class="panel-body">
          ...
          <p>In the last section we spoke about how we're going to use a Vue router library to customize what is
            displayed on the screen based on the current Url. We're going to use this library to watch for this <code
              class="language-">/oauth2/callback</code> route. Whenever the user ends up on this route we're going to
          ...
          <p>Finally the third step is to tell Vue Router whenever it sees a Url that it cares about or a path that it
            ...
            a user browses to <code class="language-">/oauth2/callback</code> and Vue Router says "Ok, I need to show
          ...
          <p>Recall that we already installed the Vue Router library a little bit ago at the very start of the this
            ...
            Vue Router:</p>
          <figure>
            <pre><code class="language-javascript">Vue.use(VueRouter);</code></pre>
...
<pre><code class="language-javascript">const router = new VueRouter ({
  routes: [
          ...
          <p>We pass in an object containing a property called <code class="language-">routes</code>. This is going to
          ...
          <p>The first route that you and I are going to define is going to be the <code class="language-">/oauth2/callback</code>
            route:
          </p>
          <figure>
<pre><code class="language-javascript">const router = new VueRouter ({
  routes: [
      { path: '/oauth2/callback', component: AuthHandler }
          ...
          <p>We haven't yet created the AuthHandler Vue component - so we will need to do that and then import it into
            <code class="language-">main.js</code>. Before we do that we need to complete one more step. At the bottom
            ...
            Vue Router as well. So we can think of the <code
              class="language-">Vue.use</code> as the initial handshake and now we need to pass the actual instance
...
&lt;/script&gt;</code></pre>
          ...
          <p>Remember that anytime a user comes back to our application and sees that <code
            class="language-">/oauth2/callback</code> route it will probably only be displayed for a split second as
          ...
          <p>In <span class="filename">main.js</span> we will wire the AuthHandler component up by adding an <code
            class="language-">import</code> statement:
          </p>
          <figure>
            <pre><code class="language-javascript">import AuthHandler from './components/AuthHandler';</code></pre>
          ...
          <p>In the last section we added Vue Router to our application. If we now refresh the browser and look at the
            address bar we will see a <code class="language-">#</code> sign is now present in the Url:
          </p>
          <figure>
            <pre><code class="language-html">http://localhost:8080/#/</code></pre>
            <figcaption>Fig 06-008</figcaption>
          </figure>
          ...
          <figure>
            <pre><code class="language-html">/oauth2/callback#access_token....</code></pre>
          ...
          <p>We can modify the router configuration to use Browser Router instead of Hash Router. The Browser Router
            option causes the router to look at all the characters between the domain <code
              class="language-">localhost:8080</code> and the hash on the right-hand side to figure out what route or
            what set of components need to be shown on screen. So the behaviour of Browser Router is a better fit for
            our requirements. We can still add further information after the <code class="language-">#</code> such as
            our access_token etc.
          </p>
          <p>To switch to using the Browser Router we will modify our router configuration in <code
            class="language-">main.js</code>:
          </p>
          <figure>
<pre><code class="language-javascript">const router = new VueRouter ({
  mode: 'history',
  routes: [
      { path: '/oauth2/callback', component: AuthHandler }
          ...
          <p>Back in the browser navigate to <code class="language-">localhost:8080</code> and you'll notice the page
...
&lt;/template&gt;</code></pre>
          ...
          <p>So now anytime VueRouter matches up a route that's going to take that component and put it inside the <code
            class="language-">vue-router</code> tag. In the browser navigate to <code class="language-">/oauth2
          ...
          <p>The key here is making sure that the instant we know AuthHandler is about to render on the screen we want
            it to call the <code class="language-">finalizeLogin</code> right away. We achieve this using a feature of
          ...
          <p>The official Vue documentation has a fantastic <a
            ...
            names. We start off at the very top where we create a new Vue instance. When we call <code
              class="language-">new Vue</code> it kicks off some initial events and internal code handling stuff to
            setup this Vue instance. After that initial bit of configuration and setup a lifecycle method called <code
              class="language-">beforeCreate</code> is then automatically called. If we define a function with a name
            of <code class="language-">beforeCreate</code> in a component:
          </p>
          <figure>
...
&lt;/script&gt;</code></pre>
          ...
          <p>is a different lifecycle method that will be called at some point inside this component's lifecycle. For
            example, right after calling <code class="language-">beforeCreate</code> we then setup some further
            configuration, immediately after that a second lifecycle method named <code
              class="language-">created</code> is called. So, again, if we add the following code:
          </p>
          <figure>
<pre><code class="language-html">&lt;script&gt;
    export default {
  ...
  &lt;/script&gt;</code></pre>
          ...
          <p>The <code class="language-">created</code> method we defined will be called automatically when the
          ...
          <p>For our current application, we want to make sure that the instant the component is created we try to call
            the <code class="language-">finalizeLogin</code> action that we're going to eventually create inside of
            our application. It's probably most appropriate for us to make use of the <code
              class="language-">created</code> lifecycle method.
          </p>
          ...
          <p>If you look at the official Vue Router documentation then it's going to suggest the <code
            class="language-">created</code> lifecycle method is the optimal location to do any initial data loading.
          ...
          <p>In the last section we spoke about the presence of lifecycle methods inside of components. Next we have to
            make sure that when the <code class="language-">created()</code> method we defined is invoked we call the
            appropriate action in the Auth module - <code class="language-">finalizeLogin</code>.
          </p>
          <p>Let's first wire up the <code class="language-">finalizeLogin</code> action to the AuthHanlder component
            and then we'll go over to the Auth module and start to define it - first we import <code
              class="language-">mapActions</code> :
          </p>
          <figure>
          ...
          <figure>
            <pre><code class="language-javascript">methods: mapActions(['finalizeLogin']),</code></pre>
            <figcaption>Fig 06-019</figcaption>
          </figure>
          <p>Now we can invoke the <code class="language-">finalizeLogin</code> action from the <code
            class="language-">created()</code> method:
          </p>
          <figure>
  ...
  this.finalizeLogin();
          ...
          <p>So to call an action that's been bound to our <code class="language-">methods</code> object from the
            lifecycle method all we have to do is use the <code class="language-">this</code> keyword followed by a
            period and then the name of the action - in this case <code class="language-">finalizeLogin</code>. We
            don't have to do anything like <code
              class="language-">this.actions</code> it's simply <code
              class="language-">this.finalizeLogin</code>.
          </p>
          ...
          <p>To get the hash from the Url we write <code class="language-">window.location.hash</code> this will
            return everything from the <code class="language-">#</code> symbol to the right hand side of the Url:
          </p>
          <figure>
<pre><code class="language-javascript">created() {
  this.finalizeLogin(window.location.hash);
          ...
          <p>Next we will work on the <code class="language-">finalizeLogin</code> action to extract the access_token
            out of the Url and then call the <code class="language-">setToken</code> mutation.
          </p>
          <p>In the browser click the login button. If you have the console open you may see an error:</p>
          <figure>
            <pre><code class="language-">[vuex] unknown action type: finalizeLogin</code></pre>
            <figcaption>Fig 06-021</figcaption>
          </figure>
          <p>This error occurs because we haven't created the <code class="language-">finalizeLogin </code> action
            yet. In the console if you write:
          </p>
          <figure>
            <pre><code class="language-javascript">window.location.hash</code></pre>
...
&account_username=maxsage&account_id=91444873"
          ...
          <p>Add the following code in the Auth module under the <code class="language-">login</code> action:</p>
          <figure>
          <pre><code class="language-javascript">finalizeLogin() {
},</code></pre>
            <figcaption>Fig 06-023</figcaption>
          </figure>
          <p>When we call <code class="language-">finalizeLogin</code> from our AuthHandler component we pass in <code
            class="language-">window.location.hash</code>:
          </p>
          <figure>
            <pre><code class="language-javascript">this.finalizeLogin(window.location.hash);</code></pre>
            <figcaption>Fig 06-024</figcaption>
          </figure>
          <p>So this argument is going to show up as the second argument to the <code
            class="language-">finalizeLogin</code> action. Remember the first argument that gets passed to any action
            is an object that has a bunch of helper methods tied to it that allow us to modify our Vuex module - for
            example it might contain the <code class="language-">commit</code> function that allows us to call
            mutations:
          </p>
          <figure>
          <pre><code class="language-javascript">finalizeLogin({ commit }, hash) {
},</code></pre>
            <figcaption>Fig 06-025</figcaption>
          </figure>
          <p>So we supply the <code class="language-">commit</code> function again in the first argument. Remember we
            supplied the <code class="language-">window.location.hash</code> property to the call to <code
              class="language-">finalizeLogin</code> in the <code class="language-"> created</code> lifecycle method
            hook so that hash will be the second argument to this action. We simply call it <code class="language-">hash</code>
  ...
  commit('setToken', queryString.access_token);
          ...
          <p><code class="language-">hash.replace('#', '')</code> removes the <code
            class="language-">#</code> character from the start of the string. We pass the remaining string into the
            qs module which is used to manipulate, parse and generate query strings. Firstly, import the qs module:
          </p>
          <figure>
          <pre><code class="language-javascript">import qs from 'qs';</code></pre>
            <figcaption>Fig 06-027</figcaption>
          </figure>
          <p>Then pass our string into the <code class="language-">qs.parse</code> function and assign it to a
            variable called <code class="language-">queryString</code>:
          </p>
          <figure>
          <pre><code class="language-javascript">const queryString = qs.parse(hash.replace('#', ''));</code></pre>
            <figcaption>Fig 06-028</figcaption>
          </figure>
          <p><code class="language-">qs.parse</code> will return an object that contains the properties and values
            from the string we pass. So in this case there will be keys like access_token, refresh_token etc. and the
            corresponding values.
          </p>
          <p>Next we call the <code class="language-">setToken</code> mutation and pass in the access_token from our
            queryString:
          </p>
          <figure>
          <pre><code class="language-javascript">commit('setToken', queryString.access_token);</code></pre>
          ...
          <p>In the last section we finished up or <code class="language-">finalizeLogin</code> action and we assumed
  ...
  isLoggedIn: state =&gt; !!state.token
          ...
          <p>We have defined one getter called <code class="language-">isLoggedIn</code> which returns a boolean value
            indicating if our user is authenticated with our application. We will use the <code class="language-">isLoggedIn</code>
            getter to supply that information to our AppHeader component allowing us to customize it's template
            depending on the value we receive. In the AppHeader component's <code class="language-">script</code>
            section import the <code class="language-">mapGetters</code> function from Vuex:</p>
          <figure>
          <pre><code class="language-javascript">import { mapActions, mapGetters } from 'vuex';</code></pre>
            <figcaption>Fig 06-032</figcaption>
          </figure>
          <p>We're going to use this <code class="language-">mapGetters</code> function to pull some information out
            of the Auth module and add it directly to the AppHeader. We use a similar syntax to what we just did with
            the <code class="language-">mapActions</code> :
          </p>
          <figure>
            <pre><code class="language-">methods: mapActions(['login'])</code></pre>
...
<pre><code class="language-javascript">export default {
  ...
  methods:
      mapActions(['login'])
          ...
          <p>Now that we've defined this getter of <code class="language-">isLoggedIn</code> and assigned it to the
            component we can reference it in our template:
          </p>
          <figure>
            <pre v-pre><code class="language-html">{{ isLoggedIn }}</code></pre>
            <figcaption>Fig 06-035</figcaption>
          </figure>
          <p>Remember whenever we are working with our templates we don't have to write out <code class="language-">this.</code>
            or <code class="language-">this.computed</code> or anything like that we just supply the name of the
            computed property - in this case <code class="language-">isLoggedIn</code> . Refresh the browser and you
          ...
          <p>If you browse to the application root instead <code class="language-">http://localhost :8080</code> the
          ...
          <p>Our AppHeader component is now aware of whether or not the user is actually signed into our application. We
            will now use the <code class="language-">v-else</code> directive to show different buttons and layout
          ...
          <p>Add the following Html inside the existing right menu <code class="language-">div</code> :</p>
          <figure>
<pre><code class="language-html">&lt;template&gt;
...
&lt;/template&gt;
          ...
          <p>So we use the <code class="language-">v-if</code> directive to say if <code
            class="language-">isLoggedIn</code> is equal to <code class="language-">true</code> then display the
            buttons (or placeholders at the moment) inside of the <code class="language-">div</code> . We then use the
            <code
              class="language-">v-else</code> directive to handle the opposite case of when a user is not logged in.
            We could have used another <code class="language-">v-if</code> directive here with <code
              class="language-">!isLoggedIn</code> but instead we us <code class="language-">v-else</code> which is
            a slightly shorter syntax. The <code class="language-">v-else</code> directive must occur on the element
            directly after an element using the <code class="language-">v-if </code> directive. If we test this in the
          ...
          <p>In the last section we added in a <code class="language-">v-if</code> directive to our AppHeader to
            ...
            we manually navigate to <code
              class="language-">localhost:8080</code> the AppHeader reverts to saying that we are logged out. So let's
            try and clear up how Vuex works in this respect because there are some misconceptions. So inside of our
            <code class="language-">auth.js</code> Vuex module we have the initial state of token set to null:
          </p>
          <figure>
<pre><code class="language-javascript">const state = {
    token: null
          ...
          <p>There's any easy fix which we can add to the auth module to make sure that we persist the authentication
            ...
            local storage by writing <code class="language-">localStorage</code> in the browser console. <code
              class="language-">localStorage</code>

            allows us to store variables that will be persisted specifically for the current domain we are on (in our
            case <code class="language-"> localhost:8080</code>. We can use this as a little spot to store a tiny bit
            of information customized for this particular user on their browser on specifically this domain of <code
              class="language-"> localhost:8080</code>. If we were to browse to another domain like for example <code
              class="language-">google.com</code> that same <code class="language-">localStorage</code> would no
            longer be available - <code class="language-">localhost:8080</code> has it's own little bucket of storage.
          </p>
          <p>We can take this access_token and put it into this <code class="language-">localStorage</code> object
            whenever a user signs into the application. Then when they come back to our application at some point in the
            future we can look inside that <code class="language-">localStorage</code> and see if that access token is
    ...
    token: null
          ...
          <p>So by default the token is set to null which equates to not logged in. So rather than starting off with
            null in every single case we're going to replace this and say that we're going to look inside that <code
              class="language-">localStorage</code> object and check to see if we have saved an access token:
          </p>
          <figure>
<pre><code class="language-javascript">const state = {
    token: window.localStorage.getItem('imgur_token');
          ...
          <p>So now every time our application starts up we're going to look to see is there's been a token stored and
            ...
            in the <code class="language-">finalizeLogin</code> action after we commit the token to the <code
              class="language-">setToken</code> mutation - we can also take that token and add it to our <code
              class="language-">localStorage</code> :
          </p>
          <figure>
<pre><code class="language-javascript">finalizeLogin({ commit }, hash) {
    ...
    window.localStorage.setItem('imgure_token', queryString.access_token);
          ...
          <p>Now, anytime a user signs in for the first time, we're going to take that token and store it on <code
            class="language-">localStorage</code> and then any time a user comes back to our application we'll check
          ...
          <p>Flip back over to the browser and test our authentication flow again. If we refresh, then click the Login
            button. Once we return to our application our <code class="language-">finalizeLogin</code> action is going
            to take that token store it in <code class="language-">localStorage</code> . We can confirm this by
            writing <code
              class="language-">localStorage.getItem('imgur_token')</code> I will see my token stored inside there. So
          ...
          <p>So now I should be able to navigate back to <code class="language-">localhost:8080</code> do a complete
...
&lt;/div&gt;</code></pre>
          ...
          <figure>
          <pre><code class="language-html">&lt;div class=&quot;right menu&quot;&gt;
  &lt;div v-if=&quot;isLoggedIn&quot;&gt;</code></pre>
            <figcaption>Fig 06-045</figcaption>
          </figure>
          <p>We want to keep this <code class="language-">div</code> because we need to use the <code
            class="language-">v-if</code> directive to make sure that we either show the set of buttons or the anchor
            ...
            library that we are using does not expect there to be a bunch of anchor tags inside of this <code
              class="language-">right menu</code> class. We're basically breaking the rules of what SemanticUI expects
            to see but for a very good reason - so we can use that <code class="language-">v-if</code> directive.
...
<pre><code class="language-html">&lt;style scoped&gt;
  ...
  &lt;/style&gt;</code></pre>
          ...
          <figure>
            <pre><code class="language-html">&lt;div v-if=&quot;isLoggedIn&quot; class=&quot;horizontal&quot;
              &gt;</code></pre>
          ...
          <p>Next we will work on the logout functionality to make sure that when a user clicks the Logout button the
            <code class="language-">logout</code> action is executed which will call the <code
              class="language-">setToken</code> mutation which in turn sets the token in state to null. If token is
          ...
          <p>In your code editor navigate to the AppHeader component's definition. You'll recall that to get an action
            wired up to a component we make use of the <code class="language-">mapActions</code> function. So we
...
<pre><code class="language-javascript">export default {
    ...
    methods:
        mapActions(['login', 'logout'])
          ...
          <p>So now we can make use of this <code class="language-">logout</code> action that's been added to the
            <code class="language-">methods</code> object by going up to the <code
              class="language-">logout</code> anchor tag and adding a click event handler to it:
          </p>
          <figure>
            <pre><code class="language-html"> &lt;a class=&quot;item&quot; @click=&quot;logout&quot;&gt;Logout&lt;
              /a&gt;</code></pre>
          ...
          <p>In the last section we added the Logout functionality. Anytime the user clicks on the Logout button the
            ...
            the button it calls the <code class="language-">logout</code> action defined inside of our Auth module.
            ...
            call the <code
              class="language-">logout</code> action to update <code class="language-">isLoggedIn</code> in the Auth
            module the getter of <code
              class="language-">isLoggedIn</code> automatically got rerun as well and caused AppHeader component to
          ...
          <p>Ok, our application is looking pretty good, but there's still some rough edges around our authentication
            ...
            to <code class="language-">localhost:8080</code> the application thinks that I am still logged in. The
            reason that the app thinks we're still logged in is that even though we are updating our piece of state -
            <code class="language-">token</code> when we call that <code class="language-">logout</code> action
            inside of our Auth module we are not emptying ouf the <code class="language-">localStorage</code> record
            of that access token.
          </p>
          <p>We need to make sure that anytime the <code class="language-">logout</code> action is called we also
            clear out that imgur_token <code class="language-">localStorage</code> item so that when a user refreshes
            the page that tokens not inside of <code class="language-">localStorage</code> :
          </p>
          <figure>
<pre><code class="language-javascript">logout: ({ commit }) =&gt; {
    commit('setToken', null);
    window.localStorage.removeItem('imgur_token');
  }</code></pre>
            <figcaption>Fig 06-050</figcaption>
          </figure>
          <p>To clear out an item that has been defined inside of <code class="language-">localStorage</code> we use a
            function called <code class="language-">removeItem</code> passing the name/key of the property -
          ...
          <p>Another issue we have is when a user comes back to our application from the Oauth2 flow we always see the
            ...
            back to the <code class="language-">/oauth2/callback</code> route that we get access to their token, we do
            some work on it, then what we need to do is redirect them to a suitable location in our application - in our
            case our root route - <code class="language-">localhost:8080</code>.
          </p>
          <p>You have already seen an example of us manipulating the user's browser by setting the <code
            class="language-">window.location</code> property to the Url that we wanted the browser to navigate to.
    ...
    routes: [{ path: '/oauth2/callback', component
          ...
          <p>This router object allows us to programmatically navigate our users around our application. So really what
            we want to do is get access to this router object at the point we need to redirect our user (the <code
              class="language-">finalizeLogin</code> action in this application). To achieve that we export the router
            object from the <span class="filename">main.js</span> file.</p>
          <figure>
<pre><code class="language-javascript">export const router = new VueRouter ({
    mode: 'history',
    routes: [
        { path: '/oauth2/callback', component: AuthHandler }
  ]
          ...
          <p>Notice the use of the curly braces. That's because this export of router we setup above in <span
            ...
            class="language-">default</code> keyword for the router which is why we need the curly braces on the
...
<pre><code class="language-javascript">finalizeLogin({ commit }, hash) {
    ...
    router.push('/');
          ...
          <p>In the browser, Login to the application and you should see that after <code class="language-">/oauth2/callback/</code>
            the browser is redirected to <code class="language-">localhost:8080</code>. So that's the automatic
            refresh in action.
          </p>
...
&lt;script&gt;
  ...
  &lt;/script&gt;</code></pre>
          ...
          <figure>
<pre><code class="language-javascript">import ImageList from './components/ImageList';
  import UploadForm from './components/UploadForm';</code></pre>
            <figcaption>Fig 06-058</figcaption>
          </figure>
    ...
    routes: [
        ...
        { path: '/oauth2/callback', component: AuthHandler },
          ...
          <p>Back in the browser reload the page and make sure the address is set to <code
            class="language-">localhost:8080</code>. You should see the ImageList component on the screen:
          </p>
          <figure>
            <img src="./images/vuejsessentials/Fig06-060.png"/>
            <figcaption>Fig 06-060</figcaption>
          </figure>
          <p>Now if you browse to <code class="language-">localhost:8080/upload</code> you should see the UploadForm
            component appear:
          </p>
          <figure>
          ...
          <p>Now that we've created the UploadForm and ImageList components we need to make sure that the user has the
            ...
            router provides the <code class="language-">router-link</code> tag which still renders an anchor tag but
          ...
          <p>Update the Galleries, Upload and Image Storage links in the AppHeader to use <code class="language-">router-link</code>
            tags instead of
            anchor tags:
          </p>
          <figure>
<pre><code class="language-html">&lt;template&gt;
  ...
  &lt;/template&gt;</code></pre>
          ...
          <p>Note that we do not change all the anchor tags to <code class="language-">router-link</code> tags - only
            ...
            the <code class="language-">logout</code> action. The same is true for the Login tag.
          </p>
          <p>In the browser if you click on one of the <code class="language-">router-link</code> tags and then
